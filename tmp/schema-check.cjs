// 查 profiles / tenant_memberships 表结构（必填列）
const fs = require("fs");
const os = require("os");
...
console.log("=== profiles 列 ===");
console.log(runQuery(`
  select column_name, is_nullable, column_default
  from information_schema.columns where table_name='profiles' order by ordinal_position;
`).trim());

console.log("\n=== tenant_memberships 列 ===");
console.log(runQuery(`
  select column_name, is_nullable, column_default
  from information_schema.columns where table_name='tenant_memberships' order by ordinal_position;
`).trim());
