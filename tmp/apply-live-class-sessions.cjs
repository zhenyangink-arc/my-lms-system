...
console.log("=== EXECUTING 202608070007 ===");
const sql = fs.readFileSync("supabase/migrations/202608070007_live_class_sessions.sql", "utf8");
try {
...
try {
  const out = runQuery(
    "insert into supabase_migrations.schema_migrations (version, name, statements) values ('202608070007', 'live_class_sessions', '{}') on conflict (version) do nothing;"
  );
  console.log(`OK: ${out.trim().slice(0, 300) || "(empty)"}`);
} catch (error) {
  const message = error.stderr ? error.stderr.toString() : error.message;
...
const verify = runQuery(
  "select count(*) as sessions_tables from information_schema.tables where table_name='live_class_sessions';"
);
