...
console.log("=== EXECUTING 202608070009 ===");
const sql = fs.readFileSync("supabase/migrations/202608070009_realtime_channel_auth.sql", "utf8");
try {
...
try {
  const out = runQuery(
    "insert into supabase_migrations.schema_migrations (version, name, statements) values ('202608070009', 'realtime_channel_auth', '{}') on conflict (version) do nothing;"
  );
  console.log(`OK: ${out.trim().slice(0, 300) || "(empty)"}`);
} catch (error) {
  const message = error.stderr ? error.stderr.toString() : error.message;
...
console.log("=== VERIFY: realtime.messages policies ===");
console.log(runQuery(`
  select policyname, cmd from pg_policies where schemaname='realtime' and tablename='messages' order by policyname;
`).trim());
