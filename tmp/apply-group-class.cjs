// 应用迁移：202608070010 group_class_members
const fs = require("fs");
const os = require("os");
...
console.log("=== EXECUTING 202608070010 ===");
const sql = fs.readFileSync("supabase/migrations/202608070010_group_class_members.sql", "utf8");
try {
  ...
  console.log(`FAIL: ${message.slice(0, 4000)}`);
  process.exit(1);
}
...
try {
  const out = runQuery(
    "insert into supabase_migrations.schema_migrations (version, name, statements) values ('202608070010', 'group_class_members', '{}') on conflict (version) do nothing;"
  );
  console.log(`OK: ${out.trim().slice(0, 300) || "(empty)"}`);
} catch (error) {
  const message = error.stderr ? error.stderr.toString() : error.message;
