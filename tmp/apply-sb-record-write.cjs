...
const sql = fs.readFileSync("supabase/migrations/202608070006_teacher_learning_record_write.sql", "utf8");
console.log("=== EXECUTING 202608070006 ===");
try {
...
try {
  const out = runQuery(
    "insert into supabase_migrations.schema_migrations (version, name, statements) values ('202608070006', 'teacher_learning_record_write', '{}') on conflict (version) do nothing;"
  );
  console.log(`OK: ${out.trim().slice(0, 300) || "(empty)"}`);
} catch (error) {
  const message = error.stderr ? error.stderr.toString() : error.message;
...
const verify = runQuery(
  "select count(*) as fns from pg_proc where proname in ('save_learning_record_note','change_learning_record_note_status') and prosrc like '%tenant_student_assignments%';"
);
