...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第06课 얼마예요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180012_chapter_six_golden_smart_textbook.sql",
);
const sourcePath =
...
function fail(message) {
  process.stderr.write(`chapter-six conversion failed: ${message}\n`);
  process.exit(1);
}
...
const assertions = [
  [Number(frontmatter.chapter_number) === 6, "chapter_number must be 6"],
  [frontmatter.chapter_slug === "shopping", "chapter_slug must stay shopping"],
  [frontmatter.chapter_test_slug === "korean-level-one-06", "chapter test slug mismatch"],
...
for (const id of [
  "chapter-06-listening-total-normal",
  "chapter-06-listening-total-slow",
]) {
  if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
}
process.stdout.write(
  `PASS chapter-six conversion: ${moduleCodes.length} modules, ${vocabulary.length} vocabulary items, ` +
