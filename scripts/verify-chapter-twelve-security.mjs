...
try {
  ...
  const pendingListeningResult = await invokeAction({ activityId: listeningActivity.id, locale: "zh-CN", response: 1 });
  ...
  const weakWriting = await invokeAction({ activityId: writingActivity.id, locale: "zh-CN", response: { text: "가나다. 라마바. 사아자. 차카타. 파하가. 나다라. 마바사.", informationKinds: Array(7).fill(true), rubricConfirmed: true } });
