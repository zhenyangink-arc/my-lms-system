...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第14课 이 옷을 입어 보세요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180020_chapter_fourteen_golden_smart_textbook.sql",
);
const sourcePath = process.argv.find((value) => value.endsWith(".md")) ?? defaultSource;
const checkOnly = process.argv.includes("--check");

function fail(message) {
  process.stderr.write(`chapter-fourteen conversion failed: ${message}\n`);
  process.exit(1);
}
...
const assertions = [
  [Number(frontmatter.chapter_number) === 14, "chapter_number must be 14"],
  [frontmatter.chapter_slug === "clothing", "chapter_slug must stay clothing"],
  [frontmatter.chapter_test_slug === "korean-level-one-14", "chapter test slug mismatch"],
  ...
  [vocabulary.length === 28, `expected 28 vocabulary rows, got ${vocabulary.length}`],
...
for (const id of ["chapter-14-listening-recipient-normal", "chapter-14-listening-recipient-slow"]) {
  if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
}
process.stdout.write(
  `PASS chapter-fourteen conversion: ${moduleCodes.length} modules, ${vocabulary.length} vocabulary items, ` +
