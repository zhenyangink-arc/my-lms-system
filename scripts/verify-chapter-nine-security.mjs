...
const GOLDEN_QUESTIONS = [
  { key: "golden-09-01", prompt: "“요리사”是什么意思？", options: ["厨师", "公司职员", "老师", "医生"], answer: 0 },
  { key: "golden-09-02", prompt: "哪一项正确完成“수진 씨___ 아버지”？", options: ["의", "를", "은", "도"], answer: 0 },
  { key: "golden-09-03", prompt: "哪一句正确表示“妈妈很会做饭”？", options: ["어머니는 요리를 잘하세요.", "어머니는 요리의 잘하세요.", "어머니는 요리가 잘하세요.", "어머니는 요리를 잘해요세요."], answer: 0 },
  { key: "golden-09-04", prompt: "尊敬地说明“爸爸是公司职员”时，哪一句正确？", options: ["아버지는 회사원이세요.", "아버지는 회사원세요.", "아버지는 회사원이에요세요.", "아버지는 회사원해요."], answer: 0 },
  { key: "golden-09-05", prompt: "尊敬地说明母亲在餐厅工作时，哪一句正确？", options: ["어머니는 식당에서 일하세요.", "어머니는 식당에서 일해요세요.", "어머니는 식당에서 일 하세요.", "저는 식당에서 일하세요."], answer: 0 },
  { key: "golden-09-06", prompt: "主场景中，秀珍母亲的职业是什么？", options: ["厨师", "公司职员", "老师", "医生"], answer: 0 },
  { key: "golden-09-07", prompt: "主场景中，秀珍父亲擅长什么？", options: ["运动", "唱歌", "做饭", "拍照"], answer: 0 },
  { key: "golden-09-08", prompt: "私有听力中，민지的爷爷从事什么职业？", options: ["老师", "医生", "厨师", "公司职员"], answer: 0 },
  { key: "golden-09-09", prompt: "家庭照片消息是谁的家庭照？", options: ["유나 씨", "민지 씨", "수진 씨", "다니엘 씨"], answer: 0 },
  { key: "golden-09-10", prompt: "消息中有娜的母亲擅长什么？", options: ["唱歌", "运动", "做饭", "拍照"], answer: 0 },
  { key: "golden-09-11", prompt: "双角色家庭照片交流的自然信息顺序是哪一项？", options: ["身份询问→第一人关系/职业→能力或活动→第二人关系/职业→能力或活动→收束", "职业列表→单人背诵→结束", "先说答案→再问身份→只介绍一人", "只说活动→不说明人物关系"], answer: 0 },
  { key: "golden-09-12", prompt: "课末家庭照片交流必须满足哪一项？", options: ["45—60秒、8—10轮、双角色并介绍至少两位尊敬对象", "只介绍一人即可", "必须获得自动发音分数", "写三句即可，无需录音"], answer: 0 },
];
...
const ACTIVITY_FIXTURES = [
  ...
  { key: "grammar-fill", type: "fill_blank", response: ["의", "를", "이세요", "세요", "일하세요", "운동하세요"], correct: true },
  { key: "pattern-order", type: "ordering", response: [2, 4, 3, 0, 5, 1], correct: true },
  ...
  { key: "listening-family-job", type: "listening", response: 0, correct: true },
  { key: "speaking-family-introduction", type: "speaking", response: { recorded: true, durationSeconds: 52, turns: 8, criteria: Array(7).fill(true) }, correct: null },
  { key: "reading-family-message", type: "single_choice", response: [0, 2, 2], correct: true },
  { key: "write-family-introduction", type: "writing", response: { text: "이 사진은 제 가족사진이에요. 이분은 제 어머니세요. 어머니는 요리사세요. 요리를 잘하세요. 이분은 제 아버지세요. 아버지는 회사원이세요. 공원에서 운동하세요.", informationKinds: Array(5).fill(true), rubricConfirmed: true }, correct: null },
  { key: "review-multiple", type: "multiple_choice", response: [0, 1, 2, 4], correct: true },
    ...
    registerHooks({
      resolve(specifier, context, nextResolve) {
        ...
        if (specifier === "next/headers") {
          return {
            shortCircuit: true,
            url: "data:text/javascript,export%20async%20function%20cookies()%7Breturn%20globalThis.__CHAPTER_NINE_SECURITY_COOKIE_STORE__%7D",
          };
        }
...
async function createAuthenticatedActionInvoker(email, password) {
  ...
  globalThis.__CHAPTER_NINE_SECURITY_COOKIE_STORE__ = {
    getAll() {
      return [...cookieJar].map(([name, value]) => ({ name, value }));
    },
    set(name, value) {
...
try {
  ...
  const chapter = await mustData(admin.from("digital_textbook_chapters").select("id,status,production_status,native_review_status,audio_status,image_status,source_revision").eq("version_id", version.id).eq("chapter_number", 9).single(), "load chapter nine");
  const priorTests = await mustData(admin.from("chapter_tests").select("id,slug,status").in("slug", ["korean-level-one-01", "korean-level-one-02", "korean-level-one-03", "korean-level-one-04", "korean-level-one-05", "korean-level-one-06", "korean-level-one-07", "korean-level-one-08"]).order("slug"), "load prior tests");
  const testRow = await mustData(admin.from("chapter_tests").select("id,status,passing_score,duration_minutes").eq("slug", "korean-level-one-09").single(), "load chapter nine test");
  ...
  assert.match(chapter.source_revision, /6e6ea21f488d64bcc6bd87bd148cd0b575d3e1679f291239084dc63c7c1e6a79/);
  ...
  console.log("PASS: chapter nine and its hard-coded 12-question vector remain draft/pending and use master-recorded 12 minutes / passing_score=60");
  ...
  assert.equal(objectValue(nodes.find((item) => item.node_code === "family-and-work").content).vocabulary.length, 20);
  assert.equal(objectValue(nodes.find((item) => item.node_code === "respectful-family-forms").content).grammarCards.length, 4);
  assert.deepEqual(objectValue(nodes.find((item) => item.node_code === "family-photo-talk").content).dialogueScenes.map((scene) => scene.lines.length), [8, 6]);
  ...
  for (const key of [
    ...
    "dialogue-fact-check",
    "dialogue-response",
    "listening-family-job",
    "reading-family-message",
    "review-multiple",
  ]) {
  ...
  const speakingConfig = objectValue(activitiesByKey.get("speaking-family-introduction").public_config);
  assert.equal(speakingConfig.minimumSeconds, 45);
  assert.equal(speakingConfig.maximumSeconds, 60);
  assert.equal(speakingConfig.minimumTurns, 8);
  assert.equal(speakingConfig.maximumTurns, 10);
  assert.equal(speakingConfig.requiredCriteria, 7);
  assert.equal(speakingConfig.pronunciationScore, false);
  const writingConfig = objectValue(activitiesByKey.get("write-family-introduction").public_config);
  assert.equal(writingConfig.minSentences, 6);
  assert.equal(writingConfig.maxSentences, 7);
  assert.equal(writingConfig.minimumInformationKinds, 5);
  assert.equal(writingConfig.requireCompletionChecklist, true);
  const listeningActivity = activitiesByKey.get("listening-family-job");
  ...
  assert.equal(listeningSecret.audio_object_key, "korean-level-one/chapter-09/listening/chapter-09-listening-family-job.mp3");
  ...
  assert.ok(media.every((item) => item.object_key.startsWith("korean-level-one/chapter-09/")));
  const normalAudio = media.find((item) => item.asset_key === "chapter-09-listening-family-job-normal");
  const slowAudio = media.find((item) => item.asset_key === "chapter-09-listening-family-job-slow");
  ...
  console.log("PASS: 8 nodes, 12 contracts, private answers, 12 pending images and 70 pending audio bindings match chapter nine");
  ...
  const email = `chapter-nine-security-${Date.now()}@accounts.puffy.invalid`;
  const password = "LocalSecurity123!";
  const { data: createdUser, error: createUserError } = await admin.auth.admin.createUser({
    email,
    password,
    email_confirm: true,
    user_metadata: { full_name: "第09章本地安全测试" },
  });
  ...
  console.log("PASS: authenticated browser cannot read chapter-nine answer secrets or private object keys");
  ...
  const speakingActivity = activitiesByKey.get("speaking-family-introduction");
  const forgedOpen = await admin.rpc("record_smart_textbook_attempt", { p_tenant_id: tenantId, p_student_id: userId, p_activity_id: speakingActivity.id, p_version_id: version.id, p_response: ACTIVITY_FIXTURES.find((item) => item.key === "speaking-family-introduction").response, p_is_correct: true, p_score: 100, p_meets_completion_requirements: true });
  ...
  const writingActivity = activitiesByKey.get("write-family-introduction");
  ...
  assert.equal(await mustData(admin.from("course_ebook_progress").select("id").eq("student_id", userId).eq("test_slug", "korean-level-one-09").maybeSingle(), "check locked chapter"), null);
  ...
  const unlockEvidence = await mustData(admin.from("course_ebook_progress").select("progress_percent,completion_source,reading_seconds").eq("student_id", userId).eq("test_slug", "korean-level-one-09").single(), "load unlock evidence");
  ...
  const { data: testResult, error: testError } = await userClient.rpc("submit_course_test", { p_test_slug: "korean-level-one-09", p_answers: answers });
  ...
  console.log("PASS: exactly 8 completed nodes write smart_textbook evidence and unlock the chapter-nine test after prior prerequisites");
} catch (error) {
  testFailure = error;
} finally {
...
if (cleanupFailures.length > 0) {
  throw new AggregateError(testFailure ? [testFailure, ...cleanupFailures] : cleanupFailures, "chapter-nine security verification and/or cleanup failed");
}
if (testFailure) throw testFailure;
console.log("PASS: cleanup restored every chapter-nine local fixture with no suppressed errors");
