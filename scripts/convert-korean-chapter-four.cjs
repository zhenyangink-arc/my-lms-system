...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第04课 어디에 있어요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180010_chapter_four_golden_smart_textbook.sql",
);
const sourcePath =
...
function fail(message) {
  process.stderr.write(`chapter-four conversion failed: ${message}\n`);
  process.exit(1);
}
...
const assertions = [
  [Number(frontmatter.chapter_number) === 4, "chapter_number must be 4"],
  [frontmatter.chapter_slug === "location", "chapter_slug must stay location"],
  [frontmatter.chapter_test_slug === "korean-level-one-04", "chapter test slug mismatch"],
  ...
  [grammarCards.length === 3, `expected 3 grammar cards, got ${grammarCards.length}`],
  [mainDialogue.length === 9, `expected 9 main dialogue turns, got ${mainDialogue.length}`],
  ...
  [images.length === 11, `expected 11 image bindings, got ${images.length}`],
...
for (const id of [
  "chapter-04-listening-campus-location-normal",
  "chapter-04-listening-campus-location-slow",
]) {
  if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
}
process.stdout.write(
  `PASS chapter-four conversion: ${moduleCodes.length} modules, ${vocabulary.length} vocabulary items, ` +
