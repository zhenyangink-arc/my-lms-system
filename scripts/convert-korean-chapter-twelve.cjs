...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第12课 여보세요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180018_chapter_twelve_golden_smart_textbook.sql",
);
const sourcePath = process.argv.find((value) => value.endsWith(".md")) ?? defaultSource;
const checkOnly = process.argv.includes("--check");

function fail(message) {
  process.stderr.write(`chapter-twelve conversion failed: ${message}\n`);
  process.exit(1);
}
...
function parseTable(block, headerStartsWith) {
  ...
  for (const line of lines.slice(headerIndex + 2)) {
    ...
    const values = line.split("|").slice(1, -1).map((value) => value.trim().replaceAll("`", ""));
...
const grammarCards = [...grammarBlock.matchAll(/^### 5\.\d+ (.+?)｜/gm)]
...
const moduleCodes = [...section(markdown, "### 3.0 学习地图", "### 3.1")
  .matchAll(/\| \d+ \| `([^`]+)` \| `([^`]+)`/g)]
  .map((match) => ({ moduleCode: match[1], nodeCode: match[2] }));
const activityBlock = section(markdown, "## 9. 交互活动合同", "## 10.");
const activityHeadings = [...activityBlock.matchAll(/^### 9\.\d+ `([^`]+)`$/gm)].map((match) => match[1]);
const declaredKeys = [...activityBlock.matchAll(/- `activity_key`：`([^`]+)`/g)].map((match) => match[1]);
const feedbackCounts = activityHeadings.map((key, index) => {
  ...
  const nextHeading = index + 1 < activityHeadings.length
    ? `### 9.${index + 2} \`${activityHeadings[index + 1]}\`` : "## 10.";
...
const assertions = [
  [Number(frontmatter.chapter_number) === 12, "chapter_number must be 12"],
  [frontmatter.chapter_slug === "phone-call", "chapter_slug must stay phone-call"],
  [frontmatter.chapter_test_slug === "korean-level-one-12", "chapter test slug mismatch"],
  ...
  [grammarCards.length === 5, `expected 5 grammar cards, got ${grammarCards.length}`],
  [mainDialogue.length === 10, `expected 10 main dialogue turns, got ${mainDialogue.length}`],
  [alternateDialogue.length === 8, `expected 8 alternate dialogue turns, got ${alternateDialogue.length}`],
  ...
  [images.length === 13, `expected 13 image bindings, got ${images.length}`],
  [images.every((image) => image["状态"] === "待制作"), "all images must remain 待制作"],
];
for (const [valid, message] of assertions) if (!valid) fail(message);

const publicConfigLines = activityBlock.split(/\r?\n/).filter((line) => line.includes("`public_config`"));
for (const key of ["normal_script", "slow_script", "pause_marks", "audio_object_key", "correct_index", "correct_indices", "correct_order", "answers="]) {
  if (publicConfigLines.some((line) => line.includes(key))) fail(`private field ${key} leaked into a public_config declaration`);
}

const converted = { source: sourcePath, sourceSha256, chapter: frontmatter, moduleCodes, vocabulary,
  grammarCards, dialogues: { main: mainDialogue, alternate: alternateDialogue }, activities: activityHeadings, images };
if (!checkOnly) {
...
if (!migration.includes(`source_sha256: ${sourceSha256}`)) fail("migration source hash does not match the current master markdown");
for (const activityKey of activityHeadings) {
  if (!migration.includes(`\"key\":\"${activityKey}\"`)) fail(`migration is missing activity ${activityKey}`);
}
...
for (const id of ["chapter-12-listening-callback-normal", "chapter-12-listening-callback-slow"]) {
  if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
}
process.stdout.write(
  `PASS chapter-twelve conversion: ${moduleCodes.length} modules, ${vocabulary.length} vocabulary items, ` +
