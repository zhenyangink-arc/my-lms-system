...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第10课 지금 몇 시예요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180016_chapter_ten_golden_smart_textbook.sql",
);
const sourcePath = process.argv.find((value) => value.endsWith(".md")) ?? defaultSource;
const checkOnly = process.argv.includes("--check");

function fail(message) {
  process.stderr.write(`chapter-ten conversion failed: ${message}\n`);
  process.exit(1);
}
...
const assertions = [
  [Number(frontmatter.chapter_number) === 10, "chapter_number must be 10"],
  [frontmatter.chapter_slug === "time-schedule", "chapter_slug must stay time-schedule"],
  [frontmatter.chapter_test_slug === "korean-level-one-10", "chapter test slug mismatch"],
...
for (const id of ["chapter-10-listening-library-time-normal", "chapter-10-listening-library-time-slow"]) {
  if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
}
process.stdout.write(
  `PASS chapter-ten conversion: ${moduleCodes.length} modules, ${vocabulary.length} vocabulary items, ` +
