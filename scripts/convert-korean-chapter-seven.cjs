...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第07课 날씨가 어때요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180013_chapter_seven_golden_smart_textbook.sql",
);
const sourcePath =
...
function fail(message) {
  process.stderr.write(`chapter-seven conversion failed: ${message}\n`);
  process.exit(1);
}
...
const assertions = [
  [Number(frontmatter.chapter_number) === 7, "chapter_number must be 7"],
  [frontmatter.chapter_slug === "weather", "chapter_slug must stay weather"],
  [frontmatter.chapter_test_slug === "korean-level-one-07", "chapter test slug mismatch"],
...
for (const id of [
  "chapter-07-listening-forecast-normal",
  "chapter-07-listening-forecast-slow",
]) {
  if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
}
process.stdout.write(
  `PASS chapter-seven conversion: ${moduleCodes.length} modules, ${vocabulary.length} vocabulary items, ` +
