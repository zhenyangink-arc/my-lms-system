...
const GOLDEN_QUESTIONS = [
  { key: "golden-04-01", prompt: "“뒤”表示哪个方位？", options: ["后面", "前面", "里面", "上面"], answer: 0 },
  { key: "golden-04-02", prompt: "哪一句用于询问洗手间的位置？", options: ["화장실이 어디에 있어요?", "화장실에서 공부해요.", "화장실에 가요.", "지금 몇 시예요?"], answer: 0 },
  { key: "golden-04-03", prompt: "“화장실은 일층___ 있어요.”应填什么？", options: ["에", "에서", "을", "와"], answer: 0 },
  { key: "golden-04-04", prompt: "哪一句正确表达“去图书馆”？", options: ["도서관에 가요.", "도서관에서 가요.", "도서관을 가요.", "도서관이 있어요."], answer: 0 },
  { key: "golden-04-05", prompt: "哪一句正确表达“图书馆在主楼后面”？", options: ["도서관은 본관 뒤에 있어요.", "도서관은 뒤에 본관 있어요.", "도서관은 본관에서 뒤에 있어요.", "도서관에 본관 뒤가 있어요."], answer: 0 },
  { key: "golden-04-06", prompt: "“가요”和“와요”的选择主要依据什么？", options: ["相对说话人或交谈基准点的移动方向", "地点有没有收音", "地点在几楼", "说话人的年龄"], answer: 0 },
  { key: "golden-04-07", prompt: "主场景中，洗手间在哪里？", options: ["一楼办公室旁边", "二楼教室旁边", "三楼休息室前面", "主楼后面"], answer: 0 },
  { key: "golden-04-08", prompt: "第二场景中，敏浩要去哪里？", options: ["学生食堂", "图书馆", "办公室", "休息室"], answer: 0 },
  { key: "golden-04-09", prompt: "根据主场景，怎样回答“도서관은 어디에 있어요?”", options: ["본관 뒤에 있어요.", "도서관에 가요.", "일층에 와요.", "공책 주세요."], answer: 0 },
  { key: "golden-04-10", prompt: "主楼 안내 卡中，学生休息室在几楼？", options: ["三楼", "一楼", "二楼", "主楼外"], answer: 0 },
  { key: "golden-04-11", prompt: "位置问答的自然顺序是哪一项？", options: ["问位置→答楼层→追问→答具体方位→致谢", "致谢→答方位→问位置→追问→答楼层", "答楼层→问位置→致谢→追问→答方位", "问位置→致谢→答楼层→答方位→追问"], answer: 0 },
  { key: "golden-04-12", prompt: "课末校园向导录音必须满足哪一项？", options: ["35—50秒、至少8轮、三地点、两种方位和目的地问答", "只背诵地点清单", "必须获得自动发音分数", "只需一个角色说5句"], answer: 0 },
];
...
const ACTIVITY_FIXTURES = [
  ...
  { key: "grammar-fill", type: "fill_blank", response: ["에", "에", "에", "에", "옆에"], correct: true },
  ...
  { key: "listening-campus-location", type: "listening", response: 0, correct: true },
  { key: "speaking-campus-guide", type: "speaking", response: { recorded: true, durationSeconds: 40, turns: 8, criteria: Array(6).fill(true) }, correct: null },
  { key: "reading-building-note", type: "single_choice", response: [0, 0, 0], correct: true },
  { key: "writing-location-note", type: "writing", response: { text: "여기가 우리 학교예요. 교실은 본관 이층에 있어요. 사무실은 교실 옆에 있어요. 도서관은 본관 앞에 있어요. 저는 지금 도서관에 가요.", informationKinds: Array(6).fill(true), rubricConfirmed: true }, correct: null },
    ...
    registerHooks({
      resolve(specifier, context, nextResolve) {
        ...
        if (specifier === "next/headers") {
          return {
            shortCircuit: true,
            url: "data:text/javascript,export%20async%20function%20cookies()%7Breturn%20globalThis.__CHAPTER_FOUR_SECURITY_COOKIE_STORE__%7D",
          };
        }
...
async function createAuthenticatedActionInvoker(email, password) {
  ...
  globalThis.__CHAPTER_FOUR_SECURITY_COOKIE_STORE__ = {
    getAll() {
      return [...cookieJar].map(([name, value]) => ({ name, value }));
    },
    set(name, value) {
...
assertInvalid("strict speaking duration", { kind: "open" }, { recorded: true, durationSeconds: "40", turns: 8, criteria: Array(6).fill(true) }, "speaking", { minimumSeconds: 35, maximumSeconds: 50, minimumTurns: 8, requiredCriteria: 6, enforceCompletionRequirements: true }, []);
...
try {
  ...
  const chapter = await mustData(admin.from("digital_textbook_chapters").select("id,status,production_status,native_review_status,audio_status,image_status,source_revision").eq("version_id", version.id).eq("chapter_number", 4).single(), "load chapter four");
  const priorTests = await mustData(admin.from("chapter_tests").select("id,slug,status").in("slug", ["korean-level-one-01", "korean-level-one-02", "korean-level-one-03"]).order("slug"), "load prior tests");
  const testRow = await mustData(admin.from("chapter_tests").select("id,status,passing_score,duration_minutes").eq("slug", "korean-level-one-04").single(), "load chapter four test");
  ...
  assert.match(chapter.source_revision, /9c250e59a45e3654a1d724d28740a4a628b3ac44dbf90510bf2a1566d9b4e9a6/);
  ...
  console.log("PASS: chapter four and its hard-coded 12-question vector remain draft/pending and use master-recorded 12 minutes / passing_score=60");
  ...
  assert.equal(objectValue(nodes.find((item) => item.node_code === "campus-and-position").content).vocabulary.length, 22);
  assert.equal(objectValue(nodes.find((item) => item.node_code === "location-and-destination").content).grammarCards.length, 3);
  assert.deepEqual(objectValue(nodes.find((item) => item.node_code === "campus-guide-dialogue").content).dialogueScenes.map((scene) => scene.lines.length), [9, 6]);
  ...
  const listeningActivity = activitiesByKey.get("listening-campus-location");
  ...
  assert.equal(listeningSecret.audio_object_key, "korean-level-one/chapter-04/listening/chapter-04-listening-campus-location.mp3");
  ...
  assert.equal(media.filter((item) => item.media_type === "image").length, 11);
  assert.equal(media.filter((item) => item.media_type === "audio").length, 72);
  assert.ok(media.every((item) => item.production_status === "pending"));
  assert.ok(media.every((item) => item.object_key.startsWith("korean-level-one/chapter-04/")));
  const normalAudio = media.find((item) => item.asset_key === "chapter-04-listening-campus-location-normal");
  const slowAudio = media.find((item) => item.asset_key === "chapter-04-listening-campus-location-slow");
  ...
  console.log("PASS: 8 nodes, 12 contracts, private answers, 11 pending images and 72 pending audio bindings match chapter four");
  ...
  const email = `chapter-four-security-${Date.now()}@accounts.puffy.invalid`;
  const password = "LocalSecurity123!";
  const { data: createdUser, error: createUserError } = await admin.auth.admin.createUser({
    email,
    password,
    email_confirm: true,
    user_metadata: { full_name: "第04章本地安全测试" },
  });
  ...
  console.log("PASS: authenticated browser cannot read chapter-four answer secrets or private object keys");
  ...
  const speakingActivity = activitiesByKey.get("speaking-campus-guide");
  const forgedOpen = await admin.rpc("record_smart_textbook_attempt", { p_tenant_id: tenantId, p_student_id: userId, p_activity_id: speakingActivity.id, p_version_id: version.id, p_response: ACTIVITY_FIXTURES.find((item) => item.key === "speaking-campus-guide").response, p_is_correct: true, p_score: 100, p_meets_completion_requirements: true });
  ...
  const writingActivity = activitiesByKey.get("writing-location-note");
  const weakWriting = await invokeAction({ activityId: writingActivity.id, locale: "zh-CN", response: { text: "가. 나. 다. 라. 마.", informationKinds: Array(6).fill(true), rubricConfirmed: true } });
  ...
  assert.equal(await mustData(admin.from("course_ebook_progress").select("id").eq("student_id", userId).eq("test_slug", "korean-level-one-04").maybeSingle(), "check locked chapter"), null);
  ...
  const unlockEvidence = await mustData(admin.from("course_ebook_progress").select("progress_percent,completion_source,reading_seconds").eq("student_id", userId).eq("test_slug", "korean-level-one-04").single(), "load unlock evidence");
  ...
  const { data: testResult, error: testError } = await userClient.rpc("submit_course_test", { p_test_slug: "korean-level-one-04", p_answers: answers });
  ...
  console.log("PASS: exactly 8 completed nodes write smart_textbook evidence and unlock the chapter-four test after prior prerequisites");
} catch (error) {
  testFailure = error;
} finally {
...
if (cleanupFailures.length > 0) {
  throw new AggregateError(testFailure ? [testFailure, ...cleanupFailures] : cleanupFailures, "chapter-four security verification and/or cleanup failed");
}
if (testFailure) throw testFailure;
console.log("PASS: cleanup restored every chapter-four local fixture with no suppressed errors");
