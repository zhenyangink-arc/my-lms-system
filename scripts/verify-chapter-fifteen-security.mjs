...
try {
  ...
  const weakWriting = await invokeAction({ activityId: writingActivity.id, locale: "zh-CN", response: { text: "가나다라 마바사아자. 차카타파 하가나다. 라마바사 아자차카. 타파하가 나다라마. 바사아자 차카타파. 하가나다 라마바사. 아자차카 타파하가. 나다라마 바사아자.", informationKinds: Array(10).fill(true), rubricConfirmed: true } });
