...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第15课 여행을 가고 싶어요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180021_chapter_fifteen_golden_smart_textbook.sql",
);
const sourcePath = process.argv.find((value) => value.endsWith(".md")) ?? defaultSource;
const checkOnly = process.argv.includes("--check");

function fail(message) {
  process.stderr.write(`chapter-fifteen conversion failed: ${message}\n`);
  process.exit(1);
}
...
const assertions = [
  [Number(frontmatter.chapter_number) === 15, "chapter_number must be 15"],
  [frontmatter.chapter_slug === "travel-wishes", "chapter_slug must stay travel-wishes"],
  [frontmatter.chapter_test_slug === "korean-level-one-15", "chapter test slug mismatch"],
...
for (const id of ["chapter-15-listening-rain-plan-normal", "chapter-15-listening-rain-plan-slow"]) {
  if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
}
process.stdout.write(
  `PASS chapter-fifteen conversion: ${moduleCodes.length} modules, ${vocabulary.length} vocabulary items, ` +
