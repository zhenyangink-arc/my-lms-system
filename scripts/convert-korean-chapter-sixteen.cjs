...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第16课 우리 집에 올 수 있어요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180022_chapter_sixteen_golden_smart_textbook.sql",
);
const sourcePath = process.argv.find((value) => value.endsWith(".md")) ?? defaultSource;
const checkOnly = process.argv.includes("--check");

function fail(message) {
  process.stderr.write(`chapter-sixteen conversion failed: ${message}\n`);
  process.exit(1);
}
...
const assertions = [
  [Number(frontmatter.chapter_number) === 16, "chapter_number must be 16"],
  [frontmatter.chapter_slug === "invitation", "chapter_slug must stay invitation"],
  [frontmatter.chapter_test_slug === "korean-level-one-16", "chapter test slug mismatch"],
  ...
  [vocabulary.length === 24, `expected 24 vocabulary rows, got ${vocabulary.length}`],
...
for (const key of ["normal_script", "slow_script", "pause_marks", "audio_object_key", "correct_option_id", "correct_option_ids", "correct_order", "answers="]) {
...
for (const id of ["chapter-16-listening-preparation-normal", "chapter-16-listening-preparation-slow"]) {
  if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
}
process.stdout.write(
  `PASS chapter-sixteen conversion: ${moduleCodes.length} modules, ${vocabulary.length} vocabulary items, ` +
