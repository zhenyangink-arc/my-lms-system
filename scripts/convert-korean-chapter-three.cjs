...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第03课 한국어를 공부해요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180009_chapter_three_golden_smart_textbook.sql",
);
const sourcePath =
...
function fail(message) {
  process.stderr.write(`chapter-three conversion failed: ${message}\n`);
  process.exit(1);
}
...
const activityTypes = [
  ...activityBlock.matchAll(/- `activity_type`：`?([^`（\n]+)`?/g),
...
const assertions = [
  [converted.chapter.number === 3, "chapter_number must be 3"],
  [converted.chapter.slug === "daily-actions", "chapter_slug must stay daily-actions"],
  [converted.chapter.testSlug === "korean-level-one-03", "chapter test slug mismatch"],
  ...
  [vocabulary.length === 21, `expected 21 vocabulary rows, got ${vocabulary.length}`],
...
if (checkOnly) {
  ...
  for (const word of converted.vocabulary.map((item) => item["韩语"])) {
    if (!migration.includes(`\"ko\":\"${word}\"`)) {
      fail(`migration is missing vocabulary item ${word}`);
    }
  }
  for (const id of [
    "chapter-03-listening-daily-routine-normal",
    "chapter-03-listening-daily-routine-slow",
  ]) {
    if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
  }
  process.stdout.write(
    `PASS chapter-three conversion: ${moduleCodes.length} modules, ` +
      `${vocabulary.length} vocabulary items, ${grammarCards.length} grammar cards, ` +
      `${mainDialogue.length}+${alternateDialogue.length} dialogue turns, ` +
