...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第11课 감기에 걸렸어요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180017_chapter_eleven_golden_smart_textbook.sql",
);
const sourcePath = process.argv.find((value) => value.endsWith(".md")) ?? defaultSource;
const checkOnly = process.argv.includes("--check");

function fail(message) {
  process.stderr.write(`chapter-eleven conversion failed: ${message}\n`);
  process.exit(1);
}
...
const assertions = [
  [Number(frontmatter.chapter_number) === 11, "chapter_number must be 11"],
  [frontmatter.chapter_slug === "health", "chapter_slug must stay health"],
  [frontmatter.chapter_test_slug === "korean-level-one-11", "chapter test slug mismatch"],
...
for (const id of ["chapter-11-listening-advice-normal", "chapter-11-listening-advice-slow"]) {
  if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
}
process.stdout.write(
  `PASS chapter-eleven conversion: ${moduleCodes.length} modules, ${vocabulary.length} vocabulary items, ` +
