...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第13课 서울역으로 가 주세요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180019_chapter_thirteen_golden_smart_textbook.sql",
);
const sourcePath = process.argv.find((value) => value.endsWith(".md")) ?? defaultSource;
const checkOnly = process.argv.includes("--check");

function fail(message) {
  process.stderr.write(`chapter-thirteen conversion failed: ${message}\n`);
  process.exit(1);
}
...
const assertions = [
  [Number(frontmatter.chapter_number) === 13, "chapter_number must be 13"],
  [frontmatter.chapter_slug === "transportation", "chapter_slug must stay transportation"],
  [frontmatter.chapter_test_slug === "korean-level-one-13", "chapter test slug mismatch"],
  ...
  [vocabulary.length === 22, `expected 22 vocabulary rows, got ${vocabulary.length}`],
...
for (const key of ["normal.script", "slow.script", "pause_marks", "audio_object_key", "correct_option_id", "correct_option_ids", "correct_order", "answers="]) {
...
for (const id of ["chapter-13-listening-route-normal", "chapter-13-listening-route-slow"]) {
  if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
}
process.stdout.write(
  `PASS chapter-thirteen conversion: ${moduleCodes.length} modules, ${vocabulary.length} vocabulary items, ` +
