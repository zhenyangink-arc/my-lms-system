...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第08课 영화 볼까요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180014_chapter_eight_golden_smart_textbook.sql",
);
const sourcePath =
...
function fail(message) {
  process.stderr.write(`chapter-eight conversion failed: ${message}\n`);
  process.exit(1);
}
...
const assertions = [
  [Number(frontmatter.chapter_number) === 8, "chapter_number must be 8"],
  [frontmatter.chapter_slug === "movie-plan", "chapter_slug must stay movie-plan"],
  [frontmatter.chapter_test_slug === "korean-level-one-08", "chapter test slug mismatch"],
...
for (const id of [
  "chapter-08-listening-plan-place-normal",
  "chapter-08-listening-plan-place-slow",
]) {
  if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
}
process.stdout.write(
  `PASS chapter-eight conversion: ${moduleCodes.length} modules, ${vocabulary.length} vocabulary items, ` +
