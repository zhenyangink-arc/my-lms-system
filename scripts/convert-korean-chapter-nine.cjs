...
const defaultSource = path.resolve(
  projectRoot,
  "../uply-vault/UPLY BOOK/韩国语1级/第09课 이분은 누구세요.md",
);
const migrationPath = path.join(
  projectRoot,
  "supabase/migrations/202608180015_chapter_nine_golden_smart_textbook.sql",
);
const sourcePath = process.argv.find((value) => value.endsWith(".md")) ?? defaultSource;
const checkOnly = process.argv.includes("--check");

function fail(message) {
  process.stderr.write(`chapter-nine conversion failed: ${message}\n`);
  process.exit(1);
}
...
const assertions = [
  [Number(frontmatter.chapter_number) === 9, "chapter_number must be 9"],
  [frontmatter.chapter_slug === "family", "chapter_slug must stay family"],
  [frontmatter.chapter_test_slug === "korean-level-one-09", "chapter test slug mismatch"],
...
for (const id of ["chapter-09-listening-family-job-normal", "chapter-09-listening-family-job-slow"]) {
  if (!migration.includes(id)) fail(`migration is missing audio binding ${id}`);
}
process.stdout.write(
  `PASS chapter-nine conversion: ${moduleCodes.length} modules, ${vocabulary.length} vocabulary items, ` +
