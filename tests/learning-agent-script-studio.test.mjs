...
} from "../src/lib/smart-textbook-learning-targets.ts";

const root = new URL("../", import.meta.url);

test("老师讲解指向选择情景诊断后切换到第二页目标", () => {
  const targets = buildOrientationLearningTargets();
  const pageTarget = defaultLearningTargetForPage(targets, "diagnosis");
  assert.equal(pageTarget?.key, "orientation:page:diagnosis");
  assert.equal(pageTarget?.pageLabel, "第2页 · 情景诊断");
  assert.equal(pageTarget?.regionKey, "page");
  assert.equal(
...
test("平台负责人脚本工作台支持定位、编辑、排序和发布", async () => {
  ...
  const previewRespondRoute = await readFile(
    new URL("src/app/api/learning-agent/preview-respond/route.ts", root),
    "utf8",
  );
  const bufferPresets = await readFile(
    new URL("src/lib/learning-agent-buffer-presets.ts", root),
    "utf8",
  );
  ...
  assert.match(editor, /老师说什么/);
  assert.match(editor, /学生看到什么/);
  ...
  assert.match(editor, /说到本句时的界面/);
  assert.match(editor, /教学区 30% · 学习区 70%/);
  ...
  assert.match(editor, /setBufferLineKo\(preset\.text\["ko-KR"\]\)/);
  ...
  assert.match(actions, /bufferPresetId: String\(formData\.get\("buffer_preset_id"\) \?\? ""\)/);
  assert.match(actions, /configuration\.bufferLine = bufferPreset\.text/);
  assert.match(actions, /configuration\.bufferPresetId = bufferPreset\.id/);
  assert.match(editor, /过渡台词/);
  assert.match(editor, /name="buffer_preset_id"/);
  assert.doesNotMatch(editor, /过渡台词只能从词库选择/);
  ...
  assert.match(editor, /学生看到什么/);
  assert.match(editor, /播放时会按这个顺序执行/);
  assert.match(editor, /切换界面/);
  assert.match(editor, /展示老师/);
  assert.match(editor, /播放黑板/);
  assert.match(editor, /联动学习区/);
  assert.match(editor, /learningLayoutSummary/);
  assert.match(editor, /教学区布局与人物/);
  ...
  assert.doesNotMatch(characterStage, /当前按学生端沉浸式完整教学区预览/);
  assert.match(characterStage, /onPointerDown=\{handleBlackboardPointerDown\}/);
  assert.match(characterStage, /onKeyDown=\{handleBlackboardKeyDown\}/);
  ...
  assert.doesNotMatch(characterStage, /教学区 \$\{splitTeachingAreaWidthPercent\}%、学习区 \$\{splitLearningAreaWidthPercent\}%/);
  ...
  assert.match(characterStage, /handlePointerDown/);
  assert.match(characterStage, /handleDialoguePointerDown/);
  ...
  assert.match(editor, /操作后回答/);
  assert.match(editor, /participationSummary/);
  assert.match(editor, /答题反馈设置/);
  ...
  assert.doesNotMatch(characterStage, /黑板画面、人物动作和站位/);
  ...
  assert.doesNotMatch(editor, /hintLabel="查看老师讲解指向说明"/);
  ...
  assert.match(editor, /role="radiogroup" aria-label="老师讲解指向的页面或固定区域"/);
  assert.match(editor, /onClick=\{\(\) => selectLearningTargetPage\(page\.key\)\}/);
  assert.match(editor, /role="radio"/);
  assert.match(editor, /aria-checked=\{selected\}/);
  assert.match(editor, /selectLearningTargetRegion\(event\.currentTarget\.value\)/);
  assert.doesNotMatch(editor, /onInput=\{\(event\) => event\.stopPropagation\(\)\}/);
  ...
  assert.doesNotMatch(editor, /hintLabel="查看宠物操作说明"/);
  ...
  assert.match(editor, /当前教学流程/);
  assert.match(editor, /teachingFlowSummary/);
  assert.match(editor, /按顺序继续/);
  assert.match(editor, /跳到指定小节/);
  assert.match(editor, /继续按钮设置/);
  assert.match(editor, /按左侧顺序进入下一小节/);
  assert.doesNotMatch(editor, /跳转到指定小节/);
...
test("黑板内容可以直接在画面框里双击编辑、选中后一键删除", async () => {
  ...
  assert.doesNotMatch(blackboardEditor, /双击内容可直接改文字，选中后可直接删除/);
});
