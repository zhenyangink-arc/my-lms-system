...
test("韩语课程首页按完整分类展示课程并保持租户应用路由", () => {
  ...
  assert.match(directCatalogSource, /progressMap\.get\(lesson\.id\)/);
  assert.match(directCatalogSource, /Math\.round\(progressTotal \/ totalLessons\)/);
  assert.match(directCatalogSource, /<KoreanCourseCatalogBrowser sections=\{courseSections\}/);
  assert.match(catalogBrowserSource, /aria-labelledby=\{sectionTitleId\}/);
  assert.match(catalogBrowserSource, /aria-label="按学习状态筛选课程"/);
  assert.match(catalogBrowserSource, /role="progressbar"/);
  assert.match(catalogBrowserSource, /aria-valuenow=\{course\.progressPercent\}/);
  assert.match(catalogBrowserSource, /已完成 \{course\.completedLessons\} \/ \{course\.totalLessons\} 课时/);
