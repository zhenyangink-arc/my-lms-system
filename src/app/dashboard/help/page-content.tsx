...
export default async function HelpCenterPage() {
  ...
  return <div className="pb-12"><div className="mx-auto mt-5 w-full max-w-[1500px] space-y-5 px-4 sm:px-6 lg:px-8">{canManage && <div className="flex justify-end"><Link href="/dashboard/admin/help" className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-black text-white" style={{ backgroundColor: "var(--app-secondary)" }}>进入帮助后台<ArrowRight size={15} /></Link></div>}{canManage && <section className="app-card rounded-3xl border p-5 sm:p-6" style={{ background: "linear-gradient(125deg, var(--app-hero-end), var(--app-card-bg), var(--app-success-soft))" }}><div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_500px] xl:items-center"><div><span className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black" style={{ color: "var(--app-success)", backgroundColor: "var(--app-success-soft)" }}><Eye size={14} />学生端只读预览</span><DashboardTitleWithHint className="mt-3" title="检查学生能看到的帮助内容" description="帮助文章、求助进度和后台回复都来自同一套数据库。后台处理后，学生页面会同步显示最新状态与消息。" /></div><div className="dashboard-title-metrics">{[["帮助文章", articles.length, BookOpenText, "var(--app-accent)", "var(--app-accent-soft)"], ["处理中", openCount, Clock3, "var(--app-warm)", "var(--app-warm-soft)"], ["已解决", solvedCount, CheckCircle2, "var(--app-success)", "var(--app-success-soft)" ]].map(([label, value, Icon, color, soft]) => { const MetricIcon = Icon as typeof HelpCircle; return <div key={String(label)} className="app-card rounded-2xl border p-4 text-center"><span className="mx-auto flex h-9 w-9 items-center justify-center rounded-xl" style={{ color: String(color), backgroundColor: String(soft) }}><MetricIcon size={17} /></span><p className="mt-2 text-2xl font-black">{String(value)}</p><p className="app-muted-text text-xs font-black">{String(label)}</p></div>; })}</div></div></section>}
  ...
  {role === "student" && <div className="grid items-start gap-5 xl:grid-cols-[minmax(350px,0.78fr)_minmax(0,1.22fr)]"><HelpTicketForm /><section className="app-card rounded-3xl border p-4 sm:p-5"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-2xl" style={{ color: "var(--app-accent)", backgroundColor: "var(--app-accent-soft)" }}><MessageCircleQuestion size={19} /></span><h2 className="text-lg font-black">我的求助记录</h2></div><div className="mt-5 divide-y app-divider">{tickets.map((ticket) => { const solved = ticket.status === "resolved" || ticket.status === "closed"; return <Link key={ticket.id} href={`/dashboard/help/tickets/${ticket.id}`} className="app-flat-row group block rounded-xl p-3"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full px-2.5 py-1 text-xs font-black" style={{ color: "var(--app-secondary)", backgroundColor: "var(--app-secondary-soft)" }}>{HELP_TICKET_CATEGORY_LABELS[ticket.category]}</span><span className="rounded-full px-2.5 py-1 text-xs font-black" style={{ color: solved ? "var(--app-success)" : "var(--app-warm)", backgroundColor: solved ? "var(--app-success-soft)" : "var(--app-warm-soft)" }}>{HELP_TICKET_STATUS_LABELS[ticket.status]}</span>{ticket.priority === "urgent" && <span className="text-xs font-black" style={{ color: "#c94f45" }}>紧急</span>}<ArrowRight className="ml-auto transition group-hover:translate-x-1" size={15} /></div><h3 className="mt-3 text-sm font-black">{ticket.subject}</h3><p className="app-muted-text mt-2 text-xs">更新于 <LocalDateTime value={ticket.updated_at} options={HELP_DATE_TIME_OPTIONS} /></p></Link>; })}{tickets.length === 0 && <div className="app-empty-state rounded-2xl p-6 text-center"><ShieldCheck className="mx-auto opacity-30" size={28} /><p className="mt-3 text-sm font-black">还没有求助记录</p><p className="app-muted-text mt-1 text-xs">提交后，处理进度会显示在这里。</p></div>}</div></section></div>}
  </div></div>;
}
