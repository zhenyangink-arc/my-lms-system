    ...
    <div className="mx-auto w-full max-w-[1500px] space-y-5 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.75fr)]">
        <div className="space-y-5">
          <section className="app-card rounded-3xl border p-4 sm:p-5"><h2 className="flex items-center gap-2 text-lg font-black"><BookOpenText size={19} style={{ color: "var(--app-accent)" }} />情景任务</h2><p className="app-muted-text mt-4 whitespace-pre-wrap text-sm leading-6">{scenario.situation || "请根据标题设定完成一轮自然对话。"}</p>{objectives.length > 0 && <div className="mt-5 border-t pt-5" style={{ borderColor: "var(--app-border-soft)" }}><p className="text-xs font-black">本次目标</p><ul className="mt-3 space-y-2">{objectives.map((objective) => <li key={objective} className="flex items-start gap-2 text-sm leading-6"><span className="mt-1.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full" style={{ color: "white", backgroundColor: "var(--app-success)" }}><Check size={10} /></span>{objective}</li>)}</ul></div>}</section>

          <section className="app-card rounded-3xl border p-4 sm:p-5"><h2 className="flex items-center gap-2 text-lg font-black"><MessageCircleMore size={19} style={{ color: "var(--app-secondary)" }} />示范对话</h2><div className="mt-5 space-y-3">{dialogue.map((line, index) => <article key={`${line.speaker}-${index}`} className="app-soft-card rounded-2xl border p-4"><div className="flex items-start gap-3"><span className="rounded-lg px-2.5 py-1 text-xs font-black" style={{ color: "var(--app-secondary)", backgroundColor: "var(--app-secondary-soft)" }}>{line.speaker}</span><div className="min-w-0"><p className="text-sm font-black leading-7">{line.korean}</p><p className="app-muted-text mt-1 text-xs leading-5">{line.chinese}</p></div></div></article>)}{dialogue.length === 0 && <p className="app-muted-text rounded-2xl border border-dashed p-5 text-center text-xs">该场景暂未填写示范对话。</p>}</div></section>
        </div>

        <div className="space-y-5">
          <section className="app-card rounded-3xl border p-4 sm:p-5"><h2 className="flex items-center gap-2 text-base font-black"><Sparkles size={17} style={{ color: "var(--app-warm)" }} />重点表达</h2><div className="mt-4 space-y-3">{expressions.map((expression, index) => <div key={`${expression.korean}-${index}`} className="app-soft-card rounded-xl border p-3"><p className="text-sm font-black leading-6">{expression.korean}</p><p className="app-muted-text mt-1 text-xs leading-5">{expression.chinese}</p></div>)}{expressions.length === 0 && <p className="app-muted-text text-xs">该场景暂未填写重点表达。</p>}</div></section>
          {(scenario.starter_prompt || scenario.practice_tips) && <section className="app-card rounded-3xl border p-4 sm:p-5"><h2 className="flex items-center gap-2 text-base font-black"><Lightbulb size={17} style={{ color: "var(--app-success)" }} />开始开口</h2>{scenario.starter_prompt && <div className="mt-4"><p className="text-xs font-black">开场任务</p><p className="app-muted-text mt-2 whitespace-pre-wrap text-xs leading-5">{scenario.starter_prompt}</p></div>}{scenario.practice_tips && <div className="mt-4 border-t pt-4" style={{ borderColor: "var(--app-border-soft)" }}><p className="text-xs font-black">练习提示</p><p className="app-muted-text mt-2 whitespace-pre-wrap text-xs leading-5">{scenario.practice_tips}</p></div>}</section>}
