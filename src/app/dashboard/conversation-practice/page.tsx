...
import { ArrowRight, CheckCircle2, Clock3, Eye, MessageCircleMore, MessagesSquare, Sparkles } from "lucide-react";
...
export default async function ConversationPracticePage() {
  ...
  return (
    <div className="pb-12">
      <DashboardPageHeader
        title="会话练习"
        description={canManage ? "以学生视角检查已经发布的情景会话。" : "跟着情景开口练习，积累能真正用出来的韩语表达。"}
        action={canManage ? <Link href="/dashboard/admin/conversation-practice" className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-black text-white" style={{ backgroundColor: "var(--app-secondary)" }}>进入后台管理<ArrowRight size={15} /></Link> : undefined}
      />
      <div className="mx-auto mt-6 w-full max-w-[1450px] space-y-6 px-4 sm:px-6 lg:px-8">
        <section className="app-card overflow-hidden rounded-[30px] border p-6 sm:p-8" style={{ background: "linear-gradient(125deg, var(--app-hero-end), var(--app-card-bg), var(--app-accent-soft))" }}>
          <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_520px] xl:items-end">
            <div><span className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black" style={{ color: "var(--app-accent)", backgroundColor: "var(--app-accent-soft)" }}>{canManage ? <Eye size={14} /> : <MessageCircleMore size={14} />}{canManage ? "学生端只读预览" : "情景开口训练"}</span><h2 className="mt-5 text-3xl font-black tracking-tight sm:text-4xl">{canManage ? "检查学生实际看到的会话内容" : "从一句开场白，练到完整对话"}</h2><p className="app-muted-text mt-4 max-w-2xl text-sm leading-7">{canManage ? "这里仅展示已发布场景，不提供编辑入口。需要修改内容、发布场景或查看练习数据时，请进入后台管理。" : "先读情景和示范对话，再替换重点表达完成自己的版本。每次练习都可以记录自信程度与复盘。"}</p></div>
          ...
          {scenarios.map((scenario) => {
            ...
            return (
              <Link key={scenario.id} href={`/dashboard/conversation-practice/${scenario.id}`} className="app-card group flex h-full flex-col rounded-[1.75rem] border p-5 transition hover:-translate-y-0.5 hover:shadow-lg">
                <div className="flex flex-wrap items-center gap-2"><span className="rounded-full px-2.5 py-1 text-[10px] font-black" style={{ color: "var(--app-secondary)", backgroundColor: "var(--app-secondary-soft)" }}>{CONVERSATION_CATEGORY_LABELS[scenario.category]}</span><span className="rounded-full px-2.5 py-1 text-[10px] font-black" style={{ color: tone.color, backgroundColor: tone.soft }}>{CONVERSATION_DIFFICULTY_LABELS[scenario.difficulty]}</span>{scenario.is_featured && <span className="rounded-full px-2.5 py-1 text-[10px] font-black" style={{ color: "var(--app-warm)", backgroundColor: "var(--app-warm-soft)" }}>推荐</span>}<ArrowRight className="ml-auto transition group-hover:translate-x-1" size={16} /></div>
                <h3 className="mt-4 text-lg font-black leading-7">{scenario.title}</h3><p className="app-muted-text mt-2 line-clamp-3 text-xs leading-6">{scenario.description || "打开场景查看完整情景、示范对话和重点表达。"}</p>
                <div className="mt-auto flex items-end justify-between gap-3 border-t pt-4" style={{ borderColor: "var(--app-border-soft)" }}><span className="app-muted-text inline-flex items-center gap-1 text-[11px] font-bold"><Clock3 size={12} />建议 {scenario.duration_minutes} 分钟</span>{itemProgress && <span className="text-[11px] font-black" style={{ color: itemProgress.status === "completed" ? "var(--app-success)" : "var(--app-accent)" }}>{itemProgress.status === "completed" ? "已掌握" : `已练习 ${itemProgress.practice_count} 次`}</span>}</div>
              </Link>
            );
          })}
          {!scenariosResult.error && scenarios.length === 0 && <div className="app-card col-span-full rounded-[1.75rem] border border-dashed p-12 text-center"><MessagesSquare className="mx-auto opacity-30" size={36} /><p className="mt-4 font-black">当前还没有开放的会话场景</p><p className="app-muted-text mt-2 text-sm">后台发布第一个场景后，会自动显示在这里。</p></div>}
        </section>
      </div>
    </div>
  );
