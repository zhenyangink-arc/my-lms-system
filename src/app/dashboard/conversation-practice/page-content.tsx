      ...
      <div className="mx-auto mt-5 w-full max-w-[1500px] space-y-5 px-4 sm:px-6 lg:px-8">
        ...
        <section className="app-card overflow-hidden rounded-3xl border p-5 sm:p-6" style={{ background: "linear-gradient(125deg, var(--app-hero-end), var(--app-card-bg), var(--app-accent-soft))" }}>
          <div className={canManage ? "grid gap-5 xl:grid-cols-[minmax(0,1fr)_520px] xl:items-center" : "grid"}>
            {canManage && <div><span className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black" style={{ color: "var(--app-accent)", backgroundColor: "var(--app-accent-soft)" }}><Eye size={14} />学生端只读预览</span><DashboardTitleWithHint className="mt-3" headingLevel={2} title="检查学生实际看到的会话内容" description="这里仅展示已发布场景，不提供编辑入口。需要修改内容、发布场景或查看练习数据时，请进入后台管理。" /></div>}
