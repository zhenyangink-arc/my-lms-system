...
export default async function AssignmentsPage() {
  ...
  return (
    <div className="pb-12">
      <DashboardPageHeader title="作业与考试" description={isManager ? "以学生视角预览已经发布的作业、测验与考试。" : "按截止时间完成任务，查看老师的逐题评分与反馈。"} action={isManager ? <Link href="/dashboard/admin/assignments" className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-black text-white" style={{ backgroundColor: "var(--app-secondary)" }}>进入后台管理<ArrowRight size={15} /></Link> : undefined} />
      <div className="mx-auto mt-6 w-full max-w-[1450px] space-y-6 px-4 sm:px-6 lg:px-8">
        <section className="app-card overflow-hidden rounded-[30px] border p-6 sm:p-8" style={{ background: "linear-gradient(125deg, var(--app-hero-end), var(--app-card-bg), var(--app-secondary-soft))" }}>
          <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_520px] xl:items-end"><div><span className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black" style={{ color: "var(--app-accent)", backgroundColor: "var(--app-accent-soft)" }}><ClipboardList size={15} />{isManager ? "学生端预览" : "我的学习任务"}</span><h2 className="mt-5 text-3xl font-black tracking-tight sm:text-4xl">{isManager ? "检查学生实际看到的任务页面" : "每次提交都有记录，每份反馈都有下一步"}</h2><p className="app-muted-text mt-4 max-w-2xl text-sm leading-7">{isManager ? "这里只展示已经发布的任务，页面不会进入布置或批改流程。需要管理时请使用右上角的后台入口。" : "作业、章节测验和考试统一按截止时间排列。老师退回后可以按意见重新提交，历史版本不会丢失。"}</p></div><div className="grid grid-cols-3 gap-3">{overviewMetrics.map(([label, value, Icon, color, soft]) => { const MetricIcon = Icon as typeof Award; return <div key={String(label)} className="app-card rounded-2xl border p-4 text-center"><span className="mx-auto flex h-9 w-9 items-center justify-center rounded-xl" style={{ color: String(color), backgroundColor: String(soft) }}><MetricIcon size={17} /></span><p className="mt-2 text-2xl font-black">{String(value)}</p><p className="app-muted-text text-[10px] font-black">{String(label)}</p></div>; })}</div></div>
        </section>
