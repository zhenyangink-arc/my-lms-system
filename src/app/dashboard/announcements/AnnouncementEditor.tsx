...
export function AnnouncementEditor({
  ...
  return (
    <details className="app-soft-card rounded-2xl border p-4">
      ...
      <form action={formAction} className="mt-4 space-y-3 border-t pt-4" style={{ borderColor: "var(--border-subtle)" }}>
        ...
        {state.message && <p className="text-xs font-bold" style={{ color: state.status === "error" ? "#c94f45" : "var(--status-success)" }}>{state.message}</p>}
        <button type="submit" disabled={pending} className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-bold text-white disabled:opacity-50" style={{ backgroundColor: "var(--support)" }}>
