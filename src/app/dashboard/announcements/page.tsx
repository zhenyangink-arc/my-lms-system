...
export default async function AnnouncementsPage() {
  ...
  return (
    <div className="pb-12">
      <DashboardPageHeader
        title="通知公告"
        description="查看课程、考试和平台发布的重要消息。"
        action={access.canAccess ? <Link href="/dashboard/admin/announcements" className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-black text-white" style={{ backgroundColor: "var(--app-secondary)" }}>进入公告后台<ArrowRight size={15} /></Link> : undefined}
      />

      <div className="mx-auto mt-6 w-full max-w-[1250px] space-y-6 px-4 sm:px-6 lg:px-8">
        <section className="app-card overflow-hidden rounded-[30px] border p-6 sm:p-8" style={{ background: "linear-gradient(125deg, var(--app-hero-end), var(--app-card-bg), var(--app-accent-soft))" }}>
          <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_380px] lg:items-end">
            <div><span className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black" style={{ color: "var(--app-accent)", backgroundColor: "var(--app-accent-soft)" }}><BellRing size={14} />消息公告栏</span><h2 className="mt-5 text-3xl font-black tracking-tight sm:text-4xl">重要消息，一处查看</h2><p className="app-muted-text mt-4 max-w-2xl text-sm leading-7">这里只展示已经正式发布的公告。置顶、重要和紧急通知会优先排列，草稿与后台编辑功能不会出现在学生页面。</p></div>
        ...
        <section className="space-y-4">
          {announcements.map((announcement) => {
            ...
            return <article key={announcement.id} className="app-card rounded-[26px] border p-5 sm:p-7"><div className="flex flex-wrap items-center gap-2">{announcement.is_pinned && <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-black" style={{ color: "var(--app-accent)", backgroundColor: "var(--app-accent-soft)" }}><Pin size={11} />置顶</span>}<span className="rounded-full px-2.5 py-1 text-[10px] font-black" style={{ color: "var(--app-secondary)", backgroundColor: "var(--app-secondary-soft)" }}>{CATEGORY_LABELS[announcement.category]}</span><span className="rounded-full px-2.5 py-1 text-[10px] font-black" style={{ color: tone.color, backgroundColor: tone.soft }}>{PRIORITY_LABELS[announcement.priority]}</span></div><h2 className="mt-4 text-xl font-black leading-8">{announcement.title}</h2><p className="app-muted-text mt-4 whitespace-pre-wrap text-sm leading-8">{announcement.content}</p><div className="app-muted-text mt-5 flex items-center gap-2 border-t pt-4 text-[11px] font-bold" style={{ borderColor: "var(--app-border-soft)" }}><CalendarDays size={13} />{announcement.published_at ? `发布于 ${dateFormatter.format(new Date(announcement.published_at))}` : "平台公告"}</div></article>;
          })}
          {!error && announcements.length === 0 && <div className="app-card rounded-[26px] border border-dashed p-12 text-center"><BellRing className="mx-auto opacity-30" size={34} /><h2 className="mt-4 font-black">当前没有新公告</h2><p className="app-muted-text mt-2 text-sm">后台发布通知后，会自动显示在这里。</p></div>}
        </section>
      </div>
    </div>
  );
