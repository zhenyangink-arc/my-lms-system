...
function FilterSection({
  ...
  return (
    <section className="border-b pb-5 last:border-b-0 last:pb-0" style={{ borderColor: "var(--border-subtle)" }}>
      {/* 每组筛选都有独立收缩按钮，长列表中也能快速定位。 */}
      <button type="button" aria-expanded={expanded} onClick={() => setExpanded((current) => !current)} className={`flex w-full items-center justify-between gap-3 rounded-lg text-left text-xs font-bold ${focusRing} ${expanded ? "mb-3" : ""}`}>
        <span>{title}</span>
...
function FilterButton({
  ...
  return (
    <button
      ...
      aria-pressed={active}
      onClick={onClick}
      className={`inline-flex items-center justify-between gap-2 rounded-xl border px-3 py-2 text-left text-xs font-bold transition hover:-translate-y-0.5 ${focusRing}`}
      style={
        active
          ? {
...
function RankPanel({
  ...
  return (
    <div className="rounded-2xl border px-3 py-2.5" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-soft)" }}>
      <p className="text-[10px] font-bold app-muted-text">{year ? `${year} ${label}` : label}</p>
      <p className="mt-1 text-sm font-bold">{rank ? `第 ${rank}` : "暂无"}</p>
    </div>
  );
}
...
function UniversityAssessmentDialog({ university }: { university: KoreanUniversity }) {
  ...
  return (
    <Dialog>
      <DialogTrigger
        type="button"
        className={`inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl px-2 py-2.5 text-xs font-bold text-white transition hover:opacity-90 ${focusRing}`}
        style={{ backgroundColor: "var(--support)" }}
      >
        <Sparkles size={13} aria-hidden="true" /> 在线评估
      </DialogTrigger>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl [&_.app-input:focus-visible]:outline-none [&_.app-input:focus-visible]:ring-2 [&_.app-input:focus-visible]:ring-[var(--ring)] [&_.app-input:focus-visible]:ring-offset-2">
        <DialogHeader>
          <DialogTitle className="text-lg font-bold">{university.name_zh} · 在线评估</DialogTitle>
          <DialogDescription>
        ...
        <form action={formAction} className="space-y-4">
          ...
          {state.message && (
            <div
              ...
              style={{
                color: state.status === "success" ? resultColor : "var(--destructive)",
                borderColor: state.status === "success" ? resultColor : "var(--destructive)",
              ...
              {state.status === "success" && state.breakdown ? (
                <>
                  <div className="flex items-end justify-between gap-4">
                    <div>
                      <p className="text-xs font-bold">初步结果</p>
                      <p className="mt-1 text-lg font-bold">{state.resultLabel}</p>
                    </div>
                    <p className="text-2xl font-bold">{state.score}<span className="text-xs"> 分</span></p>
                  </div>
          ...
          <button disabled={pending} type="submit" className={`inline-flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-60 ${focusRing}`} style={{ backgroundColor: "var(--primary)" }}>
...
function UniversityDetailsDialog({ university }: { university: KoreanUniversity }) {
  return (
    <Dialog>
      <DialogTrigger
        type="button"
        className={`inline-flex w-full items-center justify-center gap-1.5 rounded-xl border px-3 py-2.5 text-xs font-bold transition hover:-translate-y-0.5 ${focusRing}`}
        style={{ color: "var(--primary-hover)", borderColor: "var(--primary)", backgroundColor: "var(--accent)" }}
      >
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <div className="flex items-center gap-3 pr-8">
            ...
            <div>
              <DialogTitle className="text-xl font-bold">{university.name_zh}</DialogTitle>
              <DialogDescription className="mt-1">{university.name_ko} · {university.city}</DialogDescription>
            </div>
          </div>
        </DialogHeader>

        <section className="rounded-2xl border p-4" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-soft)" }}>
          <h3 className="text-sm font-bold">学校介绍</h3>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border px-3 py-2.5" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-soft)" }}>
            <p className="text-[10px] font-bold app-muted-text">{university.tuition_reference_year} 年度学费参考</p>
            <p className="mt-1 text-sm font-bold">{formatWan(university.tuition_min_cny)}万—{formatWan(university.tuition_max_cny)}万元</p>
          </div>
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <section className="app-soft-card rounded-2xl border p-4">
            <h3 className="flex items-center gap-2 text-sm font-bold"><BookOpen size={16} aria-hidden="true" /> 申请阶段</h3>
          ...
          <section className="app-soft-card rounded-2xl border p-4">
            <h3 className="flex items-center gap-2 text-sm font-bold"><GraduationCap size={16} aria-hidden="true" /> 优势学科</h3>
            <div className="mt-3 flex flex-wrap gap-2">
        ...
        <section>
          <h3 className="text-sm font-bold">院校亮点</h3>
          <div className="mt-3 grid gap-2 sm:grid-cols-3">
            {university.highlights.map((highlight) => (
              <div key={highlight} className="flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold" style={{ borderColor: "var(--border-subtle)" }}>
                <Check size={13} style={{ color: "var(--status-success)" }} aria-hidden="true" /> {highlight}
              </div>
            ))}
          </div>
        </section>

        <div className="rounded-2xl p-4 text-xs leading-5" style={{ color: "var(--status-warning)", backgroundColor: "var(--status-warning-surface)" }}>
          学费按专业、学期和奖学金情况变化，人民币金额也受汇率影响；排名为标注年份的数据。申请前请由顾问在管理中心复核当年招生简章与缴费通知。
        </div>
        <div className="flex flex-wrap gap-2">
          {university.ranking_source_url && (
            <a href={university.ranking_source_url} target="_blank" rel="noreferrer" className={`app-soft-card inline-flex items-center gap-1.5 rounded-xl border px-4 py-2.5 text-xs font-bold ${focusRing}`}>
              查看排名来源 <ExternalLink size={13} aria-hidden="true" />
            </a>
          )}
        </div>
      </DialogContent>
...
function UniversityCard({
  ...
  return (
    <article className="app-card group flex min-h-[530px] flex-col overflow-hidden rounded-3xl border transition duration-300 hover:-translate-y-1 hover:shadow-xl">
      <div className="relative h-2" style={{ background: "linear-gradient(90deg, var(--primary), var(--support), var(--status-success))" }} />
      <div className="flex flex-1 flex-col p-4">
        <div className="flex items-start justify-between gap-3">
          <SchoolCrest logoUrl={university.logo_url} name={university.name_zh} />
          <div className="flex flex-wrap justify-end gap-1.5">
            {university.is_featured && <span className="rounded-full px-2 py-1 text-[10px] font-bold" style={{ color: "var(--primary-hover)", backgroundColor: "var(--accent)" }}>重点院校</span>}
            <span className="rounded-full px-2 py-1 text-[10px] font-bold" style={{ color: "var(--support)", backgroundColor: "var(--support-surface)" }}>{ownershipLabelMap[university.ownership]}</span>
          </div>
        </div>

        <h3 className="mt-4 text-base font-bold tracking-tight">{university.name_zh}</h3>
        <p className="mt-1 truncate text-xs font-bold app-muted-text">{university.name_ko}</p>
        <p className="mt-3 flex items-center gap-1.5 text-xs font-bold app-muted-text"><MapPin size={13} style={{ color: "var(--primary)" }} aria-hidden="true" /> {university.province} · {university.city}</p>
        ...
        <div className="mt-3 rounded-2xl border p-3" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--status-warning-surface)" }}>
          <div className="flex items-center justify-between gap-2">
            <p className="text-[10px] font-bold app-muted-text">{university.tuition_reference_year} 每年学费参考</p>
            <CircleDollarSign size={13} style={{ color: "var(--status-warning)" }} aria-hidden="true" />
          </div>
          <p className="mt-1 text-sm font-bold">{formatWan(university.tuition_min_cny)}万—{formatWan(university.tuition_max_cny)}万元</p>
        ...
        <div className="mt-3 flex min-h-12 flex-wrap content-start gap-1.5">
          {university.discipline_groups.slice(0, 3).map((group) => (
            <span key={group} className="rounded-full border px-2 py-1 text-[10px] font-bold app-muted-text" style={{ borderColor: "var(--border-subtle)" }}>{disciplineLabelMap[group] ?? group}</span>
          ))}
          {university.discipline_groups.length > 3 && <span className="rounded-full border px-2 py-1 text-[10px] font-bold app-muted-text" style={{ borderColor: "var(--border-subtle)" }}>+{university.discipline_groups.length - 3}</span>}
        </div>
        ...
        <div className="mt-auto space-y-2 pt-4">
          <form action={addLibraryUniversityTargetAction.bind(null, university.id)} data-permission="university_target" className="flex gap-2">
            <select name="admissionTrack" disabled={isTarget} defaultValue={university.admission_stages[0]} aria-label={`${university.name_zh}的申请阶段`} className={`app-input min-w-0 flex-1 rounded-xl border px-2 py-2.5 text-xs font-bold disabled:opacity-60 ${focusRing}`}>
              {university.admission_stages.map((stage) => <option key={stage} value={stage}>{stageLabelMap[stage] ?? stage}</option>)}
            </select>
            <button disabled={isTarget} type="submit" className={`inline-flex items-center gap-1 rounded-xl px-3 py-2.5 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-60 ${focusRing}`} style={{ backgroundColor: isTarget ? "var(--status-success)" : "var(--primary)" }}>
              {isTarget ? <Check size={12} aria-hidden="true" /> : <Target size={12} aria-hidden="true" />}{isTarget ? "已是目标" : "加入目标"}
            </button>
          </form>
          <div className="flex gap-2">
            <form action={toggleUniversityComparisonAction.bind(null, university.id)} data-permission="university_comparison" className="flex-1">
              <button type="submit" className={`app-soft-card inline-flex w-full items-center justify-center gap-1.5 rounded-xl border px-2 py-2.5 text-xs font-bold transition hover:opacity-80 ${focusRing}`} style={isCompared ? { color: "var(--primary-hover)", borderColor: "var(--primary)" } : undefined}>
                {isCompared ? <X size={13} aria-hidden="true" /> : <Scale size={13} aria-hidden="true" />}{isCompared ? "移出对比" : "加入对比"}
              </button>
            </form>
            <UniversityAssessmentDialog university={university} />
          </div>
          <UniversityDetailsDialog university={university} />
          <Link href={`/dashboard/universities/library/${university.id}`} className={`inline-flex w-full items-center justify-center gap-1.5 rounded-xl px-3 py-2.5 text-xs font-bold text-white ${focusRing}`} style={{ backgroundColor: "var(--support)" }}>进入学校页面 <ArrowUpRight size={13} aria-hidden="true" /></Link>
        </div>
      </div>
    </article>
  );
    ...
    <section className="grid items-start gap-5 lg:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="app-card rounded-3xl border p-4 lg:sticky lg:top-24 lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2"><Filter size={17} style={{ color: "var(--primary)" }} aria-hidden="true" /><h2 className="text-sm font-bold">选校导航</h2></div>
          {activeFilterCount > 0 && <span className="rounded-full px-2 py-1 text-xs font-bold" style={{ color: "var(--primary-hover)", backgroundColor: "var(--accent)" }}>{activeFilterCount} 项</span>}
        </div>
        <button type="button" onClick={resetFilters} className={`mt-4 inline-flex w-full items-center justify-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-bold app-muted-text ${focusRing}`} style={{ borderColor: "var(--border-subtle)" }}><RotateCcw size={12} aria-hidden="true" /> 重置全部筛选</button>
      ...
      <div className="min-w-0 space-y-5">
        <div className="app-card rounded-3xl border p-4 sm:p-5">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              <div className="flex items-center gap-2"><Landmark size={18} style={{ color: "var(--support)" }} aria-hidden="true" /><h2 className="text-lg font-bold">韩国大学学校库</h2></div>
              <p className="mt-1 text-xs app-muted-text" aria-live="polite" aria-atomic="true">找到 {filteredUniversities.length} 所符合条件的大学，当前显示 {visibleUniversities.length} 所，共收录 {universities.length} 所。</p>
            </div>
            <label className="block xl:w-80">
              <span className="mb-2 block text-xs font-bold">搜索学校</span>
              <span className="relative block">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 app-muted-text" size={16} aria-hidden="true" />
                <input value={search} onChange={(event) => { setSearch(event.target.value); setVisibleCount(24); }} placeholder="大学、韩文名或地区" className={`app-input w-full rounded-2xl border py-3 pl-10 pr-4 text-sm ${focusRing}`} />
              </span>
            </label>
          </div>
        </div>
        ...
        <section className="app-card rounded-3xl border p-4 sm:p-5">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex items-center gap-3">
              <span className="flex h-10 w-10 items-center justify-center rounded-2xl" style={{ color: "var(--primary)", backgroundColor: "var(--accent)" }}><Scale size={18} aria-hidden="true" /></span>
              <div><h3 className="text-sm font-bold">四校对比</h3><p className="mt-0.5 text-xs app-muted-text">已选择 {comparedUniversities.length}／4 所，点击已选院校可直接移出。</p></div>
            </div>
              ...
              {[0, 1, 2, 3].map((index) => {
                ...
                return university ? (
                  <form key={university.id} action={toggleUniversityComparisonAction.bind(null, university.id)} data-permission="university_comparison">
                    <button type="submit" aria-label={`从对比中移除${university.name_zh}`} className={`group/compare flex min-h-20 w-full min-w-0 items-center gap-2 rounded-xl border px-3 py-2 text-left transition hover:-translate-y-0.5 ${focusRing}`} style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-soft)" }}>
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-xs font-bold text-white" style={{ backgroundColor: "var(--support)" }}>{university.name_zh.slice(0, 1)}</span>
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-xs font-bold">{university.name_zh}</span>
                  ...
                  <div key={`empty-${index}`} className="flex min-h-20 items-center justify-center rounded-xl border border-dashed px-3 py-2 text-xs font-bold app-muted-text" style={{ borderColor: "var(--border)" }}>待选择</div>
                );
              })}
            </div>
          </div>
        ...
        {visibleCount < filteredUniversities.length && (
          <button type="button" onClick={() => setVisibleCount((current) => current + 24)} className={`app-card inline-flex w-full items-center justify-center rounded-2xl border px-4 py-3 text-xs font-bold transition hover:-translate-y-0.5 ${focusRing}`} style={{ color: "var(--primary-hover)" }}>
            再显示 24 所大学
          </button>
        )}

        <div className="flex items-start gap-2 rounded-2xl border p-4 text-xs leading-5 app-muted-text" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--card)" }}>
          <BookOpen className="mt-0.5 shrink-0" size={14} aria-hidden="true" />
