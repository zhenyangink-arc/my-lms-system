...
export default async function VisaPage() {
  ...
  const [caseResult, tasksResult, eventsResult] = await Promise.all([
    ...
    <>
      ...
      <div className="mx-auto w-full max-w-[1500px] space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <section className="grid items-start gap-4 xl:grid-cols-[minmax(320px,1fr)_minmax(320px,1.1fr)]">
                ...
                {notifications.map((event) => (
                  <div key={event.id} className="flex items-start gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="app-muted-text mt-0.5 text-[11px]">{formatEventTime(event.created_at)}</p>
                    </div>
                  </div>
                ))}
          ...
          {visaCase ? (
            <div className="app-card rounded-2xl border p-5"><div className="mb-4 flex flex-wrap items-start justify-between gap-2"><div><p className="app-muted-text text-xs font-black">签证档案</p><h2 className="mt-0.5 text-lg font-black">基础办理信息</h2></div>{visaCase.target_entry_date && <span className="inline-flex items-center gap-1.5 text-xs font-bold app-muted-text"><CalendarDays size={13} />{visaCase.target_entry_date}</span>}</div>{canEdit ? <VisaCaseForm visaType={visaCase.visa_type} targetEntryDate={visaCase.target_entry_date} applicationCity={visaCase.application_city} /> : <p className="rounded-xl px-3 py-2.5 text-sm font-bold app-muted-text" style={{ backgroundColor: "var(--app-soft-bg)" }}>当前账号可以浏览签证路线，但没有修改权限。</p>}{visaCase.advisor_note && <div className="mt-3 rounded-xl px-3 py-2.5 text-sm leading-6" style={{ color: "var(--app-secondary)", backgroundColor: "var(--app-secondary-soft)" }}><b>顾问提醒：</b>{visaCase.advisor_note}</div>}</div>
          ) : (
            <div className="app-card flex items-center justify-center rounded-2xl border border-dashed p-5 text-center text-xs font-bold app-muted-text">建立签证准备路线后，可以在这里查看和修改基础办理信息。</div>
          )}
        </section>
