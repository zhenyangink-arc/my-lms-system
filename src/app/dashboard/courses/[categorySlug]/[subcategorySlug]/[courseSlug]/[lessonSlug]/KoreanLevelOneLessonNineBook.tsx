      ...
      {lines.map((line, index) => (
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className={`text-[10px] font-bold leading-5 text-[#71857b] ${showChinese ? "opacity-100" : "opacity-0"}`}>{line.chinese}</p>
          </div>
...
const dividers: Record<string, { step: string; title: string; goal: string; icon: ReactNode }> = {
  "02": { step: "STEP 01", title: "课前导航", goal: "从家庭照片出发，完成身份、关系、年龄、能力和长辈日常的得体介绍。", icon: <UsersRound size={24} /> },
  "04": { step: "STEP 02", title: "核心词汇", goal: "建立家庭成员、年龄数字、敬语名词和敬语动词四组词汇网络。", icon: <House size={24} /> },
  "09": { step: "STEP 03", title: "语法讲解", goal: "四个语法各占一页，讲清所属、能力、名词敬语和主体敬语。", icon: <NotebookPen size={24} /> },
  "14": { step: "STEP 04", title: "句型操练", goal: "把普通表达升级为敬语，并判断年龄、姓名、家和日常动作该换哪些词。", icon: <Crown size={24} /> },
  "18": { step: "STEP 05", title: "实战对话", goal: "通过三组八句对话介绍家人、询问长辈并完成电话问候。", icon: <HeartHandshake size={24} /> },
  "22": { step: "STEP 06", title: "听说任务", goal: "读懂家庭树和人物档案，完成一段有关系、有年龄、有能力的介绍。", icon: <ContactRound size={24} /> },
  "26": { step: "STEP 07", title: "读写拓展", goal: "阅读家庭介绍，写出普通信息与主体敬语搭配正确的原创短文。", icon: <BookOpenCheck size={24} /> },
  "29": { step: "STEP 08", title: "自测与复盘", goal: "综合检查家庭词汇、年龄、敬语词汇和四项核心语法。", icon: <CheckCircle2 size={24} /> },
};
...
export function KoreanLevelOneLessonNineBook({
  ...
  function renderPage(number: string) {
    ...
    const pages: Record<string, ReactNode> = {
      "03": content("03", "介绍家人时要切换两套语言", "谈自己的信息用普通表达；谈值得尊敬的主体时同步升级名词、谓语和提问方式。", <UsersRound size={22} />, <>
      ...
      "05": content("05", "1. 家庭成员", "형/오빠、누나/언니会随说话者性别变化，不能只按中文机械翻译。", <UsersRound size={22} />, <><WordGrid words={familyWords} speak={speak} showChinese={Boolean(revealed.chinese05)} /><Note title="称呼视角">男性称哥哥为 형、姐姐为 누나；女性称哥哥为 오빠、姐姐为 언니。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese05)} onClick={() => toggle("chinese05")} />),
      "06": content("06", "2. 年龄专用数字", "年龄使用固有词数字加 살；20在量词前变为 스무。", <ContactRound size={22} />, <><WordGrid words={ageWords} speak={speak} showChinese={Boolean(revealed.chinese06)} /><Note title="20的关键变化" tone="rose">20岁：스무 살；21岁：스물한 살。只有整20在 살 前用 스무，带个位时恢复 스물-。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese06)} onClick={() => toggle("chinese06")} />),
      "07": content("07", "3. 敬语名词", "对长辈提问时不能只给普通名词加礼貌句尾，名词本身也要升级。", <Crown size={22} />, <><WordGrid words={honorificNouns} speak={speak} showChinese={Boolean(revealed.chinese07)} /><Note title="提问升级" tone="amber">이름 → 성함，나이 → 연세，집 → 댁，생일 → 생신，사람 → 분。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese07)} onClick={() => toggle("chinese07")} />),
      "08": content("08", "4. 敬语动词与能力表达", "有特殊敬语动词时优先使用特殊词；没有时再添加 -(으)시-。", <Languages size={22} />, <><WordGrid words={honorificVerbs} speak={speak} showChinese={Boolean(revealed.chinese08)} /><Note title="能力三级">잘하다：擅长；잘 못하다：不太擅长；못하다：不会或因条件不能做。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese08)} onClick={() => toggle("chinese08")} />),
      "10": content("10", "1. N(의) N", "连接两个名词表示所属、关系或类别；口语中的助词 의 常读作 [에]。", <House size={22} />, <>
        <div className="mt-5 grid grid-cols-2 gap-3">{[
          ["基本结构", "선생님의 책", "老师的书"],
          ["家庭关系", "민수의 아버지", "民洙的父亲"],
          ["저 + 의 → 제", "제 어머니", "我的母亲（谦辞）"],
      ...
      "11": content("11", "2. N을/를 잘하다", "用宾格助词标记能力领域，再区分擅长、不太擅长和不会。", <CheckCircle2 size={22} />, <>
        <div className="mt-4 space-y-3">{[
          ["잘하다", "아버지는 요리를 잘하세요.", "父亲很擅长做饭。"],
          ["잘 못하다", "저는 수영을 잘 못해요.", "我不太会游泳。"],
          ["못하다", "동생은 운전을 못해요.", "弟弟不会开车。"],
      ].map(([level, korean, chinese]) => <button key={level} type="button" onClick={() => speak(korean)} className="block w-full rounded-2xl border border-[#ddd0ee] bg-white p-4 text-left"><b className="text-[11px] text-[#75559a]">{level}</b><div className="mt-2 flex items-center justify-between gap-2"><p className="text-base font-black">{korean}</p><Volume2 size={14} className="shrink-0 text-[#75559a]"/></div><p className={`mt-1 text-xs text-[#71857b] ${revealed.chinese11 ? "opacity-100" : "opacity-0"}`}>{chinese}</p></button>)}</div>
      ...
      "12": content("12", "3. N(이)세요", "介绍长辈或尊敬对象的身份，相当于 이에요/예요 的敬语版本。", <Crown size={22} />, <>
        <div className="mt-5 grid grid-cols-2 gap-3">{[
          ...
          ["数量敬语", "두 분이세요.", "是两位。"],
      ].map(([rule, korean, chinese]) => <article key={rule} className="rounded-2xl border border-[#ddd0ee] bg-white p-4"><b className="text-[#75559a]">{rule}</b><div className="mt-3 text-sm font-black"><RuleSentence text={korean.replace(" → ", ". ")} speak={speak}>{korean}</RuleSentence></div><p className={`mt-1 text-xs text-[#71857b] ${revealed.chinese12 ? "opacity-100" : "opacity-0"}`}>{chinese}</p></article>)}</div>
      ...
      "13": content("13", "4. A/V-(으)시-", "把值得尊敬的主体放在主语位置时，在谓语词干后加入主体敬语词尾。", <Crown size={22} />, <>
        <div className="mt-4 grid grid-cols-2 gap-3">{[
      ...
      "15": content("15", "1. 年龄转换实验", "普通年龄用 살；询问长辈时换成 연세，并使用主体敬语句尾。", <ContactRound size={22} />, <>
        <div className="mt-4 grid grid-cols-2 gap-2.5">{[
          ["20", "스무 살"], ["21", "스물한 살"], ["34", "서른네 살"], ["45", "마흔다섯 살"],
          ["50", "쉰 살"], ["63", "예순세 살"], ["72", "일흔두 살"], ["88", "여든여덟 살"],
          ["30", "서른 살"], ["41", "마흔한 살"], ["56", "쉰여섯 살"], ["90", "아흔 살"],
        ].map(([number, korean]) => <article key={number} className="grid grid-cols-[65px_1fr] items-center rounded-xl border border-[#ead8be] bg-white p-3"><b className="text-[#9b6b32]">{number}岁</b><span className={`text-sm font-black transition ${revealed.age ? "opacity-100" : "opacity-0"}`}>{korean}</span></article>)}</div>
      ...
      "16": content("16", "2. 普通词 → 敬语词", "句尾礼貌并不等于主体敬语；名词和特殊动词也要一起升级。", <Crown size={22} />, <>
        <div className="mt-4 overflow-hidden rounded-2xl border border-[#ead0d6]"><div className="grid grid-cols-3 bg-[#fff4f6] p-3 text-center text-[11px] font-black"><span>普通表达</span><span>敬语表达</span><span>完整句子</span></div>{[
          ["이름", "성함", "성함이 어떻게 되세요?"],
          ["나이", "연세", "연세가 어떻게 되세요?"],
          ["집", "댁", "댁에 계세요."],
          ["생일", "생신", "생신이 언제세요?"],
          ...
          ["사람", "분", "이분은 누구세요?"],
        ].map((row) => <div key={row[0]} className="grid grid-cols-3 border-t border-[#f0dfe3] p-3 text-center text-xs font-bold">{row.map((cell, index) => <span key={`${row[0]}-${index}`} className={`${index === 1 ? "text-[#a65b68]" : ""} ${index > 0 && !revealed.honorificTable ? "opacity-0" : "opacity-100"}`}>{cell}</span>)}</div>)}</div>
      </>, <KoreanEbookRevealButton shown={Boolean(revealed.honorificTable)} onClick={() => toggle("honorificTable")} answer />),
      "17": content("17", "3. 敬语升级诊所", "找出不自然之处，再完成名词、主语和谓语的同步升级。", <Languages size={22} />, <>
        ...
        ].map(([ordinary, honorific]) => <article key={ordinary} className="grid grid-cols-[1fr_30px_1fr] items-center rounded-xl border border-[#ead8be] bg-white p-3 text-xs font-black"><span className="text-[#a65b68]">{ordinary}</span><span>→</span><span className={`text-[#487a54] transition ${revealed.clinic ? "opacity-100" : "opacity-0"}`}>{honorific}</span></article>)}</div>
      ...
      "23": content("23", "1. 家庭树表达", "从中心人物出发，用 제／내 和 가족称谓说明关系。", <UsersRound size={22} />, <>
        <div className="mt-4 grid grid-cols-3 gap-3 text-center">{[
          ...
          ["本人", "저／나", "저는 막내예요."],
        ].map(([group, words, sentence]) => <button key={group} type="button" onClick={() => speak(sentence)} className="rounded-2xl border border-[#cfddec] bg-white p-4 text-center"><b className="text-[#3e7fa3]">{group}</b><p className="mt-2 text-xs font-black">{words}</p><p className="mt-2 text-[10px] text-[#71857b]">{sentence}</p></button>)}</div>
        <Note title="文化小提示">韩国日常介绍自己的父母常使用 아버지、어머니；对他人的父母可用 아버님、어머님表示尊敬。</Note>
      </>),
      "24": content("24", "2. 听力 · 人物档案", "听两遍，记录关系、年龄、职业、能力和所在地。", <Headphones size={22} />, <>
        <button type="button" onClick={() => speak("이분은 제 어머니세요. 성함은 김미영이세요. 쉰여덟 살이시고 선생님이세요. 중국어를 잘하시고 요리도 아주 잘하세요. 지금은 부산에 사세요.")} className="mt-5 flex items-center justify-center gap-2 rounded-2xl bg-[#3e7fa3] p-4 text-sm font-black text-white"><Volume2 size={17} />播放人物介绍</button>
        <div className="mt-5 grid grid-cols-2 gap-3">{[
          ["关系", "어머니"], ["姓名", "김미영"], ["年龄", "쉰여덟 살"], ["职业", "선생님"],
          ["能力", "중국어／요리"], ["居住地", "부산"],
          ["身份句", "제 어머니세요"], ["敬语谓语", "잘하세요／사세요"],
        ].map(([label, answer]) => <article key={label} className="rounded-2xl border border-[#cfddec] bg-white p-3"><b className="text-[11px] text-[#3e7fa3]">{label}</b><p className={`mt-2 text-sm font-black transition ${revealed.listening ? "opacity-100" : "opacity-0"}`}>{answer}</p></article>)}</div>
        <p className="mt-4 text-center text-[11px] font-bold text-[#71857b]">先听写关键词，再点击右上角核对完整人物档案。</p>
      </>, <KoreanEbookRevealButton shown={Boolean(revealed.listening)} onClick={() => toggle("listening")} answer />),
      "25": content("25", "3. 家庭采访卡", "先判断对方谈的是本人、平辈还是长辈，再选择合适问题。", <Mic2 size={22} />, <>
        <div className="mt-4 grid grid-cols-2 gap-3">{[
          ["家庭人数", "가족이 몇 명이에요?"],
          ["身份", "이분은 누구세요?"],
          ["长辈姓名", "성함이 어떻게 되세요?"],
          ...
          ["居住", "어디에 사세요?"],
          ["日常", "주말에 무엇을 하세요?"],
        ].map(([topic, question]) => <button key={topic} type="button" onClick={() => speak(question)} className="rounded-2xl border border-[#cfddec] bg-white p-4 text-left"><b className="text-[10px] text-[#3e7fa3]">{topic}</b><p className="mt-2 text-sm font-black">{question}</p></button>)}</div>
        <Note title="提问礼貌" tone="amber">询问同伴本人可用 몇 살이에요?；询问对方长辈应改用 연세가 어떻게 되세요?</Note>
      </>),
      "27": content("27", "1. 阅读 · 三代人的家", "区分作者自己的普通信息与长辈的主体敬语表达。", <BookOpenCheck size={22} />, <>
        <section className="mt-4 rounded-2xl border border-[#cfe3d4] bg-white p-5"><p className="text-[11px] font-black text-[#347b69]">우리 가족</p><p className="mt-3 text-sm font-bold leading-7">우리 가족은 여섯 명입니다. 할머니, 부모님, 누나, 남동생 그리고 저입니다. 할머니는 일흔다섯 살이시고 댁에 계십니다. 아버지는 회사원이시고 운전을 잘하십니다. 어머니는 선생님이시고 요리를 아주 잘하십니다. 누나는 음악을 잘하지만 저는 노래를 잘 못합니다.</p></section>
        <div className="mt-4 grid grid-cols-2 gap-3 text-xs">{[
          ["家里共有几个人？", "여섯 명"], ["奶奶多大年纪？", "일흔다섯 살"],
          ["父亲擅长什么？", "운전"], ["作者和姐姐的能力有什么不同？", "누나는 음악을 잘하지만 저는 노래를 잘 못합니다."],
          ["母亲的职业是什么？", "선생님"], ["奶奶在哪里？", "댁에 계십니다"],
        ].map(([question, answer], index) => <article key={question} className="rounded-xl bg-[#e7f5f1] p-3 font-bold"><p><span className="mr-2 text-[#347b69]">{index + 1}.</span>{question}</p><p className={`mt-2 text-[#347b69] transition ${revealed.reading ? "opacity-100" : "opacity-0"}`}>{answer}</p></article>)}</div>
      </>, <KoreanEbookRevealButton shown={Boolean(revealed.reading)} onClick={() => toggle("reading")} answer />),
      "28": content("28", "2. 写作 · 我的家庭介绍", "写 8—10 句原创介绍，不必公开真实姓名，可使用虚构资料练习。", <NotebookPen size={22} />, <>
        ...
        <section className="mt-4 rounded-2xl border border-dashed border-[#9fc8b9] bg-[#fbfdfa] p-5"><p className="text-[11px] font-black text-[#347b69]">原创示范</p><p className="mt-3 text-sm font-bold leading-7">우리 가족은 네 명이에요. 이분은 제 아버지세요. 쉰두 살이시고 회사원이세요. 운동을 잘하시지만 요리는 잘 못하세요. 제 어머니는 선생님이세요. 어머니는 요리를 아주 잘하시고 책을 많이 읽으세요. 저는 음악을 잘하지만 제 동생은 노래를 못해요.</p></section>
        <div className="mt-4 grid grid-cols-2 gap-2 text-[11px] font-black text-[#347b69]">{["✓ 两种家庭关系","✓ 年龄与职业","✓ 能力三级比较","✓ 名词／主体敬语"].map((item) => <span key={item} className="rounded-xl bg-[#f2f8f3] px-3 py-2">{item}</span>)}</div>
      </>),
      "30": content("30", "1. 家庭与年龄闪测", "看到中文后两秒内说出韩语。", <UsersRound size={22} />, <div className="mt-4 grid grid-cols-3 gap-2.5">{[
        ["爷爷", "할아버지"], ["奶奶", "할머니"], ["父亲", "아버지"], ["母亲", "어머니"],
        ["丈夫", "남편"], ["妻子", "아내"], ["儿子", "아들"], ["女儿", "딸"],
        ["20岁", "스무 살"], ["50岁", "쉰 살"], ["70岁", "일흔 살"], ["90岁", "아흔 살"],
      ].map(([chinese, korean], index) => <article key={`${chinese}-${korean}`} className="rounded-xl border border-[#cfe3d4] bg-white p-3 text-center"><p className="text-[10px] text-[#487a54]">{index + 1}</p><b>{chinese}</b><p className={`mt-2 rounded-lg bg-[#e8f4eb] p-2 text-xs font-black transition ${revealed.words ? "opacity-100" : "opacity-0"}`}>{korean}</p></article>)}</div>, <KoreanEbookRevealButton shown={Boolean(revealed.words)} onClick={() => toggle("words")} answer />),
      "31": content("31", "2. 敬语词汇检测", "把普通词升级为对长辈使用的表达。", <Crown size={22} />, <>
        <div className="mt-4 grid grid-cols-2 gap-2.5">{[
          ["이름", "성함"], ["나이", "연세"], ["집", "댁"], ["생일", "생신"],
          ["사람", "분"], ["있다", "계시다"], ["먹다", "드시다"], ["자다", "주무시다"],
          ["말하다", "말씀하시다"], ["죽다", "돌아가시다"],
          ["밥", "진지"], ["부모", "부모님"],
        ].map(([ordinary, honorific], index) => <article key={`${ordinary}-${index}`} className="grid min-h-[54px] grid-cols-[1fr_30px_1fr] items-center rounded-xl border border-[#cfe3d4] bg-white p-4 text-xs font-black"><span>{ordinary}</span><span>→</span><span className={`text-[#487a54] transition ${revealed.honorifics ? "opacity-100" : "opacity-0"}`}>{honorific}</span></article>)}</div>
      ...
      "32": content("32", "3. 综合句型检测", "完成所属、能力、名词敬语和主体敬语表达。", <CheckCircle2 size={22} />, <div className="mt-4 space-y-2">{[
        ["这位是我的父亲。", "이분은 제 아버지세요."],
        ...
        ["老师住在首尔。", "선생님이 서울에 사세요."],
      ].map(([question, answer], index) => <article key={`${question}-${index}`} className="grid grid-cols-2 gap-3 rounded-xl border border-[#cfe3d4] bg-white p-3 text-xs font-bold"><span>{index + 1}. {question}</span><span className={`text-[#487a54] transition ${revealed.grammar ? "opacity-100" : "opacity-0"}`}>{answer}</span></article>)}</div>, <KoreanEbookRevealButton shown={Boolean(revealed.grammar)} onClick={() => toggle("grammar")} answer />),
      "33": content("33", "4. 口语验收 · 十句家庭介绍", "不看稿介绍两位家人，再回答同伴的三个追问。", <Mic2 size={22} />, <>
        <section className="mt-4 rounded-2xl border border-[#cfe3d4] bg-[#f2f8f3] p-5"><p className="text-xs font-black text-[#487a54]">八项必达信息</p><div className="mt-4 grid grid-cols-2 gap-3 text-xs">{["说明家庭人数","介绍两种关系","使用제或내","说出一个年龄","介绍职业","比较能力等级","使用N(이)세요","使用特殊敬语动词"].map((task, index) => <label key={`${task}-${index}`} className="flex items-center gap-2 rounded-xl bg-white p-3 font-bold"><input type="checkbox" className="accent-[#487a54]" />{task}</label>)}</div></section>
        <div className="mt-4 grid grid-cols-3 gap-3 text-center text-[11px] font-black"><span className="rounded-xl bg-[#f1f6fb] px-3 py-2 text-[#3d6f9f]">信息完整 40%</span><span className="rounded-xl bg-[#f2f8f3] px-3 py-2 text-[#487a54]">敬语正确 40%</span><span className="rounded-xl bg-[#fff8ed] px-3 py-2 text-[#9a6b2f]">表达自然 20%</span></div>
        <button type="button" onClick={() => speak("우리 가족은 네 명이에요. 이분은 제 아버지세요. 아버지는 쉰두 살이세요. 회사원이시고 운전을 잘하세요. 주말에는 댁에서 책을 읽으세요. 이분은 제 어머니세요. 어머니는 선생님이세요. 요리를 아주 잘하시고 음악도 좋아하세요. 지금은 부모님 두 분 모두 부산에 사세요. 저는 주말마다 부모님 댁에 가요.")} className="mt-auto flex items-center justify-center gap-2 rounded-2xl bg-[#487a54] p-4 text-sm font-black text-white"><Volume2 size={16} />播放十句示范</button>
      </>),
      "34": <div className="flex h-full flex-col justify-center"><div className="mx-auto w-full max-w-[440px] text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#e8f4eb] text-[#487a54]"><Sparkles size={27} /></span><p className="mt-4 text-xs font-black tracking-[0.18em] text-[#487a54]">LESSON 09 · COMPLETE</p><h2 className="mt-3 text-4xl font-black">이분은 누구세요?</h2><p className="mt-3 text-lg font-black">你已经完成第九课</p><p className="mx-auto mt-3 max-w-[390px] text-sm leading-7 text-[#60736a]">现在你能介绍家庭关系与能力，并根据主体身份正确切换名词敬语、身份敬语和动作敬语。</p><div className="mt-4 grid grid-cols-2 gap-3 text-left">{[
        ["01", "说明所属", "N(의) N"], ["02", "评价能力", "N을/를 잘하다"],
        ["03", "敬语身份", "N(이)세요"], ["04", "主体敬语", "A/V-(으)시-"],
      ].map(([index, title, detail]) => <div key={index} className="rounded-xl border border-[#dce8e1] bg-white px-4 py-3"><p className="text-[10px] font-black text-[#487a54]">{index}</p><p className="mt-1 text-xs font-black">{title}</p><p className="mt-1 text-[10px] text-[#71857b]">{detail}</p></div>)}</div><div className="mt-4 rounded-2xl border border-[#cfe3d4] bg-[#f2f8f3] px-5 py-3.5 text-left"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black tracking-[0.14em] text-[#487a54]">LESSON 9 TEST · 本课测试</p><p className="mt-1 text-xs font-bold text-[#52685e]">检验家庭、年龄、能力表达和主体敬语。</p></div><button type="button" onClick={() => window.location.assign("/dashboard/assignments/korean")} className="shrink-0 rounded-full bg-white px-3 py-2 text-[10px] font-black text-[#487a54] shadow-sm">前往测试专区</button></div></div><button type="button" onClick={() => flipBookRef.current?.pageFlip()?.flip(1)} className="mt-4 rounded-full bg-[#eaf2fb] px-4 py-3 text-xs font-black text-[#3d6f9f]">返回目录</button></div></div>,
    };
    return pages[number];
  }
