      ...
      {lines.map((line, index) => (
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className={`mt-1 text-[11px] font-bold text-[var(--foreground-secondary)] ${showChinese ? "opacity-100" : "opacity-0"}`}>{line.chinese}</p>
          </div>
  ...
  const pages = [
    ...
    <Page key="05-03" number="03">
      <div className="flex h-full flex-col">
        <Heading title="从一句过去时，到一段故事" description="第五课的目标是回答：什么时候、做了什么、然后怎样、为什么。" icon={<History aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.chinese03)} onClick={() => toggle("chinese03")} />} />
        ...
        <section className="mt-5 rounded-2xl bg-[var(--status-warning-surface)] p-5">
          <p className="text-xs font-bold text-[var(--status-warning)]">本课叙事发动机</p>
    ...
    <Page key="05-05" number="05">
      <div className="flex h-full flex-col">
        <Heading title="1. 汉字词数字与时间单位" description="日期、分钟、号码和价格常使用汉字词数字；本页先掌握 1—10。" icon={<Clock3 aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.chinese05)} onClick={() => toggle("chinese05")} />} />
    ...
    <Page key="05-06" number="06">
      <div className="flex h-full flex-col">
        <Heading title="2. 月份与日期" description="数字直接加 월 构成月份，但 6 月和 10 月必须记住特殊形式。" icon={<CalendarDays aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.chinese06)} onClick={() => toggle("chinese06")} />} />
        <VocabularyGrid items={monthWords} speak={speak} showChinese={Boolean(revealed.chinese06)} />
        <p className="mt-auto rounded-xl bg-[var(--status-warning-surface)] p-3 text-[11px] font-bold text-[var(--status-warning)]">
    ...
    <Page key="05-07" number="07">
      <div className="flex h-full flex-col">
        <Heading title="3. 星期与时间词" description="星期由一个汉字词加 요일 构成；注意无需加 에 的四个高频时间词。" icon={<CalendarDays aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.chinese07)} onClick={() => toggle("chinese07")} />} />
        <VocabularyGrid items={weekdayWords} speak={speak} showChinese={Boolean(revealed.chinese07)} />
        <div className="mt-auto grid grid-cols-4 gap-2 text-center text-[10px] font-bold text-[var(--destructive)]">
          {["어제 × 에", "오늘 × 에", "내일 × 에", "지금 × 에"].map((item) => <span key={item} className="rounded-xl bg-[var(--card)] p-2">{item}</span>)}
        </div>
      </div>
    </Page>,
    <Page key="05-08" number="08">
      <div className="flex h-full flex-col">
        <Heading title="4. 周末活动与状态" description="把动作词直接与过去式一起记忆，为后面的叙事练习做准备。" icon={<Sparkles aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.chinese08)} onClick={() => toggle("chinese08")} />} />
        ...
        <div className="mt-auto grid grid-cols-3 gap-2 text-center text-[10px] font-bold">
          <span className="rounded-xl bg-[var(--accent)] p-2 text-[var(--primary)]">만나다 → 만났어요</span>
          <span className="rounded-xl bg-[var(--status-success-surface)] p-2 text-[var(--status-success)]">찍다 → 찍었어요</span>
          <span className="rounded-xl bg-[var(--status-warning-surface)] p-2 text-[var(--status-warning)]">미안하다 → 미안했어요</span>
        </div>
      </div>
    </Page>,
    ...
    <Page key="05-10" number="10">
      <div className="flex h-full flex-col">
        <Heading title="1. 날짜와 요일" description="学习读写年月日和星期，并能询问“几月几号、星期几”。" icon={<CalendarDays aria-hidden="true" size={22} />} />
        <section className="mt-4 rounded-2xl bg-[var(--card)] p-5 text-center">
          <p className="text-[11px] font-bold text-[var(--primary)]">日期排列</p>
          <p className="mt-3 text-lg font-bold">年 + 년　月 + 월　日 + 일</p>
          <p className="mt-3 text-sm font-bold">이천이십육 년 오월 십일</p>
          <p className="mt-1 text-xs text-[var(--foreground-secondary)]">2026年5月10日</p>
        </section>
        <div className="mt-3 grid grid-cols-2 gap-3">
          <NoteBox label="询问日期" tone="blue">
            <RuleSentence text="오늘이 몇 월 며칠이에요?" speak={speak}>오늘이 <b>몇 월 며칠</b>이에요?</RuleSentence>
            <span className="block text-[10px] text-[var(--foreground-secondary)]">今天是几月几号？</span>
            <RuleSentence text="생일이 몇 월 며칠이에요?" speak={speak}>생일이 몇 월 며칠이에요?</RuleSentence>
            <span className="block text-[10px] text-[var(--foreground-secondary)]">生日是几月几号？</span>
          </NoteBox>
          <NoteBox label="询问星期" tone="green">
            <RuleSentence text="오늘이 무슨 요일이에요?" speak={speak}>오늘이 <b>무슨 요일</b>이에요?</RuleSentence>
            <span className="block text-[10px] text-[var(--foreground-secondary)]">今天星期几？</span>
            <RuleSentence text="금요일이에요." speak={speak}>금요일이에요.</RuleSentence>
            <span className="block text-[10px] text-[var(--foreground-secondary)]">是星期五。</span>
          </NoteBox>
        </div>
        ...
        <section className="mt-3 rounded-2xl border border-[var(--border)] bg-white p-4">
          <p className="text-xs font-bold">日期与星期是两个不同答案</p>
          <div className="mt-3 grid grid-cols-2 gap-2 text-center text-xs">
            <p className="rounded-xl bg-[var(--accent)] p-3"><b>오월 십일</b><br />日期</p>
            <p className="rounded-xl bg-[var(--status-success-surface)] p-3"><b>금요일</b><br />星期</p>
          </div>
        </section>
        <p className="mt-auto text-[11px] font-bold text-[var(--foreground-secondary)]">며칠 是“几号／几天”的固定疑问词，不要拆成 몇 일 来使用。</p>
      </div>
    </Page>,
    <Page key="05-11" number="11">
      <div className="flex h-full flex-col">
        <Heading title="2. 时间名词 + 에" description="把动作固定在具体时间点，相当于“在……时候”。" icon={<Clock3 aria-hidden="true" size={22} />} />
        <section className="mt-4 rounded-2xl bg-[var(--accent)] p-5 text-center">
          <p className="text-[11px] font-bold text-[var(--primary)]">时间句骨架</p>
          <p className="mt-3 text-lg font-bold">具体时间 + 에 + 动作</p>
          <p className="mt-3 text-sm font-bold">토요일에 친구를 만났어요.</p>
        </section>
        <div className="mt-3 grid grid-cols-2 gap-3">
    ...
    <Page key="05-12" number="12">
      <div className="flex h-full flex-col">
        <Heading title="3. V-았／었-" description="表示过去发生的动作或过去的状态；选择规则与现在时 -아／어요 同源。" icon={<History aria-hidden="true" size={22} />} />
        ...
        <section className="mt-3 rounded-2xl bg-[var(--card)] p-4">
          <p className="text-xs font-bold text-[var(--primary)]">现在与过去的声音对照</p>
        ...
        <p className="mt-auto text-[11px] font-bold text-[var(--foreground-secondary)]">ㅆ 是“事情已经跨过现在”的时间标记，听到它就先想到过去。</p>
      </div>
    </Page>,
    <Page key="05-13" number="13">
      <div className="flex h-full flex-col">
        <Heading title="4. V-고" description="直接接在动词词干后，连接并列动作或先后发生的动作。" icon={<Link2 aria-hidden="true" size={22} />} />
        <section className="mt-4 rounded-2xl bg-[var(--card)] p-5 text-center">
          <p className="text-[11px] font-bold text-[var(--primary)]">连接结构</p>
          <p className="mt-3 text-lg font-bold">动作1 词干 + 고 + 动作2（句尾时态）</p>
          <p className="mt-3 text-sm font-bold">친구를 만나고 밥을 먹었어요.</p>
        </section>
        <div className="mt-3 grid grid-cols-2 gap-3">
          <NoteBox label="先后顺序" tone="blue">
            <RuleSentence text="밥을 먹고 산책했어요." speak={speak}>밥을 먹고 산책했어요.</RuleSentence>
            <span className="block text-[10px] text-[var(--foreground-secondary)]">吃了饭，然后散步。</span>
            <RuleSentence text="책을 읽고 잤어요." speak={speak}>책을 읽고 잤어요.</RuleSentence>
            <span className="block text-[10px] text-[var(--foreground-secondary)]">看书后睡了。</span>
          </NoteBox>
          <NoteBox label="并列罗列" tone="green">
            <RuleSentence text="사진을 찍고 음악을 들었어요." speak={speak}>사진을 찍고 음악을 들었어요.</RuleSentence>
            <span className="block text-[10px] text-[var(--foreground-secondary)]">拍了照，也听了音乐。</span>
        ...
        <p className="mt-auto text-[11px] font-bold text-[var(--foreground-secondary)]">-고 本身不表示过去，它只连接动作；整句时态由最后一个动词决定。</p>
      </div>
    </Page>,
    ...
    <Page key="05-15" number="15">
      <div className="flex h-full flex-col">
        <Heading title="1. 动词三态转换表" description="从词典形出发，同时说出现在时和过去时，建立快速转换反射。" icon={<History aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.forms)} onClick={() => toggle("forms")} answer />} />
        <div className="mt-4 overflow-hidden rounded-2xl border border-[var(--border)]">
          <div className="grid grid-cols-3 bg-[var(--status-warning-surface)] px-4 py-3 text-[11px] font-bold text-[var(--status-warning)]">
          ...
          {[
            ...
            <div key={base} className="grid grid-cols-3 border-t border-[var(--border)] bg-white px-4 py-2 text-[11px] font-bold">
              <span>{base}</span>
              <span className={revealed.forms ? "opacity-100" : "opacity-0"}>{present}</span>
              <span className={`text-[var(--status-warning)] ${revealed.forms ? "opacity-100" : "opacity-0"}`}>{past}</span>
            </div>
          ))}
        </div>
        <p className="mt-auto text-[11px] font-bold text-[var(--foreground-secondary)]">训练法：遮住后两列，每个词必须在两秒内连续说出现在形和过去形。</p>
      </div>
    </Page>,
    <Page key="05-16" number="16">
      <div className="flex h-full flex-col">
        <Heading title="2. 时间助词诊断" description="判断是否需要 에；错误选项不是“少一点自然”，而是结构错误。" icon={<ClipboardCheck aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.time)} onClick={() => toggle("time")} answer />} />
        <div className="mt-4 space-y-2.5">
          {[
            ...
            <div key={question} className="grid grid-cols-[28px_1.1fr_1fr] items-center gap-3 rounded-xl border border-[var(--border)] bg-white p-3">
              <span className="text-xs font-bold text-[var(--status-warning)]">{index + 1}</span>
              <p className="text-xs font-bold">{question}</p>
              <p className={`text-[11px] font-bold text-[var(--status-success)] ${revealed.time ? "opacity-100" : "opacity-0"}`}>{answer}<span className="ml-2 text-[var(--foreground-secondary)]">· {reason}</span></p>
            </div>
          ))}
        </div>
        <p className="mt-auto rounded-xl bg-[var(--status-warning-surface)] p-3 text-[11px] font-bold text-[var(--status-warning)]">四词口令：어제、오늘、내일、지금，后面永远不加 에。</p>
      </div>
    </Page>,
    <Page key="05-17" number="17">
      <div className="flex h-full flex-col">
        <Heading title="3. 故事组装台" description="把日期、星期、时间助词、过去式和 -고 放进同一段叙事。" icon={<PencilLine aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.story)} onClick={() => toggle("story")} answer />} />
          ...
          {[
            ...
            <article key={clue} className="rounded-2xl border border-[var(--border)] bg-white p-4">
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold"><span className="mr-2 text-[var(--status-warning)]">{index + 1}.</span>{clue}</p>
                <span className="rounded-full bg-[var(--status-warning-surface)] px-3 py-1 text-[10px] font-bold text-[var(--status-warning)]">{relation}</span>
              </div>
              <p className={`mt-3 text-sm font-bold ${revealed.story ? "opacity-100" : "opacity-0"}`}>{answer}</p>
            </article>
          ))}
        </div>
    ...
    <Page key="05-19" number="19">
      <div className="flex h-full flex-col">
        <Heading title="场景 1 · 周末做了什么？" description="目标：用过去时和 -고 回顾两个连续活动。" icon={<MessageCircle aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.chinese19)} onClick={() => toggle("chinese19")} />} />
        ...
        <section className="mt-4 rounded-2xl bg-[var(--status-warning-surface)] p-4">
          <p className="text-xs font-bold text-[var(--destructive)]">周末活动卡</p>
        ...
        <p className="mt-auto text-[11px] text-[var(--foreground-secondary)]">도 表示“也”，让追问自然接续前面的活动。</p>
      </div>
    </Page>,
    <Page key="05-20" number="20">
      <div className="flex h-full flex-col">
        <Heading title="场景 2 · 生日是几号？" description="目标：询问日期，并用星期与具体日期安排见面。" icon={<CalendarDays aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.chinese20)} onClick={() => toggle("chinese20")} />} />
        ...
        <p className="mt-auto rounded-xl border border-[var(--border)] p-3 text-xs">替换任务：用自己的生日和真实星期重新回答。</p>
      </div>
    </Page>,
    <Page key="05-21" number="21">
      <div className="flex h-full flex-col">
        <Heading title="场景 3 · 为什么迟到了？" description="扩展表达：认识会话词 그래서，并用 미안했어요 表达歉意；不列入本课四项核心语法。" icon={<History aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.chinese21)} onClick={() => toggle("chinese21")} />} />
        ...
        <section className="mt-4 rounded-2xl bg-[var(--status-warning-surface)] p-4">
          <p className="text-xs font-bold text-[var(--destructive)]">因果替换卡</p>
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-bold">
            <p className="rounded-xl bg-white p-3">아팠어요 → 집에서 쉬었어요</p>
            <p className="rounded-xl bg-white p-3">수업이 끝났어요 → 친구를 만났어요</p>
          </div>
        </section>
        <p className="mt-auto text-[11px] text-[var(--foreground-secondary)]">왜（为什么）用于询问原因；回答时可以直接给原因，再用 그래서 接结果。</p>
      </div>
    </Page>,
    ...
    <Page key="05-23" number="23">
      <div className="flex h-full flex-col">
        <Heading title="1. 日期听写" description="先听月份，再听日期；特别注意 유월 和 시월。" icon={<Headphones aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.dates)} onClick={() => toggle("dates")} answer />} />
        <div className="mt-4 grid grid-cols-2 gap-3">
          {[
            ...
            <button key={date} type="button" onClick={() => speak(date)} className="rounded-2xl border border-[var(--border)] bg-white p-4 text-left">
              <p className="text-[10px] font-bold text-[var(--primary)]">日期语音 {index + 1}</p>
              <p className={`mt-3 text-sm font-bold ${revealed.dates ? "opacity-100" : "opacity-0"}`}>{date}</p>
              <p className={`mt-1 text-[11px] font-bold text-[var(--foreground-secondary)] ${revealed.dates ? "opacity-100" : "opacity-0"}`}>{chinese}</p>
            </button>
          ))}
        </div>
        <section className="mt-4 rounded-2xl bg-[var(--accent)] p-4">
          <p className="text-[11px] font-bold text-[var(--primary)]">日期拆分法</p>
        ...
        <p className="mt-3 text-[11px] font-bold text-[var(--foreground-secondary)]">听写顺序：先写月份，再写数字日期；유월、시월要按固定形式记录。</p>
      </div>
    </Page>,
    <Page key="05-24" number="24">
      <div className="flex h-full flex-col">
        <Heading title="2. 听见过去时" description="比较现在时和过去时，捕捉句尾中的 ㅆ 音及缩约后的整体声音。" icon={<Headphones aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.chinese24)} onClick={() => toggle("chinese24")} />} />
        <div className="mt-4 space-y-3">
          {[
            ...
            <button key={pair} type="button" onClick={() => speak(pair.replace("／", ""))} className="w-full rounded-2xl border border-[var(--border)] bg-white p-4 text-left">
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold">PAIR {index + 1}</p><Volume2 aria-hidden="true" size={15} className="text-[var(--primary)]" />
              </div>
              <p className="mt-3 text-sm font-bold">{pair}</p>
              <p className={`mt-1 text-[10px] font-bold text-[var(--foreground-secondary)] ${revealed.chinese24 ? "opacity-100" : "opacity-0"}`}>{meaning}</p>
            </button>
          ))}
        </div>
    ...
    <Page key="05-25" number="25">
      <div className="flex h-full flex-col">
        <Heading title="3. 我的周末记忆路线" description="用 45 秒讲述三个时间点、三个动作和一个原因结果。" icon={<Mic2 aria-hidden="true" size={22} />} />
        <div className="mt-5 grid grid-cols-2 gap-4">
          <section className="rounded-2xl border border-[var(--border)] bg-white p-5">
            <p className="text-xs font-bold text-[var(--primary)]">时间轴脚手架</p>
          ...
          <section className="rounded-2xl bg-[var(--accent)] p-5">
            ...
            <button type="button" onClick={() => speak("토요일에 친구를 만났어요. 밥을 먹고 사진을 찍었어요. 일요일에는 책을 읽고 집에서 쉬었어요.")} className="mt-4 rounded-full bg-white px-4 py-2 text-[11px] font-bold text-[var(--primary)]">播放示范</button>
          </section>
        </div>
        ...
        <p className="mt-auto text-center text-[11px] font-bold text-[var(--foreground-secondary)]">先画四格时间轴，再开口；画面会帮助你保持事件顺序。</p>
      </div>
    </Page>,
    ...
    <Page key="05-27" number="27">
      <div className="flex h-full flex-col">
        <Heading title="1. 阅读 · 수진 씨의 주말" description="圈出日期与星期，给过去式画线，用箭头连接 -고 前后的动作。" icon={<BookOpenCheck aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.reading)} onClick={() => toggle("reading")} answer />} />
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
          <div className={`rounded-xl border border-[var(--border)] bg-white p-4 ${revealed.reading ? "opacity-100" : "opacity-0"}`}>
            <p className="font-bold text-[var(--status-success)]">星期六</p>
            <p className="mt-2 leading-6">见朋友 → 吃午饭 → 散步 → 拍照</p>
          </div>
          <div className={`rounded-xl border border-[var(--border)] bg-white p-4 ${revealed.reading ? "opacity-100" : "opacity-0"}`}>
            <p className="font-bold text-[var(--status-success)]">星期日</p>
    ...
    <Page key="05-28" number="28">
      <div className="flex h-full flex-col">
        <Heading title="2. 写作 · 四格周末日记" description="每格只写一句，但四句必须构成有时间、有顺序、有原因的完整故事。" icon={<PencilLine aria-hidden="true" size={22} />} />
        <div className="mt-5 grid grid-cols-2 gap-3">
          {[
            ...
            <section key={title} className="rounded-2xl border border-[var(--border)] bg-white p-5">
              <p className="text-[10px] font-bold text-[var(--status-success)]">{title}</p>
              <p className="mt-3 text-sm font-bold">{template}</p>
              <div className="mt-4 h-10 rounded-lg border-b border-dashed border-[var(--border)]" />
            </section>
          ))}
        </div>
        ...
        <p className="mt-auto rounded-xl bg-[var(--card)] p-3 text-[11px] font-bold">写完后圈出所有 ㅆ，并确认 -고 前面的动词没有变过去时。</p>
      </div>
    </Page>,
    ...
    <Page key="05-30" number="30">
      <div className="flex h-full flex-col">
        <Heading title="1. 两秒过去式挑战" description="看到词典形后两秒内说出过去式，再展开核对。" icon={<History aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.past)} onClick={() => toggle("past")} answer />} />
        <div className="mt-4 grid grid-cols-3 gap-2.5">
          {[
            ...
            <article key={base} className="rounded-xl border border-[var(--border)] bg-white p-3 text-center">
              ...
              <p className={`mt-2 rounded-lg bg-[var(--status-success-surface)] p-2 text-[11px] font-bold text-[var(--status-success)] ${revealed.past ? "opacity-100" : "opacity-0"}`}>{answer}</p>
            </article>
          ))}
        </div>
        <p className="mt-auto text-center text-[11px] text-[var(--foreground-secondary)]">10 个以上在两秒内完成：进入综合检测；否则回到第 15 页继续三态训练。</p>
      </div>
    </Page>,
    <Page key="05-31" number="31">
      <div className="flex h-full flex-col">
        <Heading title="2. 十题综合检测" description="同时检查日期星期、时间助词、过去式和 -고。" icon={<ClipboardCheck aria-hidden="true" size={22} />} action={<KoreanEbookRevealButton shown={Boolean(revealed.test)} onClick={() => toggle("test")} answer />} />
        ...
        <p className="mt-auto text-center text-[11px] text-[var(--foreground-secondary)]">9—10题：进入口语验收；8题以下：按错误类型返回对应语法页。</p>
      </div>
    </Page>,
    <Page key="05-32" number="32">
      <div className="flex h-full flex-col">
        <Heading title="3. 口语验收 · 过去的一天" description="不看稿讲述 45 秒，并回答“什么时候”和“为什么”两个追问。" icon={<Mic2 aria-hidden="true" size={22} />} />
        <section className="mt-5 rounded-2xl border border-[var(--border)] bg-[var(--status-success-surface)] p-5">
          <p className="text-xs font-bold text-[var(--status-success)]">四项必达信息</p>
        ...
        <div className="mt-5 grid grid-cols-2 gap-4">
          <section className="rounded-2xl border border-[var(--border)] bg-white p-5">
            <p className="text-xs font-bold">时态自检</p>
        ...
        <button type="button" onClick={() => speak("오월 십일은 토요일이에요. 토요일에 친구를 만났어요. 같이 밥을 먹고 사진을 찍었어요. 일요일에는 책을 읽고 집에서 쉬었어요.")} className="mt-auto flex items-center justify-center gap-2 rounded-2xl bg-[var(--status-success)] p-4 text-sm font-bold text-white"><Volume2 aria-hidden="true" size={16} />播放最终示范</button>
      </div>
    </Page>,
    <Page key="05-33-ending" number="33">
      <div className="flex h-full flex-col justify-center">
        <div className="mx-auto w-full max-w-[440px] text-center">
          ...
          <p className="mt-4 text-xs font-bold tracking-[0.18em] text-[var(--status-success)]">LESSON 05 · COMPLETE</p>
          <h3 className="mt-3 text-[34px] font-bold text-[var(--status-success)]">주말에 친구를 만났어요.</h3>
          <p className="mt-3 text-lg font-bold text-[var(--foreground)]">你已经完成第五课</p>
          ...
          <div className="mt-4 rounded-2xl border border-[var(--border)] bg-[var(--status-success-surface)] px-5 py-3.5 text-left">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-bold tracking-[0.14em] text-[var(--status-success)]">LESSON 5 TEST · 本课测试</p>
                <p className="mt-1 text-xs font-bold text-[var(--foreground-secondary)]">前往章节测试专区，检验日期星期、时间助词、过去时与动作顺序。</p>
              </div>
              <KoreanEbookTestLink />
            </div>
          </div>
          <div className="mt-4 rounded-2xl bg-[var(--accent)] px-5 py-3.5 text-left">
            <p className="text-[10px] font-bold tracking-[0.14em] text-[var(--primary)]">NEXT · LESSON 06</p>
            <div className="mt-2 flex items-end justify-between gap-4">
              <div>
                <p className="text-lg font-bold text-[var(--status-success)]">얼마예요?</p>
                <p className="mt-1 text-[11px] text-[var(--foreground-secondary)]">下一课：学习购物数量、价格与实际交易表达。</p>
              </div>
          ...
          <p className="mt-1 text-[11px] font-bold text-[var(--foreground-secondary)]">能讲述昨天，意味着你的韩语第一次拥有了时间。</p>
        </div>
      </div>
    </Page>,
  ];
