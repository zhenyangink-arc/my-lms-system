...
export function KoreanLevelOneLessonEightBook({
  ...
  function renderPage(number: string) {
    ...
    const pages: Record<string, ReactNode> = {
      ...
      "34": <div className="flex h-full flex-col justify-center"><div className="mx-auto w-full max-w-[440px] text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--status-success-surface)] text-[var(--status-success)]"><Sparkles aria-hidden="true" size={27} /></span><p className="mt-4 text-xs font-bold tracking-[0.18em] text-[var(--status-success)]">LESSON 08 · 1A COMPLETE</p><h3 className="mt-3 text-4xl font-bold">영화 볼까요?</h3><p className="mt-3 text-lg font-bold">你已经完成韩国语 1A</p><p className="mx-auto mt-3 max-w-[390px] text-sm leading-7 text-[var(--foreground-secondary)]">现在你能主动提出活动、商量约会、处理方案变化，并自然表达现场发现与评价。</p><div className="mt-4 grid grid-cols-2 gap-3 text-left">{[
        ["01", "提出建议", "V-(으)ㄹ까요?"], ["02", "掌握变化", "ㄷ 불규칙"],
        ["03", "指示对象", "이/그/저 + N"], ["04", "表达感叹", "A/V-네요"],
      ].map(([index, title, detail]) => <div key={index} className="rounded-xl border border-[var(--border)] bg-white px-4 py-3"><p className="text-[10px] font-bold text-[var(--status-success)]">{index}</p><p className="mt-1 text-xs font-bold">{title}</p><p className="mt-1 text-[10px] text-[var(--foreground-secondary)]">{detail}</p></div>)}</div><div className="mt-4 rounded-2xl border border-[var(--border)] bg-[var(--status-success-surface)] px-5 py-3.5 text-left"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold tracking-[0.14em] text-[var(--status-success)]">LESSON 8 TEST · 1A 综合测试</p><p className="mt-1 text-xs font-bold text-[var(--foreground-secondary)]">前往测试专区，完成韩国语 1A 阶段验收。</p></div><button type="button" onClick={() => window.location.assign("/dashboard/assignments/korean")} className="shrink-0 rounded-full bg-white px-3 py-2 text-[10px] font-bold text-[var(--status-success)] shadow-sm">前往测试专区</button></div></div><button type="button" onClick={() => flipBookRef.current?.pageFlip()?.flip(1)} className="mt-4 rounded-full bg-[var(--accent)] px-4 py-3 text-xs font-bold text-[var(--primary)]">返回目录</button></div></div>,
    };
    return pages[number];
  }
