...
function Note({ title, children, tone = "green" }: { title: string; children: ReactNode; tone?: "green" | "rose" | "amber" }) {
  const style = tone === "rose" ? "border-[var(--border)] bg-[var(--card)]" : tone === "amber" ? "border-[var(--border)] bg-[var(--status-warning-surface)]" : "border-[var(--border)] bg-[var(--status-success-surface)]";
  return <section className={`rounded-2xl border p-5 ${style}`}><p className="text-xs font-bold text-[var(--status-success)]">{title}</p><div className="mt-2 text-[13px] font-bold leading-7 text-[var(--foreground-secondary)]">{children}</div></section>;
}
...
export function KoreanLevelOneLessonSixteenBook({ lesson, isFullscreen, initialPage = 0, onPageChange, speechRate = 0.78 }: { lesson: KoreanLevelOneLesson; isFullscreen: boolean; initialPage?: number; onPageChange?: (page: number) => void; speechRate?: number }) {
  ...
  const grammar = (page: string, title: string, purpose: string, rules: string[], examples: Array<[string, string]>, caution: string) => section(page, title, purpose, <NotebookPen aria-hidden="true" size={22} />, <><div className="mt-4 grid grid-cols-2 gap-3"><Note title="构成规则">{rules.map((rule) => <p key={rule}>{rule}</p>)}</Note><Note title="注意点" tone="amber">{caution}</Note></div><div className="mt-4 grid grid-cols-2 gap-2">{examples.map(([ko, zh]) => <article key={ko} className="rounded-xl border border-[var(--border)] bg-white p-3 text-xs">{speakLine(ko)}<p className={`mt-1 text-[10px] text-[var(--foreground-secondary)] ${revealed[`c${page}`] ? "opacity-100" : "opacity-0"}`}>{zh}</p></article>)}</div><div className="mt-3 rounded-xl bg-[var(--accent)] px-4 py-3 text-xs font-bold text-[var(--primary)]">口头替换：改变人物、时间、地点或物品，把本页例句再说两遍。</div></>, reveal(`c${page}`));

  const dividers: Record<string, [string, string, string, ReactNode]> = {
    "02": ["第一步", "课前导航", "建立“发出邀请—确认能否参加—分配任务—回复邀请”的聚会交际链。", <House aria-hidden="true" key="a" />],
    "04": ["第二步", "核心词汇", "掌握聚会、邀请、准备动作、高频副词和四项核心表达。", <PartyPopper aria-hidden="true" key="b" />],
    "09": ["第三步", "语法讲解", "四个语法各占一页：能力、承诺、移动目的和同时动作。", <NotebookPen aria-hidden="true" key="c" />],
    "14": ["第四步", "句型操练", "把能否参加、主动分工、行动目的和同时动作组合起来。", <Mail aria-hidden="true" key="d" />],
    "18": ["第五步", "实战对话", "完成邀请温居、聚会分工和婉拒再约三组八句对话。", <MessageCircle aria-hidden="true" key="e" />],
    "22": ["第六步", "听说任务", "听懂聚会时间、参加能力、分工承诺和礼物选择。", <Headphones aria-hidden="true" key="f" />],
    "26": ["第七步", "读写拓展", "读懂邀请函与回复，并独立写出完整邀请信息。", <BookOpenCheck aria-hidden="true" key="g" />],
    "29": ["第八步", "自测与复盘", "综合检查词汇、四项语法、邀请回复与聚会交际能力。", <CheckCircle2 aria-hidden="true" key="h" />],
  };
  ...
  function renderPage(number: string): ReactNode {
    ...
    if (number === "03") return section(number, "本课学习路线", "从一句“能来吗”走到完整邀请、回复与聚会协作。", <House aria-hidden="true" />, <div className="mt-5 grid grid-cols-2 gap-3">{[["01","说明活动","집들이를 해요"],["02","确认能力","올 수 있어요?"],["03","回应邀请","갈 수 있어요"],["04","主动分工","제가 할게요"],["05","说明目的","사러 가요"],["06","同步准备","들으면서 만들어요"],["07","准确表达","일찍·같이·미리"],["08","礼貌收尾","토요일에 만나요"]].map(([n,t,d])=><Note key={n} title={`${n} · ${t}`}>{d}</Note>)}</div>);
    ...
    if (number === "23") return section(number,"1. 听力 · 温居邀请与分工","播放后记录时间、参加者、承诺和购物目的。",<Headphones aria-hidden="true" size={22}/>,<><button type="button" onClick={()=>speak("토요일 오후 여섯 시에 지수 씨 집에서 집들이를 해요. 민수 씨는 일찍 올 수 있어요. 민수 씨가 음료수를 사러 갈게요. 수진 씨는 음악을 들으면서 음식을 만들 거예요. 모두 휴지나 세제를 선물로 준비하려고 해요.")} className="mt-5 flex items-center justify-center gap-2 rounded-2xl bg-[var(--destructive)] p-4 text-sm font-bold text-white"><Volume2 aria-hidden="true" size={17}/>播放温居安排</button><Cards items={[["时间","토요일 오후 6시"],["地点","지수 씨 집"],["谁能早来？","민수 씨"],["民秀的承诺","음료수를 사러 갈게요"],["秀珍同时做什么？","들으면서 만들어요"],["礼物","휴지나 세제"],["聚会类型","집들이"],["准备方式","같이 준비해요"]]} show={Boolean(revealed.listen)}/></>,reveal("listen",true));
    if (number === "24") return section(number,"2. 文化漫步 · 집들이 선물","了解传统寓意，同时尊重现代个人需求。",<Gift aria-hidden="true" size={22}/>,<><div className="mt-4 grid grid-cols-2 gap-3"><Note title="휴지 · 卷筒纸">传统解释常把纸张顺畅展开，联系到生活和事情顺利展开。</Note><Note title="세제 · 洗涤剂">洗涤剂产生丰富泡沫，因此民间也会把它与财富、福气增长联系起来。</Note><Note title="现代选择" tone="amber">绿植、香氛、生活用品或礼券也很常见。礼物应结合主人的喜好、空间和实际需要。</Note><Note title="不是硬性规定" tone="rose">卫生纸和洗涤剂是知名的传统乔迁礼物，并非每位韩国人都要求收到，也不是唯一正确选择。</Note><Note title="拜访表达">초대해 줘서 고마워요.<br/>집이 정말 예쁘네요!</Note><Note title="送礼表达">집들이 선물이에요.<br/>마음에 들면 좋겠어요.</Note></div></>);
    if (number === "25") return section(number,"3. 60秒邀请与回复","两人轮流完成邀请、确认能力、承诺和分工。",<Mic2 aria-hidden="true" size={22}/>,<div className="mt-4 grid grid-cols-2 gap-3"><Note title="邀请人">이번 ______에 ______을/를 해요.<br/>우리 집에 올 수 있어요?<br/>______을 준비해 주세요.<br/>같이 ______면서 준비해요.</Note><Note title="受邀人">네, 갈 수 있어요.<br/>제가 ______할게요.<br/>______을 사러 갈게요.<br/>조금 ______ 갈게요.</Note><Note title="不能参加时" tone="amber">미안하지만 갈 수 없어요.<br/>______러 가야 돼요.<br/>다음에 제가 먼저 연락할게요.</Note><Note title="必须包含">四项核心语法各至少一次，并使用两个本课副词。</Note><Note title="确认信息">몇 시에 시작해요?<br/>어디로 가면 돼요?</Note><Note title="礼貌收尾">초대해 줘서 고마워요.<br/>그날 만나요!</Note></div>);
    if (number === "27") return section(number,"1. 阅读 · 집들이 초대와 답장","找出邀请时间、地点、活动、是否参加和来宾承诺。",<BookOpenCheck aria-hidden="true" size={22}/>,<><section className="mt-4 rounded-2xl border border-[var(--border)] bg-white p-5"><p className="text-[11px] font-bold text-[var(--destructive)]">초대</p><p className="mt-2 text-sm font-bold leading-7">이번 일요일 오후 다섯 시에 새 집에서 집들이를 해요. 같이 음식을 먹으면서 이야기하고 싶어요. 우리 집에 올 수 있어요?</p><p className="mt-3 text-[11px] font-bold text-[var(--destructive)]">답장</p><p className="mt-2 text-sm font-bold leading-7">초대해 줘서 고마워요. 일요일에 갈 수 있어요. 제가 음료수를 사러 갈게요. 준비를 도울 수 있으니까 조금 일찍 갈게요.</p></section><Cards items={[["什么时候？","일요일 오후 5시"],["在哪里？","새 집"],["边做什么边聊天？","음식을 먹으면서"],["能参加吗？","갈 수 있어요"],["承诺做什么？","음료수를 사러 갈게요"],["何时到？","조금 일찍"]]} show={Boolean(revealed.read)}/></>,reveal("read",true));
    if (number === "28") return section(number,"2. 写作 · 我的邀请函与回复","分别写6—8句邀请和回复，信息必须具体。",<Mail aria-hidden="true" size={22}/>,<><div className="mt-4 grid grid-cols-2 gap-3"><Note title="邀请函骨架">活动 → 日期 → 时间 → 地点 → 邀请对象 → 能否参加 → 一起做什么 → 联系方式</Note><Note title="回复骨架">感谢 → 能／不能参加 → 原因 → 到达时间 → 主动承诺 → 去买／带来物品</Note></div><section className="mt-4 rounded-2xl border border-dashed border-[var(--border)] bg-[var(--card)] p-5"><p className="text-[11px] font-bold text-[var(--destructive)]">原创示范</p><p className="mt-3 text-sm font-bold leading-7">토요일 저녁에 우리 집에서 작은 파티를 해요. 오후 여섯 시에 올 수 있어요? 친구들과 음악을 들으면서 저녁을 먹을 거예요. 올 수 있으면 금요일까지 연락해 주세요. 저는 갈 수 있어요. 조금 일찍 가서 준비를 도울게요. 그리고 케이크를 사러 갈게요.</p></section><div className="mt-3 grid grid-cols-2 gap-3 text-xs font-bold"><span className="rounded-xl bg-[var(--status-success-surface)] p-3">✓ 올 수 있어요?</span><span className="rounded-xl bg-[var(--status-warning-surface)] p-3">✓ 제가 준비할게요.</span><span className="rounded-xl bg-[var(--status-success-surface)] p-3">✓ 사러 갈게요.</span><span className="rounded-xl bg-[var(--status-warning-surface)] p-3">✓ 들으면서 먹어요.</span></div></>);
    ...
    if (number === "34") return section(number,"5. 1B结业口语验收 · 我的聚会","完成不少于12句的邀请、回复和现场分工。",<Mic2 aria-hidden="true" size={22}/>,<><section className="mt-4 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5"><div className="grid grid-cols-2 gap-3 text-xs">{["说明聚会类型","说出日期和时间","说明地点","询问能否参加","接受或婉拒","主动承担任务","说明行动目的","使用同时动作","使用两个副词","提到乔迁礼物"].map((task)=><label key={task} className="flex items-center gap-2 rounded-xl bg-white p-3 font-bold"><input type="checkbox" className="accent-[var(--destructive)]"/>{task}</label>)}</div></section><div className="mt-3 grid grid-cols-3 gap-2 text-center text-[11px] font-bold"><span className="rounded-xl bg-[var(--accent)] p-3">内容完整 40%</span><span className="rounded-xl bg-[var(--status-success-surface)] p-3">语法正确 40%</span><span className="rounded-xl bg-[var(--status-warning-surface)] p-3">表达自然 20%</span></div><button type="button" onClick={()=>speak("이번 토요일에 우리 집에서 집들이를 해요. 오후 여섯 시에 올 수 있어요? 네, 갈 수 있어요. 제가 음료수를 준비할게요. 케이크도 사러 갈게요. 조금 일찍 가서 음악을 들으면서 같이 준비해요.")} className="mt-auto flex items-center justify-center gap-2 rounded-2xl bg-[var(--destructive)] p-4 text-sm font-bold text-white"><Volume2 aria-hidden="true" size={16}/>播放原创示范</button></>);
    if (number === "35") return <div className="flex h-full flex-col justify-center"><div className="mx-auto w-full max-w-[440px] text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--card)] text-[var(--destructive)]"><Sparkles aria-hidden="true" size={27}/></span><h3 className="mt-3 text-4xl font-bold">우리 집에 올 수 있어요?</h3><p className="mt-3 text-lg font-bold">你已经完成韩国语1级课程</p><p className="mx-auto mt-3 max-w-[400px] text-sm leading-7 text-[var(--foreground-secondary)]">现在你能发出和回复邀请、询问能否参加、主动承诺分工、说明行动目的，并描述同时进行的动作。</p><div className="mt-4 grid grid-cols-2 gap-3 text-left">{[["01","能力条件","V-(으)ㄹ 수 있다/없다"],["02","承诺决定","V-(으)ㄹ게요"],["03","移动目的","V-(으)러 가다/오다"],["04","同时动作","V-(으)면서"]].map(([i,t,d])=><div key={i} className="rounded-xl border border-[var(--border)] bg-white px-4 py-3"><p className="text-[10px] font-bold text-[var(--destructive)]">{i}</p><p className="mt-1 text-xs font-bold">{t}</p><p className="mt-1 text-[10px] text-[var(--foreground-secondary)]">{d}</p></div>)}</div><button type="button" onClick={()=>flipBookRef.current?.pageFlip()?.flip(1)} className="mt-4 rounded-full bg-[var(--card)] px-4 py-3 text-xs font-bold text-[var(--destructive)]">返回目录</button></div></div>;
    return null;
  }
