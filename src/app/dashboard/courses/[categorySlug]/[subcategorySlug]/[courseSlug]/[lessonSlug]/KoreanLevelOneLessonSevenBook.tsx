...
export function KoreanLevelOneLessonSevenBook({
  ...
  pages.push(
    page("03", "今天的天气任务", "同一条天气信息，要学会用日常体交流，也要能改写成正式播报体。", <CloudSun aria-hidden="true" size={22} />,
      ...
      ].map(([tag, korean, chinese]) => <button key={tag} type="button" onClick={() => speak(korean)} className="rounded-2xl border border-[var(--border)] bg-white p-4 text-left"><b className="text-[10px] text-[var(--status-warning)]">{tag}</b><div className="mt-2 flex items-center justify-between gap-2"><p className="text-sm font-bold">{korean}</p><Volume2 aria-hidden="true" size={14} className="shrink-0 text-[var(--primary)]"/></div><p className={`mt-1 text-[11px] font-bold text-[var(--foreground-secondary)] ${revealed.chinese03 ? "opacity-100" : "opacity-0"}`}>{chinese}</p></button>)}</div>
  ...
  pages.push(
    ...
    <Page key="07-34" number="34"><div className="flex h-full flex-col justify-center"><div className="mx-auto w-full max-w-[440px] text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--status-success-surface)] text-[var(--status-success)]"><Sparkles aria-hidden="true" size={27}/></span><p className="mt-4 text-xs font-bold tracking-[0.18em] text-[var(--status-success)]">LESSON 07 · COMPLETE</p><h3 className="mt-3 text-4xl font-bold">날씨가 어때요?</h3><p className="mt-3 text-lg font-bold">你已经完成第七课</p><p className="mx-auto mt-3 max-w-[380px] text-sm leading-7 text-[var(--foreground-secondary)]">现在你能描述天气与季节、比较特征，并在日常体与正式播报体之间正确切换。</p><div className="mt-4 grid grid-cols-2 gap-3 text-left">{[["01","掌握变化","ㅂ 불규칙"],["02","表达反差","A/V-지만"],["03","正式播报","A/V-습니다/ㅂ니다"],["04","并列信息","A/V-고"]].map(([number,title,detail]) => <div key={number} className="rounded-xl border border-[var(--border)] bg-white px-4 py-3"><p className="text-[10px] font-bold text-[var(--status-success)]">{number}</p><p className="mt-1 text-xs font-bold">{title}</p><p className="mt-1 text-[10px] text-[var(--foreground-secondary)]">{detail}</p></div>)}</div><div className="mt-4 rounded-2xl border border-[var(--border)] bg-[var(--status-success-surface)] px-5 py-3.5 text-left"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold tracking-[0.14em] text-[var(--status-success)]">LESSON 7 TEST · 本课测试</p><p className="mt-1 text-xs font-bold text-[var(--foreground-secondary)]">检验天气词汇、语法变形与正式播报。</p></div><button type="button" onClick={() => window.location.assign("/dashboard/assignments/korean")} className="shrink-0 rounded-full bg-white px-3 py-2 text-[10px] font-bold text-[var(--status-success)] shadow-sm">前往测试专区</button></div></div><button type="button" onClick={() => flipBookRef.current?.pageFlip()?.flip(1)} className="mt-4 rounded-full bg-[var(--accent)] px-4 py-3 text-xs font-bold text-[var(--primary)]">返回目录</button></div></div></Page>,
  );
