...
function Note({ title, children, tone = "blue" }: { title: string; children: ReactNode; tone?: "blue" | "rose" | "green" | "amber" | "purple" }) {
  ...
  return <section className={`mt-4 rounded-2xl border p-4 ${tones[tone]}`}><p className="text-[11px] font-black">{title}</p><div className="mt-2 text-xs font-bold leading-6 text-[#45574f]">{children}</div></section>;
}
...
function Dialogue({ lines, speak, showChinese }: { lines: Line[]; speak: Speak; showChinese: boolean }) {
  return <div className="mt-4 grid grid-cols-2 gap-3">{lines.map((line, index) => (
    <div key={`${index}-${line.speaker}-${line.korean}`} className={`flex gap-2 rounded-xl p-3.5 ${index % 2 ? "bg-[#fff7ed]" : "bg-[#f4f8f6]"}`}>
      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white text-[9px] font-black">{line.speaker}</span>
      <div className="min-w-0 flex-1"><p className="text-[13px] font-black leading-6">{line.korean}</p><p className={`text-[10px] font-bold leading-5 text-[#71857b] ${showChinese ? "opacity-100" : "opacity-0"}`}>{line.chinese}</p></div>
...
export function KoreanLevelOneLessonElevenBook({ lesson, isFullscreen, initialPage = 0, onPageChange, speechRate = 0.78 }: { lesson: KoreanLevelOneLesson; isFullscreen: boolean; initialPage?: number; onPageChange?: (page: number) => void; speechRate?: number }) {
  ...
  function renderPage(number: string) {
    ...
    const pages: Record<string, ReactNode> = {
      ...
      "05": content("05", "1. 身体部位", "身体部位通常加 이/가，再接 아파요描述疼痛。", <Activity size={22} />, <><WordGrid words={bodyWords} speak={speak} showChinese={Boolean(revealed.chinese05)} /><Note title="句型词块">머리가 아파요／배가 아파요／목이 아파요。不要把身体部位误标成动作宾语。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese05)} onClick={() => toggle("chinese05")} />),
      "06": content("06", "2. 常见症状", "症状既有形容词，也有“名词 + 이/가 + 动词”的固定搭配。", <Thermometer size={22} />, <><WordGrid words={symptomWords} speak={speak} showChinese={Boolean(revealed.chinese06)} /><Note title="搭配优先">韩语说 열이 나요、기침을 해요、콧물이 나요；不要逐字翻译中文结构。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese06)} onClick={() => toggle("chinese06")} />),
      "07": content("07", "3. 医疗场所与治疗词块", "从挂号、问诊到取药，把名词和动作一起记。", <Cross size={22} />, <><WordGrid words={careWords} speak={speak} showChinese={Boolean(revealed.chinese07)} /><Note title="流程地图" tone="green">병원 → 진찰을 받다 → 처방전 → 약국 → 약을 먹다。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese07)} onClick={() => toggle("chinese07")} />),
      "08": content("08", "4. 禁止、必须与限定词块", "先熟悉完整医嘱，再在语法页拆解规则。", <ShieldAlert size={22} />, <><WordGrid words={adviceWords} speak={speak} showChinese={Boolean(revealed.chinese08)} /><Note title="建议组合" tone="rose">약을 먹어야 돼요. 그리고 오늘은 운동하지 마세요.</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese08)} onClick={() => toggle("chinese08")} />),
      "10": content("10", "1. ㅡ 탈락", "词干末尾ㅡ遇到아/어词尾时脱落，再看前一音节决定아或어。", <NotebookPen size={22} />, <>
      ...
      "11": content("11", "2. V-지 마세요", "词干直接加지 마세요，表达禁止、劝阻或礼貌提醒。", <ShieldAlert size={22} />, <>
      ...
      "12": content("12", "3. N만", "限定为“只、仅仅”；通常替换이/가或을/를，但排在에、에서后面。", <Pill size={22} />, <>
      ...
      "13": content("13", "4. V-아야／어야 되다", "表示义务、必要条件或必须采取的行动，口语常用돼요。", <ClipboardPlus size={22} />, <>
        <div className="mt-4 grid grid-cols-2 gap-3">{[
      ...
      "15": content("15", "1. 症状匹配实验", "根据身体部位选择自然的症状表达。", <HeartPulse size={22} />, <>
        ...
        ].map(([part, answer]) => <article key={part} className="grid grid-cols-[70px_1fr] items-center rounded-xl border border-[#ead8be] bg-white p-3"><b>{part}</b><span className={`text-sm font-black text-[#9b6b32] ${revealed.symptom ? "opacity-100" : "opacity-0"}`}>{answer}</span></article>)}</div>
      ...
      "16": content("16", "2. 医生建议生成器", "把同一症状转换成“必须做”和“不要做”两类建议。", <Stethoscope size={22} />, <>
        ...
        ].map(([symptom, must, ban]) => <article key={symptom} className="grid grid-cols-3 gap-3 rounded-xl border border-[#ead8be] bg-white p-3 text-xs font-black"><span>{symptom}</span><span className={`text-[#487a54] ${revealed.advice ? "opacity-100" : "opacity-0"}`}>{must}</span><span className={`text-[#a65b68] ${revealed.advice ? "opacity-100" : "opacity-0"}`}>{ban}</span></article>)}</div>
        <p className="mt-3 grid grid-cols-3 text-center text-[10px] font-black text-[#71857b]"><span>症状</span><span>必须做</span><span>不要做</span></p>
      </>, <KoreanEbookRevealButton shown={Boolean(revealed.advice)} onClick={() => toggle("advice")} answer />),
      "17": content("17", "3. 만 助词诊所", "判断만替换原助词，还是排在时间、地点助词后面。", <Pill size={22} />, <>
        <div className="mt-4 grid grid-cols-2 gap-3">{[
        ...
        ].map(([formula, answer]) => <article key={formula} className="rounded-2xl border border-[#ead8be] bg-white p-4"><b className="text-[11px] text-[#9b6b32]">{formula}</b><p className={`mt-2 text-sm font-black ${revealed.only ? "opacity-100" : "opacity-0"}`}>{answer}</p></article>)}</div>
      </>, <KoreanEbookRevealButton shown={Boolean(revealed.only)} onClick={() => toggle("only")} answer />),
      "23": content("23", "1. 听力 · 症状记录表", "第一遍抓身体部位，第二遍补症状和持续时间。", <Headphones size={22} />, <>
        <button type="button" onClick={() => speak("어제 저녁부터 목이 아프고 기침을 했어요. 오늘 아침에는 열도 났어요. 콧물이 나고 몸이 많이 피곤해요. 병원에 가서 진찰을 받아야 돼요.")} className="mt-5 flex items-center justify-center gap-2 rounded-2xl bg-[#3e7fa3] p-4 text-sm font-black text-white"><Volume2 size={17} />播放症状描述</button>
        <div className="mt-5 grid grid-cols-2 gap-3">{[
      ...
      "24": content("24", "2. 医嘱四步卡", "按“症状—必须—禁止—限定”顺序复述。", <ClipboardPlus size={22} />, <>
        <div className="mt-4 space-y-2.5">{[
          ["01 症状", "감기에 걸려서 열이 나요."], ["02 必须", "약을 먹고 푹 쉬어야 돼요."],
          ["03 禁止", "오늘은 운동하지 마세요."], ["04 限定", "따뜻한 물만 마시세요."],
          ["05 复查", "계속 아프면 다시 병원에 가세요."],
          ["06 确认", "약은 식사 후에만 드세요."],
        ].map(([step, sentence]) => <article key={step} className="grid grid-cols-[80px_1fr] items-center rounded-xl border border-[#cfddec] bg-white p-3"><b className="text-[#3e7fa3]">{step}</b><RuleSentence text={sentence} speak={speak}><span className="text-sm font-black">{sentence}</span></RuleSentence></article>)}</div>
        <Note title="安全表达">语言练习中只使用一般性建议；不要根据练习文本自行诊断或改变真实药物用法。</Note>
      </>),
      "25": content("25", "3. 90秒问诊角色扮演", "病人至少说明三项信息，医生至少给出三条建议。", <Mic2 size={22} />, <>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 text-[11px] font-black text-[#75559a]"><span className="rounded-xl bg-[#f7f2fb] px-3 py-3">场景 A · 感冒与发烧</span><span className="rounded-xl bg-[#f7f2fb] px-3 py-3">场景 B · 肚子疼</span></div>
        <p className="mt-4 rounded-xl bg-[#fff4f6] p-3 text-center text-[11px] font-bold text-[#8d5b64]">验收：症状3项＋持续时间＋必须1项＋禁止1项＋限定1项。</p>
      </>),
      "27": content("27", "1. 阅读 · 감기 안내 카드", "找出症状、服用说明、禁止事项和需要再次就医的条件。", <BookOpenCheck size={22} />, <>
        <section className="mt-4 rounded-2xl border border-[#cfe3d4] bg-white p-5"><p className="text-[11px] font-black text-[#347b69]">감기에 걸렸을 때</p><p className="mt-3 text-sm font-bold leading-7">감기에 걸리면 목이 아프고 열이 날 수 있어요. 물을 많이 마시고 푹 쉬어야 돼요. 약은 식사 후에만 드세요. 술을 마시지 말고 무리하게 운동하지 마세요. 열이 계속 나거나 숨쉬기가 힘들면 병원에 가세요.</p></section>
      ...
      "28": content("28", "2. 写作 · 症状与建议记录", "写 8—10 句虚构练习，不填写真实隐私或真实处方信息。", <NotebookPen size={22} />, <>
        ...
        <section className="mt-4 rounded-2xl border border-dashed border-[#9fc8b9] bg-[#fbfdfa] p-5"><p className="text-[11px] font-black text-[#347b69]">原创示范</p><p className="mt-3 text-sm font-bold leading-7">어제부터 머리가 아프고 열이 나요. 기침도 조금 해요. 오늘은 집에서만 쉬어야 돼요. 따뜻한 물을 많이 마시고 약을 먹어야 돼요. 찬 음료를 마시지 마세요. 무리하게 운동하지 마세요. 계속 아프면 병원에 가야 돼요.</p></section>
        <div className="mt-4 grid grid-cols-2 gap-2 text-[11px] font-black text-[#347b69]">{["✓ 开始时间与程度","✓ 三项不同症状","✓ 必须／禁止／限定","✓ 再次就医条件"].map((item) => <span key={item} className="rounded-xl bg-[#f2f8f3] px-3 py-2">{item}</span>)}</div>
      </>),
      "30": content("30", "1. 身体与症状闪测", "看到中文后两秒内说出韩语。", <HeartPulse size={22} />, <div className="mt-4 grid grid-cols-3 gap-2.5">{[
        ["头", "머리"], ["喉咙", "목"], ["肚子", "배"], ["腿", "다리"],
      ...
      "31": content("31", "2. ㅡ 脱落检测", "去掉ㅡ后，根据前一音节元音选择아요或어요。", <NotebookPen size={22} />, <>
        <div className="mt-4 grid grid-cols-2 gap-2.5">{[
        ...
        ].map(([question, answer]) => <article key={question} className="rounded-xl border border-[#cfe3d4] bg-white p-3"><b>{question}</b><p className={`mt-2 text-sm font-black text-[#487a54] ${revealed.eu ? "opacity-100" : "opacity-0"}`}>{answer}</p></article>)}</div>
      </>, <KoreanEbookRevealButton shown={Boolean(revealed.eu)} onClick={() => toggle("eu")} answer />),
      "32": content("32", "3. 综合语法检测", "完成禁止、限定和义务表达。", <CheckCircle2 size={22} />, <div className="mt-4 space-y-2">{[
      ...
      "33": content("33", "4. 口语验收 · 医生与病人", "不看稿完成至少十句问诊交流，交换角色后再做一次。", <Mic2 size={22} />, <>
        <section className="mt-4 rounded-2xl border border-[#cfe3d4] bg-[#f2f8f3] p-5"><p className="text-xs font-black text-[#487a54]">八项必达信息</p><div className="mt-4 grid grid-cols-2 gap-3 text-xs">{["询问哪里不舒服","说明开始时间","说出三项症状","使用一个ㅡ脱落词","给出两项必须事项","给出两项禁止事项","使用N만一次","说明再次就医条件"].map((task) => <label key={task} className="flex items-center gap-2 rounded-xl bg-white p-3 font-bold"><input type="checkbox" className="accent-[#487a54]" />{task}</label>)}</div></section>
        <div className="mt-4 grid grid-cols-3 gap-3 text-center text-[11px] font-black"><span className="rounded-xl bg-[#f1f6fb] px-3 py-2 text-[#3d6f9f]">症状完整 40%</span><span className="rounded-xl bg-[#f2f8f3] px-3 py-2 text-[#487a54]">语法正确 40%</span><span className="rounded-xl bg-[#fff8ed] px-3 py-2 text-[#9a6b2f]">表达自然 20%</span></div>
        <button type="button" onClick={() => speak("어디가 아파요? 어제부터 머리가 아프고 열이 나요. 기침도 해요. 감기에 걸렸어요. 약을 먹어야 돼요. 따뜻한 물을 많이 마셔야 돼요. 오늘은 운동하지 마세요. 술도 마시지 마세요. 집에서만 푹 쉬세요. 계속 아프면 다시 병원에 오세요.")} className="mt-auto flex items-center justify-center gap-2 rounded-2xl bg-[#487a54] p-4 text-sm font-black text-white"><Volume2 size={16} />播放十句示范</button>
      </>),
      "34": <div className="flex h-full flex-col justify-center"><div className="mx-auto w-full max-w-[440px] text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#e8f4eb] text-[#487a54]"><Sparkles size={27} /></span><p className="mt-4 text-xs font-black tracking-[0.18em] text-[#487a54]">LESSON 11 · COMPLETE</p><h2 className="mt-3 text-4xl font-black">감기에 걸렸어요.</h2><p className="mt-3 text-lg font-black">你已经完成第十一课</p><p className="mx-auto mt-3 max-w-[390px] text-sm leading-7 text-[#60736a]">现在你能描述常见症状，理解初级医疗建议，并正确表达“不要、只、必须”。</p><div className="mt-4 grid grid-cols-2 gap-3 text-left">{[
        ["01", "掌握变化", "ㅡ 탈락"], ["02", "表达禁止", "V-지 마세요"],
        ["03", "限定范围", "N만"], ["04", "说明义务", "V-아야/어야 돼요"],
      ].map(([index, title, detail]) => <div key={index} className="rounded-xl border border-[#dce8e1] bg-white px-4 py-3"><p className="text-[10px] font-black text-[#487a54]">{index}</p><p className="mt-1 text-xs font-black">{title}</p><p className="mt-1 text-[10px] text-[#71857b]">{detail}</p></div>)}</div><div className="mt-4 rounded-2xl border border-[#cfe3d4] bg-[#f2f8f3] px-5 py-3.5 text-left"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black tracking-[0.14em] text-[#487a54]">LESSON 11 TEST · 本课测试</p><p className="mt-1 text-xs font-bold text-[#52685e]">检验身体、症状、禁止、限定和义务表达。</p></div><button type="button" onClick={() => window.location.assign("/dashboard/assignments/korean")} className="shrink-0 rounded-full bg-white px-3 py-2 text-[10px] font-black text-[#487a54] shadow-sm">前往测试专区</button></div></div><button type="button" onClick={() => flipBookRef.current?.pageFlip()?.flip(1)} className="mt-4 rounded-full bg-[#eaf2fb] px-4 py-3 text-xs font-black text-[#3d6f9f]">返回目录</button></div></div>,
    };
    return pages[number];
  }
