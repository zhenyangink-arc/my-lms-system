...
export function KoreanLevelOneLessonElevenBook({ lesson, isFullscreen, initialPage = 0, onPageChange, speechRate = 0.78 }: { lesson: KoreanLevelOneLesson; isFullscreen: boolean; initialPage?: number; onPageChange?: (page: number) => void; speechRate?: number }) {
  ...
  function renderPage(number: string) {
    ...
    const pages: Record<string, ReactNode> = {
      ...
      "34": <div className="flex h-full flex-col justify-center"><div className="mx-auto w-full max-w-[440px] text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--status-success-surface)] text-[var(--status-success)]"><Sparkles aria-hidden="true" size={27} /></span><p className="mt-4 text-xs font-bold tracking-[0.18em] text-[var(--status-success)]">LESSON 11 · COMPLETE</p><h3 className="mt-3 text-4xl font-bold">감기에 걸렸어요.</h3><p className="mt-3 text-lg font-bold">你已经完成第十一课</p><p className="mx-auto mt-3 max-w-[390px] text-sm leading-7 text-[var(--foreground-secondary)]">现在你能描述常见症状，理解初级医疗建议，并正确表达“不要、只、必须”。</p><div className="mt-4 grid grid-cols-2 gap-3 text-left">{[
        ["01", "掌握变化", "ㅡ 탈락"], ["02", "表达禁止", "V-지 마세요"],
        ["03", "限定范围", "N만"], ["04", "说明义务", "V-아야/어야 돼요"],
      ].map(([index, title, detail]) => <div key={index} className="rounded-xl border border-[var(--border)] bg-white px-4 py-3"><p className="text-[10px] font-bold text-[var(--status-success)]">{index}</p><p className="mt-1 text-xs font-bold">{title}</p><p className="mt-1 text-[10px] text-[var(--foreground-secondary)]">{detail}</p></div>)}</div><div className="mt-4 rounded-2xl border border-[var(--border)] bg-[var(--status-success-surface)] px-5 py-3.5 text-left"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold tracking-[0.14em] text-[var(--status-success)]">LESSON 11 TEST · 本课测试</p><p className="mt-1 text-xs font-bold text-[var(--foreground-secondary)]">检验身体、症状、禁止、限定和义务表达。</p></div><button type="button" onClick={() => window.location.assign("/dashboard/assignments/korean")} className="shrink-0 rounded-full bg-white px-3 py-2 text-[10px] font-bold text-[var(--status-success)] shadow-sm">前往测试专区</button></div></div><button type="button" onClick={() => flipBookRef.current?.pageFlip()?.flip(1)} className="mt-4 rounded-full bg-[var(--accent)] px-4 py-3 text-xs font-bold text-[var(--primary)]">返回目录</button></div></div>,
    };
    return pages[number];
  }
