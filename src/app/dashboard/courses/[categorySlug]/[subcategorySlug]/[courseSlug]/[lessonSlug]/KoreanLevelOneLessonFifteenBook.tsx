...
function Note({ title, children }: { title: string; children: ReactNode }) {
  return <section className="rounded-2xl border border-[#cfe3d4] bg-[#f2f8f3] p-5"><p className="text-xs font-black text-[#347b69]">{title}</p><div className="mt-2 text-[13px] font-bold leading-7 text-[#45574f]">{children}</div></section>;
}
...
export function KoreanLevelOneLessonFifteenBook({ lesson, isFullscreen, initialPage = 0, onPageChange, speechRate = 0.78 }: { lesson: KoreanLevelOneLesson; isFullscreen: boolean; initialPage?: number; onPageChange?: (page: number) => void; speechRate?: number }) {
  ...
  function renderPage(number: string): ReactNode {
    ...
    if (number === "03") return section(number, "本课学习路线", "从“想去哪儿”走到一份完整、自然的旅行计划。", <Compass />, <div className="mt-5 grid grid-cols-2 gap-3">{[["01","选地点","여행지"],["02","设条件","-(으)면"],["03","说愿望","-고 싶다"],["04","谈他人","-고 싶어 하다"],["05","做推荐","V-는 N"],["06","安排活动","구경하다·찍다"],["07","准备交通","표·비행기·기차"],["08","完成计划","什么时候、和谁、做什么"]].map(([n,t,d])=><Note key={n} title={`${n} · ${t}`}>{d}</Note>)}</div>);
    ...
    if (number === "23") return section(number,"1. 听力 · 两个人的旅行计划","播放对话后记录人物、条件、目的地、活动和交通方式。",<Headphones size={22}/>,<><button type="button" onClick={()=>speak("수진 씨는 시간이 있으면 제주도에 가고 싶어 해요. 제주도에서 바다를 구경하고 사진을 찍고 싶어 해요. 민수 씨는 부산에 가고 싶어 해요. 표가 싸면 기차를 타고 갈 거예요.")} className="mt-5 flex items-center justify-center gap-2 rounded-2xl bg-[#a65b68] p-4 text-sm font-black text-white"><Volume2 size={17}/>播放旅行计划</button><Cards items={[["수진的条件","시간이 있으면"],["수진的目的地","제주도"],["수진的活动","바다 구경·사진 찍기"],["민수的目的地","부산"],["민수的条件","표가 싸면"],["交通方式","기차"],["谁想拍照？","수진 씨"],["谁想去釜山？","민수 씨"]]} show={Boolean(revealed.listen)}/></>,reveal("listen",true));
    if (number === "24") return section(number,"2. 文化漫步 · 韩国代表旅行地","了解旅行地的特色，再用自己的理由作选择。",<Map size={22}/>,<><div className="mt-4 grid grid-cols-2 gap-3"><Note title="제주도 · 济州岛">韩国最大的岛，以火山地貌、海景、柑橘和地方饮食闻名。</Note><Note title="부산 · 釜山">港口城市，海云台等海滨区域是代表性目的地。</Note><Note title="경주 · 庆州">新罗历史文化遗产集中，常被形容为“没有屋顶的博物馆”。</Note><Note title="旅行选择没有标准答案">传统蜜月地包括济州岛；现代旅行者也会按照预算、假期和个人喜好选择国内外目的地。</Note><Note title="推荐表达">바다를 좋아하면 부산을 추천해요.</Note><Note title="历史旅行表达">역사에 관심이 있으면 경주에 가 보세요.</Note></div></>);
    if (number === "25") return section(number,"3. 60秒旅行愿望发表","不看稿介绍自己或朋友的旅行计划。",<Mic2 size={22}/>,<div className="mt-4 grid grid-cols-2 gap-3"><Note title="说自己的愿望">시간이 있으면 ______에 가고 싶어요.<br/>거기에서 ______고 싶어요.<br/>제가 좋아하는 곳은 ______예요.</Note><Note title="说朋友的愿望">제 친구는 ______에 가고 싶어 해요.<br/>______면 같이 갈 거예요.<br/>우리는 ______을/를 준비할 거예요.</Note><Note title="必须包含">条件句1次、V-는 N 1次、第一人称愿望1次、第三人称愿望1次。</Note><Note title="表达顺序">时间 → 条件 → 地点 → 活动 → 同伴 → 交通 → 理由。</Note><Note title="追加问题">언제 출발하고 싶어요?<br/>며칠 동안 머물고 싶어요?</Note><Note title="自然收尾">정말 기대돼요.<br/>즐거운 여행이 되면 좋겠어요.</Note></div>);
    if (number === "27") return section(number,"1. 阅读 · 제주도 여행 계획","找出旅行时间、条件、目的地、活动和不同人物的愿望。",<BookOpenCheck size={22}/>,<><section className="mt-4 rounded-2xl border border-[#ead0d6] bg-white p-5"><p className="text-sm font-bold leading-7">이번 여름에 시간이 있으면 친구들과 제주도에 가고 싶어요. 제가 보고 싶은 곳은 성산일출봉이에요. 저는 아름다운 경치를 보고 사진을 찍고 싶어요. 민수 씨는 바다에서 수영하고 싶어 해요. 지수 씨는 조용한 카페에서 쉬고 싶어 해요. 비행기 표가 싸면 다음 주에 바로 예약할 거예요.</p></section><Cards items={[["什么时候？","이번 여름"],["条件1","시간이 있으면"],["“我”想看的地方","성산일출봉"],["民秀的愿望","바다에서 수영하기"],["智秀的愿望","카페에서 쉬기"],["条件2","표가 싸면 예약하기"]]} show={Boolean(revealed.read)}/></>,reveal("read",true));
    if (number === "28") return section(number,"2. 写作 · 我的旅行愿望","写8—10句原创旅行计划，说明条件、目的地、活动与同行人的愿望。",<NotebookPen size={22}/>,<><div className="mt-4 grid grid-cols-2 gap-3"><Note title="内容骨架">时间 → 条件 → 目的地 → 喜欢的景点 → 自己想做的事 → 同伴想做的事 → 交通 → 准备</Note><Note title="语法清单">-(으)면<br/>V-는 N<br/>-고 싶어요<br/>-고 싶어 해요</Note></div><section className="mt-4 rounded-2xl border border-dashed border-[#d9aeb8] bg-[#fff9fa] p-5"><p className="text-[11px] font-black text-[#a65b68]">原创示范</p><p className="mt-3 text-sm font-bold leading-7">가을에 시간이 있으면 경주에 가고 싶어요. 제가 보고 싶은 곳은 불국사예요. 오래된 건물을 구경하고 사진을 찍고 싶어요. 제 친구는 맛있는 음식을 먹고 싶어 해요. 날씨가 좋으면 자전거도 타고 싶어요. 우리는 기차표와 숙소를 미리 예약할 거예요. 여행 전에 지도를 보고 필요한 것을 준비할 거예요.</p></section><div className="mt-3 grid grid-cols-2 gap-3 text-xs font-black"><span className="rounded-xl bg-[#f2f8f3] p-3">✓ 시간이 있으면</span><span className="rounded-xl bg-[#fff8ed] p-3">✓ 제가 보고 싶은 곳</span><span className="rounded-xl bg-[#f2f8f3] p-3">✓ 가고 싶어요</span><span className="rounded-xl bg-[#fff8ed] p-3">✓ 친구는 쉬고 싶어 해요</span></div></>);
    ...
    if (number === "34") return section(number,"5. 口语验收 · 我的理想旅行","根据检查表完成不少于10句的原创发表。",<Mic2 size={22}/>,<><section className="mt-4 rounded-2xl border border-[#ead0d6] bg-[#fff4f6] p-5"><div className="grid grid-cols-2 gap-3 text-xs">{["说明旅行时间","提出一个条件","说出目的地","使用V-는 N","表达自己的愿望","描述朋友的愿望","说明交通方式","说出两项活动","解释选择理由","以准备计划结束"].map((task)=><label key={task} className="flex items-center gap-2 rounded-xl bg-white p-3 font-bold"><input type="checkbox" className="accent-[#a65b68]"/>{task}</label>)}</div></section><div className="mt-3 grid grid-cols-3 gap-2 text-center text-[11px] font-black"><span className="rounded-xl bg-[#eef6fb] p-3">内容完整 40%</span><span className="rounded-xl bg-[#f2f8f3] p-3">语法正确 40%</span><span className="rounded-xl bg-[#fff8ed] p-3">表达自然 20%</span></div><button type="button" onClick={()=>speak("겨울에 시간이 있으면 부산에 가고 싶어요. 제가 좋아하는 여행지는 바다가 있는 곳이에요. 저는 해운대를 구경하고 사진을 찍고 싶어요. 제 친구는 부산 음식을 맛보고 싶어 해요. 표가 싸면 기차를 타고 갈 거예요. 우리는 여행 전에 숙소를 예약하고 지도를 준비할 거예요.")} className="mt-auto flex items-center justify-center gap-2 rounded-2xl bg-[#a65b68] p-4 text-sm font-black text-white"><Volume2 size={16}/>播放原创示范</button></>);
    if (number === "35") return <div className="flex h-full flex-col justify-center"><div className="mx-auto w-full max-w-[440px] text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#fff0f3] text-[#a65b68]"><Sparkles size={27}/></span><p className="mt-4 text-xs font-black tracking-[0.18em] text-[#a65b68]">LESSON 15 · COMPLETE</p><h2 className="mt-3 text-4xl font-black">여행을 가고 싶어요</h2><p className="mt-3 text-lg font-black">你已经完成第十五课</p><p className="mx-auto mt-3 max-w-[390px] text-sm leading-7 text-[#60736a]">现在你能设置旅行条件、用动词修饰名词，并正确区分自己、对方与第三人的旅行愿望。</p><div className="mt-4 grid grid-cols-2 gap-3 text-left">{[["01","条件假设","A/V-(으)면"],["02","动词定语","V-는 N"],["03","自己的愿望","V-고 싶다"],["04","他人的愿望","V-고 싶어 하다"]].map(([i,t,d])=><div key={i} className="rounded-xl border border-[#ead0d6] bg-white px-4 py-3"><p className="text-[10px] font-black text-[#a65b68]">{i}</p><p className="mt-1 text-xs font-black">{t}</p><p className="mt-1 text-[10px] text-[#71857b]">{d}</p></div>)}</div><button type="button" onClick={()=>flipBookRef.current?.pageFlip()?.flip(1)} className="mt-4 rounded-full bg-[#fff0f3] px-4 py-3 text-xs font-black text-[#a65b68]">返回目录</button></div></div>;
    return null;
  }
