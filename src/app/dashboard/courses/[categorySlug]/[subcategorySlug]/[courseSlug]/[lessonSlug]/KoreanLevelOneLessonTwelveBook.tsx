...
const TEMPLATE = buildKoreanEbookSectionMap([
  ...
  { step: "STEP 03", label: "语法讲解", dividerPage: 9, contentPages: [10, 11, 12, 13, 14] },
  { step: "STEP 04", label: "句型操练", dividerPage: 15, contentPages: [16, 17, 18] },
  { step: "STEP 05", label: "实战对话", dividerPage: 19, contentPages: [20, 21, 22] },
  { step: "STEP 06", label: "听说任务", dividerPage: 23, contentPages: [24, 25, 26] },
  { step: "STEP 07", label: "读写拓展", dividerPage: 27, contentPages: [28, 29] },
  { step: "STEP 08", label: "自测与复盘", dividerPage: 30, contentPages: [31, 32, 33, 34, 35] },
]);
...
function Note({ title, children, tone = "blue" }: { title: string; children: ReactNode; tone?: "blue" | "rose" | "green" | "amber" }) {
  const colors = { blue: "border-[#cfddec] bg-[#f1f6fb]", rose: "border-[#ead0d6] bg-[#fff4f6]", green: "border-[#cfe3d4] bg-[#f2f8f3]", amber: "border-[#ead8be] bg-[#fff8ed]" };
  return <section className={`mt-4 rounded-2xl border p-4 ${colors[tone]}`}><p className="text-[11px] font-black">{title}</p><div className="mt-2 text-xs font-bold leading-6 text-[#45574f]">{children}</div></section>;
}
...
function CardGrid({ cards, speak, showChinese, columns = 2 }: { cards: Card[]; speak: Speak; showChinese: boolean; columns?: 2 | 3 }) {
  return <div className={`mt-4 grid gap-3 ${columns === 3 ? "grid-cols-3" : "grid-cols-2"}`}>{cards.map((card) => <article key={`${card.label}-${card.korean}`} className="min-h-[82px] rounded-2xl border border-[#ddd0ee] bg-white p-5"><b className="text-[10px] text-[#75559a]">{card.label}</b><div className="mt-2 text-sm font-black"><SpeakLine text={card.korean} speak={speak} /></div><p className={`mt-1 text-[10px] text-[#71857b] ${showChinese ? "opacity-100" : "opacity-0"}`}>{card.chinese}</p></article>)}</div>;
}
...
function Dialogue({ lines, speak, showChinese }: { lines: Line[]; speak: Speak; showChinese: boolean }) {
  return <div className="mt-4 grid grid-cols-2 gap-3">{lines.map((line, index) => <div key={`${index}-${line.korean}`} className={`flex gap-2 rounded-xl p-3.5 ${index % 2 ? "bg-[#fff7ed]" : "bg-[#f4f8f6]"}`}><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white text-[9px] font-black">{line.speaker}</span><div className="min-w-0 flex-1"><p className="text-[13px] font-black leading-6">{line.korean}</p><p className={`text-[10px] font-bold leading-5 text-[#71857b] ${showChinese ? "opacity-100" : "opacity-0"}`}>{line.chinese}</p></div><KoreanEbookSpeakButton text={line.korean} onSpeak={speak} compact /></div>)}</div>;
}
...
const dividers: Record<string, { step: string; title: string; goal: string; icon: ReactNode }> = {
  "02": { step: "STEP 01", title: "课前导航", goal: "完成“接听—确认身份—询问状态—说明原因—约定回电”的电话交际链。", icon: <PhoneCall size={24} /> },
  "04": { step: "STEP 02", title: "核心词汇", goal: "掌握电话专用词、状态动词、电话固定句和五项语法形态。", icon: <Phone size={24} /> },
  "09": { step: "STEP 03", title: "语法讲解", goal: "五个语法各占一页：确认、名词确认、进行、客观不能和原因。", icon: <NotebookPen size={24} /> },
  "15": { step: "STEP 04", title: "句型操练", goal: "在身份确认、当前状态和电话原因之间建立快速反应。", icon: <PhoneIncoming size={24} /> },
  "19": { step: "STEP 05", title: "实战对话", goal: "完成三组不少于八句的真实电话交流。", icon: <MessageCircle size={24} /> },
  "23": { step: "STEP 06", title: "听说任务", goal: "从通话中提取人物、状态、原因和后续安排。", icon: <Headphones size={24} /> },
  "27": { step: "STEP 07", title: "读写拓展", goal: "读懂电话留言并写出清楚、礼貌、可执行的回电记录。", icon: <BookOpenCheck size={24} /> },
  "30": { step: "STEP 08", title: "自测与复盘", goal: "检查电话词汇、五项语法和完整通话能力。", icon: <CheckCircle2 size={24} /> },
};
...
export function KoreanLevelOneLessonTwelveBook({ lesson, isFullscreen, initialPage = 0, onPageChange, speechRate = 0.78 }: { lesson: KoreanLevelOneLesson; isFullscreen: boolean; initialPage?: number; onPageChange?: (page: number) => void; speechRate?: number }) {
  ...
  function renderPage(number: string) {
    ...
    if (dialogues[number]) { const d = dialogues[number]; return content(number, d.title, d.description, <PhoneCall size={22} />, <><Dialogue lines={d.lines} speak={speak} showChinese={Boolean(revealed[`chinese${number}`])} /><Note title="角色交换" tone="rose">替换姓名、当前状态和原因，再完成一轮不少于八句的电话对话。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed[`chinese${number}`])} onClick={() => toggle(`chinese${number}`)} />); }
    const pages: Record<string, ReactNode> = {
      "03": content("03", "一通电话的五步结构", "接听、确认、询问、说明原因、约定后续，每一步都有固定语言任务。", <Phone size={22} />, <><CardGrid cards={[
      ...
      "05": content("05", "1. 电话交际词汇", "这些表达属于电话专用语境，不能随意替换成面对面问候。", <Phone size={22} />, <><WordGrid words={phoneWords} speak={speak} showChinese={Boolean(revealed.chinese05)} /><Note title="电话动作顺序" tone="green"><SpeakLine text="전화를 걸고, 받고, 마지막에 끊어요." speak={speak} /><span className={revealed.chinese05 ? "" : "opacity-0"}>拨打电话、接听，最后挂断。</span></Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese05)} onClick={() => toggle("chinese05")} />),
      "06": content("06", "2. 状态与日常动词", "进行时和原因句需要大量状态、工作与生活动词。", <PhoneIncoming size={22} />, <><WordGrid words={stateWords} speak={speak} showChinese={Boolean(revealed.chinese06)} /><Note title="하다动词中的못">운전하다 → 운전 못 해요；수영하다 → 수영 못 해요。못通常插在名词与하다之间。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese06)} onClick={() => toggle("chinese06")} />),
      "07": content("07", "3. 电话固定表达", "把开场、线路问题、留言和结束语作为完整词块记忆。", <PhoneCall size={22} />, <><WordGrid words={callPhrases} speak={speak} showChinese={Boolean(revealed.chinese07)} /><Note title="线路不清楚时" tone="rose"><SpeakLine text="잘 안 들려요. 다시 말해 주세요." speak={speak} /><span className={revealed.chinese07 ? "" : "opacity-0"}>听不清楚，请再说一遍。</span></Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese07)} onClick={() => toggle("chinese07")} />),
      "08": content("08", "4. 本课核心形态", "先熟悉语音和整体形态，再进入五页规则拆解。", <NotebookPen size={22} />, <><WordGrid words={formWords} speak={speak} showChinese={Boolean(revealed.chinese08)} /><Note title="先判断交际功能" tone="amber">确认事实用-지요?；描述当下动作用-고 있어요；客观做不到用못；说明原因用-아서/어서。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese08)} onClick={() => toggle("chinese08")} />),
      "10": content("10", "1. A/V-지요?", "说话人已有判断，通过-지요?寻求确认、认同或共同记忆。", <NotebookPen size={22} />, <><CardGrid cards={[
      ...
      "11": content("11", "2. N-(이)지요?", "名词确认句在电话中用于确认身份、号码、地点与时间。", <PhoneIncoming size={22} />, <><CardGrid cards={[
      ...
      "12": content("12", "3. V-고 있어요", "表示动作正在进行或某状态正在持续，词干直接加-고 있어요。", <PhoneCall size={22} />, <><CardGrid cards={[
      ...
      "13": content("13", "4. 못 + V", "表示能力不足或客观条件不允许，与主观选择的안不同。", <PhoneMissed size={22} />, <><CardGrid cards={[
      ...
      "14": content("14", "5. A/V-아서/어서 · 原因", "前项说明原因，后项说明由此产生的结果；变形同第10课。", <NotebookPen size={22} />, <><CardGrid cards={[
      ...
      "16": content("16", "1. 电话确认实验", "根据名词有无收音以及句子类型选择지요或이지요。", <PhoneIncoming size={22} />, <Exercise items={[
      ...
      "17": content("17", "2. 正在做什么？", "把动词转换为进行时，再用电话问答完成信息交换。", <PhoneCall size={22} />, <Exercise items={[
      ...
      "18": content("18", "3. 안、못与原因诊所", "先判断是主观不做还是客观不能，再补充自然原因。", <PhoneMissed size={22} />, <Exercise items={[
      ...
      "24": content("24", "1. 听力 · 通话信息表", "抓住打电话的人、当前状态、不能通话的原因和回电时间。", <Headphones size={22} />, <><button type="button" onClick={() => speak("여보세요, 지훈 씨지요? 지금 통화할 수 있어요? 미안해요. 지금 회의하고 있어서 오래 통화 못 해요. 회의가 다섯 시에 끝나요. 제가 다섯 시 반에 다시 전화할게요.")} className="mt-5 flex items-center justify-center gap-2 rounded-2xl bg-[#3e7fa3] p-4 text-sm font-black text-white"><Volume2 size={17} />播放电话录音</button><Exercise items={[["接听人", "지훈 씨"], ["当前状态", "회의하고 있어요"], ["不能做什么", "오래 통화 못 해요"], ["原因", "회의"], ["结束时间", "다섯 시"], ["回电时间", "다섯 시 반"], ["使用的确认句", "지훈 씨지요?"], ["谁会再次打电话？", "말하는 사람"]]} shown={Boolean(revealed.listening)} /></>, <KoreanEbookRevealButton shown={Boolean(revealed.listening)} onClick={() => toggle("listening")} answer />),
      "25": content("25", "2. 电话留言四要素", "一条可执行留言必须让对方知道谁、为什么、何时和如何回复。", <PhoneCall size={22} />, <><CardGrid cards={[
      ...
      "26": content("26", "3. 60秒电话挑战", "不看稿完成确认、状态、原因和后续安排。", <Mic2 size={22} />, <><div className="mt-4 grid grid-cols-2 gap-3"><Note title="来电者">여보세요, ______ 씨지요?<br/>지금 뭐 하고 있어요?<br/>잠깐 통화할 수 있어요?<br/>그럼 언제 다시 전화할까요?</Note><Note title="接听者" tone="green">네, 그런데요.<br/>지금 ______고 있어요.<br/>______해서 통화를 못 해요.<br/>______에 다시 전화해 주세요.</Note><Note title="听不清时" tone="rose">잘 안 들려요.<br/>다시 말해 주세요.</Note><Note title="结束通话" tone="amber">네, 알겠습니다.<br/>나중에 다시 전화할게요.</Note></div><div className="mt-4 grid grid-cols-2 gap-2 text-[11px] font-black text-[#75559a]">{["场景 A · 正在开会","场景 B · 正在开车","场景 C · 身体不舒服","场景 D · 信号不好"].map((item) => <span key={item} className="rounded-xl bg-[#f7f2fb] px-3 py-3">{item}</span>)}</div><p className="mt-4 text-center text-[11px] font-bold text-[#71857b]">至少使用：-(이)지요?、-고 있어요、못、-아서/어서各一次。</p></>),
      "28": content("28", "1. 阅读 · 부재중 전화 메모", "提取留言中的人物、原因、时间与请求。", <BookOpenCheck size={22} />, <><section className="mt-4 rounded-2xl border border-[#cfe3d4] bg-white p-5"><p className="text-[11px] font-black text-[#347b69]">전화 메모</p><p className="mt-3 text-sm font-bold leading-7">지훈 씨, 민수 씨가 오후 두 시에 전화했어요. 내일 회의 장소가 회사 회의실이지요? 회의 장소 때문에 전화했어요. 민수 씨는 지금 운전하고 있어서 오래 통화 못 해요. 오후 여섯 시 이후에 민수 씨에게 다시 전화해 주세요.</p></section><Exercise items={[["谁来过电话？", "민수 씨"], ["几点来电？", "오후 두 시"], ["因为什么来电？", "회의 장소"], ["为什么不能长时间通话？", "운전하고 있어서"], ["什么时候回电？", "오후 여섯 시 이후"], ["使用了哪个确认句？", "회사 회의실이지요?"], ["留言留给谁？", "지훈 씨"], ["要给谁回电？", "민수 씨"]]} shown={Boolean(revealed.reading)} /></>, <KoreanEbookRevealButton shown={Boolean(revealed.reading)} onClick={() => toggle("reading")} answer />),
      "29": content("29", "2. 写作 · 我的电话留言", "写7—9句原创留言，信息必须可以执行。", <NotebookPen size={22} />, <><div className="mt-4 grid grid-cols-2 gap-3"><Note title="内容骨架" tone="green">接听人 → 来电者 → 确认对象 → 当前状态 → 原因 → 无法做 → 回电时间 → 请求</Note><Note title="语法清单" tone="amber">-(이)지요?一次<br/>-고 있어요一次<br/>못一次<br/>-아서/어서一次</Note></div><section className="mt-4 rounded-2xl border border-dashed border-[#9fc8b9] bg-[#fbfdfa] p-5"><p className="text-[11px] font-black text-[#347b69]">原创示范</p><p className="mt-3 text-sm font-bold leading-7">수진 씨, 민수 씨가 전화했어요. 내일 약속이 세 시지요? 약속 시간 때문에 전화했어요. 민수 씨는 지금 회의하고 있어서 오래 통화 못 해요. 회의가 끝나고 다시 연락할 거예요. 다섯 시 이후에 전화해 주세요.</p></section><div className="mt-4 grid grid-cols-2 gap-2 text-[11px] font-black text-[#347b69]">{["✓ 来电者姓名","✓ 来电原因","✓ 可回电时间","✓ 明确后续请求"].map((item) => <span key={item} className="rounded-xl bg-[#f2f8f3] px-3 py-2">{item}</span>)}</div></>),
      "31": content("31", "1. 电话词汇闪测", "看到中文后两秒内说出韩语。", <Phone size={22} />, <Exercise items={[
      ...
      "32": content("32", "2. 确认与进行时检测", "完成-(이)지요?和-고 있어요。", <NotebookPen size={22} />, <Exercise items={[
      ...
      "33": content("33", "3. 못与原因检测", "区分主观否定与客观不能，并遵守原因句禁区。", <CheckCircle2 size={22} />, <Exercise items={[
      ...
      "34": content("34", "4. 口语验收 · 十句完整通话", "交换角色完成通话，并处理一次无法长时间通话的情况。", <Mic2 size={22} />, <><section className="mt-4 rounded-2xl border border-[#cfe3d4] bg-[#f2f8f3] p-5"><p className="text-xs font-black text-[#487a54]">八项必达信息</p><div className="mt-4 grid grid-cols-2 gap-3 text-xs">{["用여보세요接听","确认身份或号码","询问当前状态","使用进行时","说明客观不能","给出原因","约定回电时间","礼貌结束"].map((task) => <label key={task} className="flex items-center gap-2 rounded-xl bg-white p-3 font-bold"><input type="checkbox" className="accent-[#487a54]" />{task}</label>)}</div></section><div className="mt-4 grid grid-cols-3 gap-3 text-center text-[11px] font-black"><span className="rounded-xl bg-[#f1f6fb] px-3 py-2 text-[#3d6f9f]">信息完整 40%</span><span className="rounded-xl bg-[#f2f8f3] px-3 py-2 text-[#487a54]">语法正确 40%</span><span className="rounded-xl bg-[#fff8ed] px-3 py-2 text-[#9a6b2f]">语气自然 20%</span></div><button type="button" onClick={() => speak("여보세요, 지훈 씨 휴대폰이지요? 네, 그런데요. 지금 뭐 하고 있어요? 지금 운전하고 있어요. 잠깐 통화할 수 있어요? 미안해요. 운전하고 있어서 오래 통화 못 해요. 그럼 언제 다시 전화할까요? 삼십 분 후에 전화해 주세요. 네, 나중에 다시 전화할게요. 감사합니다.")} className="mt-auto flex items-center justify-center gap-2 rounded-2xl bg-[#487a54] p-4 text-sm font-black text-white"><Volume2 size={16} />播放十句示范</button></>),
      "35": <div className="flex h-full flex-col justify-center"><div className="mx-auto w-full max-w-[440px] text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#e8f4eb] text-[#487a54]"><Sparkles size={27} /></span><p className="mt-4 text-xs font-black tracking-[0.18em] text-[#487a54]">LESSON 12 · COMPLETE</p><h2 className="mt-3 text-4xl font-black">여보세요.</h2><p className="mt-3 text-lg font-black">你已经完成第十二课</p><p className="mx-auto mt-3 max-w-[390px] text-sm leading-7 text-[#60736a]">现在你能确认电话对象、说明正在做的事、解释无法通话的原因，并留下清楚的回电信息。</p><div className="mt-4 grid grid-cols-2 gap-3 text-left">{[["01","寻求确认","A/V-지요?"],["02","确认名词","N-(이)지요?"],["03","说明进行","V-고 있어요"],["04","客观不能","못 + V"],["05","表达原因","A/V-아서/어서"]].map(([i,t,d]) => <div key={i} className="rounded-xl border border-[#dce8e1] bg-white px-4 py-3"><p className="text-[10px] font-black text-[#487a54]">{i}</p><p className="mt-1 text-xs font-black">{t}</p><p className="mt-1 text-[10px] text-[#71857b]">{d}</p></div>)}</div><button type="button" onClick={() => flipBookRef.current?.pageFlip()?.flip(1)} className="mt-4 rounded-full bg-[#eaf2fb] px-4 py-3 text-xs font-black text-[#3d6f9f]">返回目录</button></div></div>,
    };
    return pages[number];
  }

  function Exercise({ items, shown }: { items: string[][]; shown: boolean }) {
    return <div className="mt-5 grid grid-cols-2 gap-3">{items.map(([question, answer]) => <article key={`${question}-${answer}`} className={`rounded-xl border border-[#cfe3d4] bg-white text-xs font-black ${items.length > 10 ? "min-h-[58px] p-3" : "min-h-[68px] p-4"}`}><span>{question}</span><p className={`mt-3 leading-5 text-[#487a54] ${shown ? "opacity-100" : "opacity-0"}`}>{answer}</p></article>)}</div>;
  }
