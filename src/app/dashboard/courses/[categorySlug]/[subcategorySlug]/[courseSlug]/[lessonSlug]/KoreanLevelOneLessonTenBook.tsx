    ...
    <div className="mt-4 grid grid-cols-2 gap-3">
      {lines.map((line, index) => (
        <div
          ...
          className={`flex gap-2 rounded-xl p-3.5 ${index % 2 ? "bg-[var(--status-warning-surface)]" : "bg-[var(--status-success-surface)]"}`}
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className={`text-[10px] font-bold leading-5 text-[var(--foreground-secondary)] ${showChinese ? "opacity-100" : "opacity-0"}`}>
              {line.chinese}
            </p>
          </div>
...
const dividers: Record<string, { step: string; title: string; goal: string; icon: ReactNode }> = {
  "02": { step: "第一步", title: "课前导航", goal: "从读准一个时刻，走向安排完整日程：说明起止范围、动作链和未来计划。", icon: <CalendarClock aria-hidden="true" size={24} /> },
  "04": { step: "第二步", title: "核心词汇", goal: "把时间段、作息动词和日程词块绑定记忆，让词汇直接进入时间轴。", icon: <AlarmClock aria-hidden="true" size={24} /> },
  "09": { step: "第三步", title: "语法讲解", goal: "四项语法各占一页：精确时间、起止范围、紧密动作链和未来计划。", icon: <NotebookPen aria-hidden="true" size={24} /> },
  "14": { step: "第四步", title: "句型操练", goal: "同时训练固有词小时、汉字词分钟、부터/까지和两种“然后”。", icon: <TimerReset aria-hidden="true" size={24} /> },
  "18": { step: "第五步", title: "实战对话", goal: "在上学日、周末计划和旅行行程中完成三组不少于八句的交流。", icon: <MessageCircle aria-hidden="true" size={24} /> },
  "22": { step: "第六步", title: "听说任务", goal: "从语音中抓住时间点、动作顺序和计划，再口头重建日程。", icon: <Headphones aria-hidden="true" size={24} /> },
  "26": { step: "第七步", title: "读写拓展", goal: "读懂一日安排，并写出有起止、有动作链、有未来计划的原创日程。", icon: <BookOpenCheck aria-hidden="true" size={24} /> },
  "29": { step: "第八步", title: "自测与复盘", goal: "检查时间表达、范围助词、动作连接与未来时，并完成口语验收。", icon: <CheckCircle2 aria-hidden="true" size={24} /> },
};
...
export function KoreanLevelOneLessonTenBook({
  ...
  function renderPage(number: string) {
    ...
    const pages: Record<string, ReactNode> = {
      "03": content("03", "把一天变成可执行的时间轴", "不只回答“几点”，还要说明从何时到何时、先做什么，以及接下来打算做什么。", <Route aria-hidden="true" size={22} />, <>
        <div className="mt-5 grid grid-cols-2 gap-3">{[
          ...
          <button key={tag} type="button" onClick={() => speak(korean)} className="rounded-2xl border border-[var(--border)] bg-white p-4 text-left">
            <b className="text-[10px] text-[var(--status-warning)]">{tag}</b>
            <div className="mt-2 flex items-center justify-between gap-2"><p className="text-sm font-bold">{korean}</p><Volume2 aria-hidden="true" size={14} className="shrink-0 text-[var(--primary)]" /></div>
            <p className={`mt-1 text-[11px] text-[var(--foreground-secondary)] ${revealed.chinese03 ? "opacity-100" : "opacity-0"}`}>{chinese}</p>
          </button>
        ))}</div>
      ...
      "05": content("05", "1. 时间段与日程单位", "时间段放在具体时刻前面，可以迅速消除上午、下午的歧义。", <Clock3 aria-hidden="true" size={22} />, <><WordGrid words={timeWords} speak={speak} showChinese={Boolean(revealed.chinese05)} /><Note title="时间层级">大范围在前，精确时刻在后：평일 아침 일곱 시／주말 오후 세 시 반。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese05)} onClick={() => toggle("chinese05")} />),
      "06": content("06", "2. 日常作息动词", "把动词放到真实时间点上记忆，不再只背词典形。", <AlarmClock aria-hidden="true" size={22} />, <><WordGrid words={routineWords} speak={speak} showChinese={Boolean(revealed.chinese06)} /><Note title="成对记忆" tone="green">출발하다 ↔ 도착하다；시작하다 ↔ 끝나다；출근하다 ↔ 퇴근하다。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese06)} onClick={() => toggle("chinese06")} />),
      "07": content("07", "3. 一日行程词块", "完整词块已经包含宾语或地点，能直接接时间表达和句尾。", <Route aria-hidden="true" size={22} />, <><WordGrid words={scheduleWords} speak={speak} showChinese={Boolean(revealed.chinese07)} /><Note title="直接套入时间轴" tone="amber">오전 여덟 시에 학교에 가요. 오후 여섯 시에 집에 와요.</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese07)} onClick={() => toggle("chinese07")} />),
      "08": content("08", "4. 连接形与未来形", "先熟悉本课会反复出现的形态，再进入规则推导。", <Link2 aria-hidden="true" size={22} />, <><WordGrid words={formWords} speak={speak} showChinese={Boolean(revealed.chinese08)} /><Note title="不规则预警" tone="rose">들을 거예요 中 ㄷ→ㄹ；추울 거예요 中 ㅂ→우。变化来自后接的元音词尾。</Note></>, <KoreanEbookRevealButton shown={Boolean(revealed.chinese08)} onClick={() => toggle("chinese08")} />),
      "10": content("10", "1. 시간 · 精确时间表达", "小时用固有词数字，分钟用汉字词数字；时间段放在最前面。", <Clock3 aria-hidden="true" size={22} />, <>
        <div className="mt-4 grid grid-cols-2 gap-3">{[
          ["整点", "오전 아홉 시", "上午9点"],
          ...
          ["正午时刻", "정오 열두 시", "正午12点"],
        ].map(([label, korean, chinese]) => <article key={label} className="rounded-2xl border border-[var(--border)] bg-white p-4"><b className="text-[var(--primary)]">{label}</b><div className="mt-3 text-base font-bold"><RuleSentence text={korean} speak={speak}>{korean}</RuleSentence></div><p className={`mt-1 text-xs text-[var(--foreground-secondary)] ${revealed.chinese10 ? "opacity-100" : "opacity-0"}`}>{chinese}</p></article>)}</div>
      ...
      "11": content("11", "2. N부터 N까지", "부터标记起点，까지标记终点；既可连接时间，也可连接空间。", <Route aria-hidden="true" size={22} />, <>
      ...
      "12": content("12", "3. V-아서／어서", "连接紧密相关的动作；前项常表示移动、方式、前提或原因。", <Link2 aria-hidden="true" size={22} />, <>
      ...
      "13": content("13", "4. V-(으)ㄹ 거예요", "表示未来计划、意图或有根据的预测，空格必须保留。", <CalendarClock aria-hidden="true" size={22} />, <>
        <div className="mt-4 grid grid-cols-2 gap-3">{[
      ...
      "15": content("15", "1. 时间组装实验", "按“时间段—小时—分钟—에”的顺序组装。", <Clock3 aria-hidden="true" size={22} />, <>
        ...
        ].map(([prompt, answer]) => <article key={prompt} className="grid min-h-[62px] grid-cols-[1fr_1.25fr] items-center rounded-xl border border-[var(--border)] bg-white p-4 text-xs font-bold"><span>{prompt}</span><span className={`text-[var(--status-warning)] ${revealed.time ? "opacity-100" : "opacity-0"}`}>{answer}</span></article>)}</div>
      ...
      "16": content("16", "2. 起点和终点诊所", "判断缺少的是 부터、까지，还是两者都需要。", <Route aria-hidden="true" size={22} />, <>
        ...
        ].map(([question, answer]) => <article key={question} className="grid grid-cols-2 gap-3 rounded-xl border border-[var(--border)] bg-white p-3 text-xs font-bold"><span>{question}</span><span className={`text-[var(--status-warning)] ${revealed.range ? "opacity-100" : "opacity-0"}`}>{answer}</span></article>)}</div>
      </>, <KoreanEbookRevealButton shown={Boolean(revealed.range)} onClick={() => toggle("range")} answer />),
      "17": content("17", "3. -고 还是 -아서／어서？", "根据动作之间是否存在直接逻辑联系来选择。", <Link2 aria-hidden="true" size={22} />, <>
        <div className="mt-4 grid grid-cols-2 gap-3">{[
        ...
        ].map(([korean, chinese, reason]) => <article key={korean} className="rounded-2xl border border-[var(--border)] bg-white p-4"><RuleSentence text={korean} speak={speak}><span className="text-sm font-bold">{korean}</span></RuleSentence><p className="mt-1 text-[10px] text-[var(--foreground-secondary)]">{chinese}</p><p className={`mt-2 text-[10px] font-bold text-[var(--status-warning)] ${revealed.connector ? "opacity-100" : "opacity-0"}`}>{reason}</p></article>)}</div>
      </>, <KoreanEbookRevealButton shown={Boolean(revealed.connector)} onClick={() => toggle("connector")} answer />),
      "23": content("23", "1. 听力 · 一天的时间轴", "第一遍记时刻，第二遍补动作，第三遍确认起止范围。", <Headphones aria-hidden="true" size={22} />, <>
        <button type="button" onClick={() => speak("저는 평일 아침 여섯 시 반에 일어납니다. 씻어서 일곱 시에 아침을 먹습니다. 여덟 시부터 오후 네 시까지 학교에서 공부합니다. 집에 와서 저녁을 먹고 숙제합니다. 밤 열한 시에 잘 거예요.")} className="mt-5 flex items-center justify-center gap-2 rounded-2xl bg-[var(--primary)] p-4 text-sm font-bold text-white"><Volume2 aria-hidden="true" size={17} />播放一日日程</button>
        <div className="mt-5 grid grid-cols-2 gap-3">{[
      ...
      "24": content("24", "2. 日程重建卡", "把五张信息卡按时间顺序排列，再逐条复述。", <TimerReset aria-hidden="true" size={22} />, <>
        <div className="mt-4 space-y-2.5">{[
          ["01", "오전 일곱 시에 일어나요."], ["02", "씻어서 아침을 먹어요."],
          ["03", "여덟 시부터 네 시까지 공부해요."], ["04", "친구를 만나서 같이 운동해요."],
          ["05", "밤에는 책을 읽고 잘 거예요."],
          ["06", "밤 열한 시쯤 잠을 잘 거예요."],
        ].map(([step, sentence]) => <article key={step} className="grid grid-cols-[55px_1fr] items-center rounded-xl border border-[var(--border)] bg-white p-3"><b className="text-[var(--primary)]">{step}</b><RuleSentence text={sentence} speak={speak}><span className="text-sm font-bold">{sentence}</span></RuleSentence></article>)}</div>
        <Note title="复述升级">第二轮不看文字，只看 01—05；第三轮把时间和活动替换成自己的真实安排。</Note>
      </>),
      "25": content("25", "3. 60秒未来日程播报", "用三个时间点、一个范围和两组动作链介绍明天。", <Mic2 aria-hidden="true" size={22} />, <>
        <div className="mt-4 grid grid-cols-2 gap-3">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 text-[11px] font-bold text-[var(--primary)]">{["版本 A · 工作日","版本 B · 周末","版本 C · 考试日","版本 D · 旅行日"].map((item) => <span key={item} className="rounded-xl bg-[var(--card)] px-3 py-3">{item}</span>)}</div>
        <p className="mt-4 text-center text-[11px] font-bold text-[var(--foreground-secondary)]">先画时间轴再说；说话时只看时间点，不读完整稿。</p>
      </>),
      "27": content("27", "1. 阅读 · 민지의 평일", "找出所有时刻、范围、连接形和未来计划。", <BookOpenCheck aria-hidden="true" size={22} />, <>
        <section className="mt-4 rounded-2xl border border-[var(--border)] bg-white p-5"><p className="text-[11px] font-bold text-[var(--status-success)]">민지의 하루</p><p className="mt-3 text-sm font-bold leading-7">민지는 평일 아침 여섯 시 사십 분에 일어나요. 씻어서 일곱 시에 아침을 먹어요. 여덟 시부터 오후 세 시까지 학교에서 공부해요. 수업이 끝나서 도서관에 가요. 다섯 시까지 숙제를 하고 집에 와요. 오늘 밤에는 일찍 잘 거예요.</p></section>
      ...
      "28": content("28", "2. 写作 · 我的明日日程", "写 8—10 句原创计划，让别人能够按时间轴复原你的一天。", <NotebookPen aria-hidden="true" size={22} />, <>
        ...
        <section className="mt-4 rounded-2xl border border-dashed border-[var(--border)] bg-[var(--card)] p-5"><p className="text-[11px] font-bold text-[var(--status-success)]">原创示范</p><p className="mt-3 text-sm font-bold leading-7">내일은 아침 일곱 시에 일어날 거예요. 씻어서 아침을 먹을 거예요. 아홉 시부터 열두 시까지 한국어를 공부할 거예요. 점심에는 친구를 만나서 같이 밥을 먹을 거예요. 오후에는 카페에 가서 책을 읽을 거예요. 밤 열한 시쯤 잘 거예요.</p></section>
        <div className="mt-4 grid grid-cols-2 gap-2 text-[11px] font-bold text-[var(--status-success)]">{["✓ 三个准确时刻","✓ 一个起止范围","✓ 两组紧密动作","✓ 三个未来计划"].map((item) => <span key={item} className="rounded-xl bg-[var(--status-success-surface)] px-3 py-2">{item}</span>)}</div>
      </>),
      "30": content("30", "1. 时间与作息闪测", "看到中文后两秒内说出韩语。", <AlarmClock aria-hidden="true" size={22} />, <div className="mt-4 grid grid-cols-3 gap-2.5">{[
      ...
      "31": content("31", "2. 时间与范围检测", "检查两套数字、时间段顺序和부터／까지。", <Clock3 aria-hidden="true" size={22} />, <>
        <div className="mt-4 grid grid-cols-2 gap-2.5">{[
        ...
        ].map(([question, answer]) => <article key={question} className="rounded-xl border border-[var(--border)] bg-white p-3"><b>{question}</b><p className={`mt-2 text-sm font-bold text-[var(--status-success)] ${revealed.timeTest ? "opacity-100" : "opacity-0"}`}>{answer}</p></article>)}</div>
      </>, <KoreanEbookRevealButton shown={Boolean(revealed.timeTest)} onClick={() => toggle("timeTest")} answer />),
      "32": content("32", "3. 综合语法检测", "完成动作连接和未来计划表达。", <CheckCircle2 aria-hidden="true" size={22} />, <div className="mt-4 space-y-2">{[
        ["去餐厅吃饭。", "식당에 가서 밥을 먹어요."],
      ...
      "33": content("33", "4. 口语验收 · 我的明天", "不看稿连续讲述 60 秒，再回答三个时间追问。", <Mic2 aria-hidden="true" size={22} />, <>
        <section className="mt-4 rounded-2xl border border-[var(--border)] bg-[var(--status-success-surface)] p-5"><p className="text-xs font-bold text-[var(--status-success)]">八项必达信息</p><div className="mt-4 grid grid-cols-2 gap-3 text-xs">{["三个准确时刻","一个上午／下午时间段","一个부터/까지范围","一次移动后行动","一次原因连接","三个未来计划","一个ㄷ不规则未来形","结尾说明睡觉时间"].map((task) => <label key={task} className="flex items-center gap-2 rounded-xl bg-white p-3 font-bold"><input type="checkbox" className="accent-[var(--status-success)]" />{task}</label>)}</div></section>
        <div className="mt-4 grid grid-cols-3 gap-3 text-center text-[11px] font-bold"><span className="rounded-xl bg-[var(--accent)] px-3 py-2 text-[var(--primary)]">时间完整 40%</span><span className="rounded-xl bg-[var(--status-success-surface)] px-3 py-2 text-[var(--status-success)]">语法正确 40%</span><span className="rounded-xl bg-[var(--status-warning-surface)] px-3 py-2 text-[var(--status-warning)]">表达自然 20%</span></div>
        <button type="button" onClick={() => speak("내일 아침 일곱 시에 일어날 거예요. 씻어서 아침을 먹을 거예요. 아홉 시부터 열두 시까지 공부할 거예요. 점심에는 친구를 만나서 밥을 먹을 거예요. 오후에는 도서관에 가서 책을 읽을 거예요. 저녁에는 음악을 들을 거예요. 밤 열한 시에 잘 거예요.")} className="mt-auto flex items-center justify-center gap-2 rounded-2xl bg-[var(--status-success)] p-4 text-sm font-bold text-white"><Volume2 aria-hidden="true" size={16} />播放七句示范</button>
      </>),
      "34": <div className="flex h-full flex-col justify-center"><div className="mx-auto w-full max-w-[440px] text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--status-success-surface)] text-[var(--status-success)]"><Sparkles aria-hidden="true" size={27} /></span><p className="mt-4 text-xs font-bold tracking-[0.18em] text-[var(--status-success)]">LESSON 10 · COMPLETE</p><h3 className="mt-3 text-4xl font-bold">지금 몇 시예요?</h3><p className="mt-3 text-lg font-bold">你已经完成第十课</p><p className="mx-auto mt-3 max-w-[390px] text-sm leading-7 text-[var(--foreground-secondary)]">现在你能准确表达时刻和日程范围，区分两种动作连接方式，并完整说明未来计划。</p><div className="mt-4 grid grid-cols-2 gap-3 text-left">{[
        ["01", "精确读时", "시간 표현"], ["02", "说明范围", "N부터 N까지"],
        ["03", "连接动作", "V-아서/어서"], ["04", "表达计划", "V-(으)ㄹ 거예요"],
      ].map(([index, title, detail]) => <div key={index} className="rounded-xl border border-[var(--border)] bg-white px-4 py-3"><p className="text-[10px] font-bold text-[var(--status-success)]">{index}</p><p className="mt-1 text-xs font-bold">{title}</p><p className="mt-1 text-[10px] text-[var(--foreground-secondary)]">{detail}</p></div>)}</div><div className="mt-4 rounded-2xl border border-[var(--border)] bg-[var(--status-success-surface)] px-5 py-3.5 text-left"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold tracking-[0.14em] text-[var(--status-success)]">LESSON 10 TEST · 本课测试</p><p className="mt-1 text-xs font-bold text-[var(--foreground-secondary)]">检验时间、日程范围、动作连接和未来计划。</p></div><button type="button" onClick={() => window.location.assign("/dashboard/assignments/korean")} className="shrink-0 rounded-full bg-white px-3 py-2 text-[10px] font-bold text-[var(--status-success)] shadow-sm">前往测试专区</button></div></div><button type="button" onClick={() => flipBookRef.current?.pageFlip()?.flip(1)} className="mt-4 rounded-full bg-[var(--accent)] px-4 py-3 text-xs font-bold text-[var(--primary)]">返回目录</button></div></div>,
    };
