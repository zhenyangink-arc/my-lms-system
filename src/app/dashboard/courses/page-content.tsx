...
function KoreanDirectCourseCatalog({
  ...
  chapterProgressMap: Map<string, number>;
  ...
  const courseSections: KoreanCourseCatalogSection[] = subcategories.map(
    (subcategory) => {
      ...
      const catalogCourses = categoryCourses.map((course, index) => {
        const courseLessons = (lessonsByCourseId.get(course.id) ?? []).toSorted(
        ...
        const currentLesson =
          ...
          courseLessons.at(-1) ??
          null;
        const currentLessonChapters = currentLesson
          ? chapters
              .filter((chapter) => chapter.lesson_id === currentLesson.id)
              .toSorted((a, b) => a.sort_order - b.sort_order)
          : [];
        const currentChapter =
          currentLessonChapters.find((chapter) => {
            const progress = chapterProgressMap.get(chapter.slug) ?? 0;
            return (
              !completedChapterSlugs.has(chapter.slug) &&
              progress > 0 &&
              progress < 100
            );
          }) ??
          currentLessonChapters.find(
            (chapter) => !completedChapterSlugs.has(chapter.slug),
          ) ??
          currentLessonChapters.at(-1) ??
          null;
        return {
          id: course.id,
          sequence: index + 1,
          ...
          currentChapterTitle: currentChapter
            ? currentChapter.sort_order > 0
              ? `第 ${currentChapter.sort_order} 章：${currentChapter.title}`
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] space-y-7 px-4 py-6 sm:px-6 lg:py-10">
      <header className="max-w-3xl">
        <h1 id="korean-course-catalog-title" className="text-3xl font-bold tracking-tight sm:text-4xl">韩语课程</h1>
        <p className="app-muted-text mt-3 text-base leading-7">继续当前学习，或从课程目录中选择下一门课程。</p>
      </header>
...
export async function CourseCatalog({
  ...
  if (studentAppSlug === "korean" && lessonIds.length > 0) {
    const [chapterResult, attemptResult, ebookProgressResult, overviewProgress] =
      await Promise.all([
        supabase
          .from("course_chapters")
          .select("id, lesson_id, slug, title, sort_order")
          .in("lesson_id", lessonIds)
          .eq("is_published", true)
          .order("sort_order", { ascending: true }),
        isPlatformAudit
          ? Promise.resolve({ data: [] as { test_slug: string }[] })
          : supabase
        ...
        isPlatformAudit
          ? Promise.resolve({
              data: [] as { test_slug: string; progress_percent: number }[],
            })
          : supabase
    ...
    chapters = (chapterResult.data ?? []) as CourseChapter[];
    catalogReadError ||= Boolean(chapterResult.error);

    for (const attempt of attemptResult.data ?? []) {
      completedChapterSlugs.add(String(attempt.test_slug));
    }

    for (const progress of ebookProgressResult.data ?? []) {
      const percent = Math.min(
        100,
        Math.max(0, Number(progress.progress_percent) || 0),
      );
      const chapterSlug = String(progress.test_slug);
      chapterProgressMap.set(chapterSlug, percent);
      if (percent >= 100) completedChapterSlugs.add(chapterSlug);
    }

    chapterProgressMap.set("korean-level-one-00", overviewProgress);
