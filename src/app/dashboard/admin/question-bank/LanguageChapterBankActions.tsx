          ...
          <section role="dialog" aria-modal="true" aria-label={`${chapter.title}${mode === "view" ? "查看题目" : "新增题目"}`} className="flex max-h-[94vh] w-full max-w-[1380px] flex-col overflow-hidden border bg-[var(--card)] shadow-2xl">
            ...
            <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4 sm:p-5">
              <table className="w-full border-collapse border text-left">
                ...
                <tbody>{skills.map((item) => <tr key={item.key} className="border-t" style={skill === item.key ? { backgroundColor: "var(--support-surface)" } : undefined}><td className="px-3 py-2 text-xs font-semibold">{item.label}</td><td className="border-l px-3 py-2 text-center font-mono text-xs">{materials.filter((row) => row.languageSkill === item.key).length}</td><td className="border-l px-3 py-2 text-center font-mono text-xs">{questions.filter((row) => row.languageSkill === item.key).length}</td><td className="border-l px-3 py-2 text-right"><button type="button" onClick={() => setSkill(item.key)} className="text-xs font-semibold" style={{ color: "var(--primary)" }}>{skill === item.key ? "当前" : mode === "view" ? "查看" : "新增"}</button></td></tr>)}</tbody>
              </table>
              ...
              {mode === "create" ? (
                ...
                <>
                  {(skill === "listening" || skill === "reading") && <div className="overflow-x-auto border"><table className="w-full min-w-[850px] border-collapse text-left"><caption className="border-b px-3 py-2.5 text-left text-sm font-semibold">{skill === "listening" ? "听力音频表" : "阅读文章表"}</caption><thead><tr className="border-b bg-[var(--surface-soft)] app-muted-text"><th className="px-3 py-2 text-[11px]">电子书目录</th><th className="border-l px-3 py-2 text-[11px]">韩语名称</th><th className="border-l px-3 py-2 text-center text-[11px]">难度</th><th className="border-l px-3 py-2 text-center text-[11px]">长度</th><th className="border-l px-3 py-2 text-center text-[11px]">状态</th></tr></thead><tbody>{selectedMaterials.map((item) => <tr key={item.id} className="border-b last:border-b-0"><td className="px-3 py-2 text-xs">{koreanEbookSectionLabel(item.ebookSectionStep)}{item.ebookPageReference ? ` · ${item.ebookPageReference}` : ""}</td><td className="border-l px-3 py-2 text-xs font-bold">{item.titleKo}</td><td className="border-l px-3 py-2 text-center text-xs">{difficultyLabels[item.difficulty]}</td><td className="border-l px-3 py-2 text-center text-xs">{lengthLabels[item.materialLength]}</td><td className="border-l px-3 py-2 text-center text-xs font-bold">{statusLabels[item.status]}</td></tr>)}{selectedMaterials.length === 0 && <tr><td colSpan={5} className="app-muted-text px-3 py-8 text-center text-xs">本章还没有{skill === "listening" ? "听力音频" : "阅读文章"}。</td></tr>}</tbody></table></div>}
                  <div className="overflow-x-auto border"><table className="w-full min-w-[1050px] border-collapse text-left"><caption className="border-b px-3 py-2.5 text-left text-sm font-semibold">{skills.find((item) => item.key === skill)?.label}题目表</caption><thead><tr className="border-b bg-[var(--surface-soft)] app-muted-text"><th className="px-3 py-2 text-[11px]">电子书目录</th><th className="border-l px-3 py-2 text-[11px]">考查类别</th><th className="border-l px-3 py-2 text-[11px]">题型</th><th className="border-l px-3 py-2 text-center text-[11px]">难度</th><th className="border-l px-3 py-2 text-[11px]">韩语题干</th><th className="border-l px-3 py-2 text-[11px]">关联资源</th><th className="border-l px-3 py-2 text-center text-[11px]">状态</th></tr></thead><tbody>{selectedQuestions.map((item) => <tr key={item.id} className="border-b align-top last:border-b-0"><td className="px-3 py-2 text-xs">{koreanEbookSectionLabel(item.ebookSectionStep)}{item.ebookPageReference ? ` · ${item.ebookPageReference}` : ""}</td><td className="border-l px-3 py-2 font-mono text-[11px]">{item.assessmentCategory}</td><td className="border-l px-3 py-2 text-xs font-bold">{typeLabels[item.questionType]}</td><td className="border-l px-3 py-2 text-center text-xs">{difficultyLabels[item.difficulty]}</td><td className="border-l px-3 py-2 text-xs font-bold leading-6">{item.promptKo}</td><td className="app-muted-text border-l px-3 py-2 text-xs">{item.materialId ? materialById.get(item.materialId)?.titleKo ?? "已关联资源" : "独立题目"}</td><td className="border-l px-3 py-2 text-center text-xs font-bold">{statusLabels[item.status]}</td></tr>)}{selectedQuestions.length === 0 && <tr><td colSpan={7} className="app-muted-text px-3 py-9 text-center text-xs">本章还没有该能力题目。</td></tr>}</tbody></table></div>
                </>
              )}
            </div>
          </section>
