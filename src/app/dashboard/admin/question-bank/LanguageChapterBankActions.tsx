  ...
  return (
    <>
      <div className="flex justify-end gap-1">
        <button type="button" onClick={() => setMode("view")} className="inline-flex items-center gap-1.5 px-2.5 py-2 text-xs font-black" style={{ color: "var(--app-secondary)" }}><Eye size={14} />查看题目</button>
        <button type="button" onClick={() => setMode("create")} className="inline-flex items-center gap-1.5 px-2.5 py-2 text-xs font-black" style={{ color: "var(--app-accent)" }}><Plus size={14} />新增题目</button>
      </div>
      ...
      {mode && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/45 p-3 backdrop-blur-sm sm:p-6" onMouseDown={(event) => event.target === event.currentTarget && setMode(null)}>
          <section role="dialog" aria-modal="true" aria-label={`${chapter.title}${mode === "view" ? "查看题目" : "新增题目"}`} className="flex max-h-[94vh] w-full max-w-[1380px] flex-col overflow-hidden border bg-[var(--app-card-bg)] shadow-2xl">
            <header className="flex items-center gap-4 border-b px-5 py-4">
              <div className="min-w-0 flex-1"><p className="app-muted-text text-[10px] font-black">{bank === "homework" ? "作业题库" : "考试题库"} · CH {String(chapter.chapterNumber).padStart(2, "0")}</p><h2 className="mt-1 text-lg font-black">{chapter.title}</h2></div>
            ...
            <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4 sm:p-5">
              <table className="w-full border-collapse border text-left">
                <thead><tr className="bg-[var(--app-soft-bg)] app-muted-text"><th className="px-3 py-2 text-[11px]">能力</th><th className="border-l px-3 py-2 text-center text-[11px]">资源</th><th className="border-l px-3 py-2 text-center text-[11px]">题目</th><th className="border-l px-3 py-2 text-right text-[11px]">选择</th></tr></thead>
                <tbody>{skills.map((item) => <tr key={item.key} className="border-t" style={skill === item.key ? { backgroundColor: "var(--app-secondary-soft)" } : undefined}><td className="px-3 py-2.5 text-xs font-black">{item.label}</td><td className="border-l px-3 py-2.5 text-center font-mono text-xs">{materials.filter((row) => row.languageSkill === item.key).length}</td><td className="border-l px-3 py-2.5 text-center font-mono text-xs">{questions.filter((row) => row.languageSkill === item.key).length}</td><td className="border-l px-3 py-2.5 text-right"><button type="button" onClick={() => setSkill(item.key)} className="text-xs font-black" style={{ color: "var(--app-accent)" }}>{skill === item.key ? "当前" : mode === "view" ? "查看" : "新增"}</button></td></tr>)}</tbody>
              </table>
