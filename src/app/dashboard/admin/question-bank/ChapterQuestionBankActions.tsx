              ...
              {openModal === "create" ? (
                ...
                <div className="overflow-x-auto border-y">
                  <table className="w-full min-w-[1050px] border-collapse text-left">
                    <thead><tr className="border-b bg-[var(--surface-soft)] app-muted-text"><th className="px-3 py-2 text-[11px]">序号</th><th className="border-l px-3 py-2 text-[11px]">电子书目录</th><th className="border-l px-3 py-2 text-center text-[11px]">难度</th><th className="border-l px-3 py-2 text-[11px]">题目</th><th className="border-l px-3 py-2 text-[11px]">四个选项</th><th className="border-l px-3 py-2 text-[11px]">答案与解析</th><th className="border-l px-3 py-2 text-[11px]">状态</th></tr></thead>
                    <tbody>
                  {questions.map((question, index) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-3 py-2 text-center font-mono text-xs font-semibold text-[var(--support)]">{index + 1}</td>
                        ...
                        <td className="border-l px-3 py-2 text-center text-xs font-bold">{DIFFICULTY_LABELS[question.difficulty]}</td>
                        <td className="border-l px-3 py-2"><p className="whitespace-pre-wrap text-sm font-semibold leading-6">{question.prompt}</p><p className="app-muted-text mt-1 text-[10px]">{QUESTION_TYPE_LABELS[question.question_type]} · {SKILL_LABELS[question.skill] ?? "综合"}</p></td>
                        ...
                        <td className="border-l px-3 py-2 text-xs"><p className="font-semibold">{STATUS_LABELS[question.status]}</p><p className="app-muted-text mt-1">{question.default_points} 分</p></td>
                      </tr>
                    );
                  })}
                    </tbody>
