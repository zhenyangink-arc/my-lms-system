                                ...
                                <table className="w-full min-w-[980px] border-collapse text-left">
                                  <thead>
                                    <tr className="border-b bg-[var(--surface-soft)] app-muted-text">
                                      <th className="px-4 py-2 text-[11px] font-bold">章节</th>
                                      <th className="border-l px-4 py-2 text-[11px] font-bold">章节标题</th>
                                      <th className="border-l px-4 py-2 text-center text-[11px] font-bold">题目</th>
                                      <th className="border-l px-4 py-2 text-center text-[11px] font-bold">基础</th>
                                      <th className="border-l px-4 py-2 text-center text-[11px] font-bold">中等</th>
                                      <th className="border-l px-4 py-2 text-[11px] font-bold">电子书目录覆盖</th>
                                      <th className="border-l px-4 py-2 text-right text-[11px] font-bold">操作</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                  {lesson.groups.map((group) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        <td className="px-4 py-2 font-mono text-[11px] font-semibold app-muted-text">
                                          CH {String(group.chapter_number).padStart(2, "0")}
                                        </td>
                                        <td className="border-l px-4 py-2"><p className="text-sm font-semibold">{group.title}</p><p className="app-muted-text mt-0.5 text-[10px]">{group.korean_title}{groupDrafts > 0 ? ` · ${groupDrafts} 题待发布` : ""}</p></td>
                                        <td className="border-l px-4 py-2 text-center font-mono text-xs font-semibold">{groupQuestions.length}</td>
                                        <td className="border-l px-4 py-2 text-center font-mono text-xs">{difficultyCounts.foundation ?? 0}</td>
                                        <td className="border-l px-4 py-2 text-center font-mono text-xs">{difficultyCounts.medium ?? 0}</td>
                                        <td className="border-l px-4 py-2 text-[11px] leading-5">{ebookSections.map(koreanEbookSectionLabel).join("、") || "未标注"}</td>
                                        <td className="border-l px-4 py-2 text-right"><ChapterQuestionBankActions
                                          group={
                                            labeledGroups.find(
              ...
              <>
                ...
                <section className="overflow-x-auto border-y">
                  <table className="w-full min-w-[1350px] border-collapse text-left">
                    <thead>
                      <tr className="border-b bg-[var(--surface-soft)] app-muted-text">
                        <th className="px-3 py-2 text-[11px]">序号</th>
                        <th className="border-l px-3 py-2 text-[11px]">电子书目录</th>
                        <th className="border-l px-3 py-2 text-center text-[11px]">难度</th>
                        <th className="border-l px-3 py-2 text-[11px]">题目</th>
                        <th className="border-l px-3 py-2 text-[11px]">四个选项</th>
                        <th className="border-l px-3 py-2 text-[11px]">答案与解析</th>
                        <th className="border-l px-3 py-2 text-[11px]">知识点／状态</th>
                        <th className="border-l px-3 py-2 text-right text-[11px]">操作</th>
                      </tr>
                    </thead>
                    <tbody>
                  {visibleQuestions.map((question, index) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-3 py-2 text-center font-mono text-[11px] font-semibold text-[var(--support)]">{String(absoluteIndex).padStart(2, "0")}</td>
                        <td className="border-l px-3 py-2 text-[11px] leading-5"><span className="font-semibold">{koreanEbookSectionLabel(question.ebook_section_step)}</span>{question.ebook_page_reference && <span className="app-muted-text block">{question.ebook_page_reference}</span>}</td>
                        <td className="border-l px-3 py-2 text-center text-xs font-bold">{question.difficulty === "foundation" || question.difficulty === "medium" ? DIFFICULTY_LABELS[question.difficulty] : question.difficulty}</td>
                        <td className="border-l px-3 py-2"><p className="whitespace-pre-wrap text-sm font-semibold leading-6">{question.prompt}</p><p className="app-muted-text mt-1 text-[10px]">{QUESTION_TYPE_LABELS[question.question_type]} · {group ? `第 ${group.chapter_number} 章` : "课程题目"}</p></td>
                        <td className="border-l px-3 py-2 text-xs leading-5">{options.map((option, optionIndex) => <p key={`${question.id}-${optionIndex}`} className={optionIndex === question.correct_option ? "font-semibold text-[var(--status-success)]" : ""}><span className="mr-1 font-mono">{String.fromCharCode(65 + optionIndex)}.</span>{option}</p>)}</td>
                        <td className="border-l px-3 py-2 text-xs leading-5"><p className="font-semibold text-[var(--status-success)]">{answer || "人工批改"}</p>{question.explanation && <p className="app-muted-text mt-1.5">{question.explanation}</p>}</td>
                        <td className="border-l px-3 py-2 text-xs"><p className="font-semibold">{SKILL_LABELS[question.skill] ?? "综合"}</p><p className="app-muted-text mt-1">{STATUS_LABELS[question.status]} · 版本 {question.version} · {question.default_points} 分</p></td>
                        <td className="border-l px-3 py-2 text-right"><EditStandardQuestionForm groups={labeledGroups} question={question} compact /></td>
                      </tr>
                    );
                  })}
