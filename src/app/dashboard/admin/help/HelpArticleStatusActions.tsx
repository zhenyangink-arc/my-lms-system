...
function Button({ id, current, target, label }: { id: string; current: HelpArticleStatus; target: HelpArticleStatus; label: string }) { const action = changeHelpArticleStatusAction.bind(null, id, target); const [state, formAction, pending] = useActionState(action, initialHelpCenterActionState); const Icon = target === "published" ? Send : target === "archived" ? Archive : FilePenLine; if (current === target) return null; return <form action={formAction}><button type="submit" disabled={pending} title={state.message || undefined} className="app-soft-card inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-black disabled:opacity-50"><Icon size={12} />{pending ? "处理中…" : label}</button></form>; }
