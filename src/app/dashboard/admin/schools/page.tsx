      ...
      <div className="mx-auto w-full max-w-[1500px] space-y-6 p-4 sm:p-6 lg:p-8">
        <section className="app-card overflow-hidden rounded-[30px] border p-6 sm:p-8" style={{ background: "linear-gradient(125deg, var(--app-card-bg), var(--app-hero-start), var(--app-hero-end))" }}>
          <div className="grid gap-6 xl:grid-cols-[1fr_560px] xl:items-end">
            <div><span className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black" style={{ color: "var(--app-accent)", backgroundColor: "var(--app-accent-soft)" }}><Database size={15} />统一学校数据中心</span><h2 className="mt-5 text-3xl font-black tracking-tight sm:text-4xl">一套结构，分开管理五类学校</h2><p className="app-muted-text mt-4 max-w-2xl text-sm leading-7">韩国大学保留原有 100 所学校与学生目标关联；其他院校独立建档。停用展示不会删除历史数据。</p></div>
            ...
            ].map(({ label, value, icon: Icon }) => <div key={label} className="app-card rounded-2xl border p-4 text-center"><Icon className="mx-auto" size={18} style={{ color: "var(--app-accent)" }} /><p className="mt-2 text-2xl font-black">{value}</p><p className="app-muted-text mt-1 text-[10px] font-black">{label}</p></div>)}</div>
          </div>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <Link href="/dashboard/admin/schools/overview" className="app-card group rounded-[26px] border p-5 transition hover:-translate-y-1 hover:shadow-lg">
            <span className="flex h-12 w-12 items-center justify-center rounded-2xl" style={{ color: "var(--app-accent)", backgroundColor: "var(--app-accent-soft)" }}><OverviewIcon size={22} /></span><div className="mt-5 flex items-center justify-between"><div><h3 className="text-lg font-black">{schoolOverview.label}</h3><p className="app-muted-text mt-2 text-xs leading-6">{schoolOverview.description}</p></div><ArrowRight className="shrink-0 transition group-hover:translate-x-1" size={18} /></div>
          </Link>
          {schoolCategories.map((category) => {
            const Icon = category.icon;
            const count = schools.filter((school) => school.category === category.value).length;
            return <Link key={category.slug} href={`/dashboard/admin/schools/${category.slug}`} className="app-card group rounded-[26px] border p-5 transition hover:-translate-y-1 hover:shadow-lg"><div className="flex items-center justify-between"><span className="flex h-12 w-12 items-center justify-center rounded-2xl" style={{ color: "var(--app-secondary)", backgroundColor: "var(--app-secondary-soft)" }}><Icon size={22} /></span><span className="app-soft-card rounded-full border px-3 py-1 text-xs font-black">{count} 所</span></div><div className="mt-5 flex items-end justify-between gap-3"><div><h3 className="text-lg font-black">{category.label}</h3><p className="app-muted-text mt-2 text-xs leading-6">{category.description}</p></div><ArrowRight className="shrink-0 transition group-hover:translate-x-1" size={18} /></div></Link>;
          })}
        </section>
      </div>
    </>
