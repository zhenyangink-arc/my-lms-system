          ...
          <table className="w-full min-w-[900px] border-collapse text-left">
            <thead>
              <tr className="border-b text-[11px] font-medium" style={{ borderColor: "var(--border)", color: "var(--foreground-muted)" }}>
                <th className="w-[30%] px-4 py-2 font-medium">学生</th>
                <th className="w-[12%] px-4 py-2 font-medium">会员档位</th>
                <th className="w-[10%] px-4 py-2 font-medium">状态</th>
                <th className="w-[16%] px-4 py-2 font-medium">最近活跃</th>
                <th className="px-4 py-2 font-medium">注册时间</th>
                <th className="w-[12%] px-4 py-2 text-right font-medium">操作</th>
              </tr>
            </thead>
            <tbody>
              {students.map((student) => {
                ...
                return (
                  <tr key={student.id} className="border-b text-xs last:border-b-0 hover:bg-black/[0.018]" style={{ borderColor: "var(--border)" }}>
                    ...
                    <td className="app-muted-text px-4 py-2 tabular-nums"><LocalDateTime value={student.registered_at ?? null} options={DATE_TIME_OPTIONS} fallback="暂无记录" /></td>
                    <td className="app-muted-text px-4 py-2 tabular-nums"><LocalDateTime value={student.created_at ?? null} options={DATE_TIME_OPTIONS} fallback="暂无记录" /></td>
                    <td className="px-4 py-2">
