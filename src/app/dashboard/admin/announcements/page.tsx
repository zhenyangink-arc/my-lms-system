  ...
  return (
    <div className="pb-12">
      ...
      <div className="mx-auto mt-6 w-full max-w-[1500px] space-y-6 px-4 sm:px-6 lg:px-8">
        <section className="app-card rounded-[30px] border p-6 sm:p-8" style={{ background: "linear-gradient(125deg, var(--app-card-bg), var(--app-hero-start), var(--app-accent-soft))" }}><div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_500px] xl:items-end"><div><span className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black" style={{ color: "var(--app-accent)", backgroundColor: "var(--app-accent-soft)" }}><ShieldCheck size={14} />{role === "super_admin" ? "负责人权限" : role === "ceo" ? "运营负责人权限" : "已授权管理员"}</span><h2 className="mt-5 text-3xl font-black tracking-tight sm:text-4xl">公告编辑与学生查看彻底分开</h2><p className="app-muted-text mt-4 max-w-2xl text-sm leading-7">这里可以查看草稿和归档记录并进行编辑；学生公告栏只会读取已经发布的内容。</p></div><div className="grid grid-cols-3 gap-3">{[["已发布", publishedCount, Send, "var(--app-success)", "var(--app-success-soft)"], ["草稿", draftCount, FilePenLine, "var(--app-accent)", "var(--app-accent-soft)"], ["已归档", archivedCount, Archive, "var(--app-warm)", "var(--app-warm-soft)" ]].map(([label, value, Icon, color, soft]) => { const MetricIcon = Icon as typeof Send; return <div key={String(label)} className="app-card rounded-2xl border p-4 text-center"><span className="mx-auto flex h-9 w-9 items-center justify-center rounded-xl" style={{ color: String(color), backgroundColor: String(soft) }}><MetricIcon size={17} /></span><p className="mt-2 text-2xl font-black">{String(value)}</p><p className="app-muted-text text-[10px] font-black">{String(label)}</p></div>; })}</div></div></section>
