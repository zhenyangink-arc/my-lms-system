...
const nodeSchema = z.object({
  ...
  bufferPresetId: z.string().trim().refine((value) => value === LEARNING_AGENT_BUFFER_PRESET_NONE_ID || Boolean(learningAgentBufferPreset(value)), "必须从系统兜底台词库中选择一条，或选择不显示过渡台词。"),
...
export async function saveTeachingScriptNodeAction(
  ...
  try {
    ...
    const parsed = nodeSchema.safeParse({
      ...
      bufferPresetId: String(formData.get("buffer_preset_id") ?? ""),
    ...
    if (input.bufferPresetId === LEARNING_AGENT_BUFFER_PRESET_NONE_ID) {
      configuration.bufferLine = { "zh-CN": "", "ko-KR": "" };
      configuration.bufferPresetId = LEARNING_AGENT_BUFFER_PRESET_NONE_ID;
    } else {
      const bufferPreset = learningAgentBufferPreset(input.bufferPresetId);
      if (!bufferPreset) return { status: "error", message: "必须从系统兜底台词库中选择一条，或选择不显示过渡台词。", fieldErrors: { bufferPresetId: ["请选择一条系统兜底台词，或选择不显示过渡台词。"] } };
      configuration.bufferLine = bufferPreset.text;
      configuration.bufferPresetId = bufferPreset.id;
    }
