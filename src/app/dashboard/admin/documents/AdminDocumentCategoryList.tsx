    ...
    <section className="app-card rounded-[1.75rem] border p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          className="inline-flex shrink-0 items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-semibold transition hover:-translate-y-0.5"
      ...
      {categoryGroups.length === 0 ? (
        ...
        <div className="mt-5 space-y-5">
          {categoryGroups.map(({ category, items }) => {
            ...
            return (
              <div key={category}>
                <div className="mb-3 flex items-center gap-2 border-b pb-2.5" style={{ borderColor: "var(--border-subtle)" }}>
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg" style={{ color: "var(--primary)", backgroundColor: "var(--accent)" }}><CategoryIcon size={14} /></span>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-start justify-between gap-3">
                              <div><h2 className="text-sm font-semibold">{document.title}</h2><p className="app-muted-text mt-1 text-xs">{CATEGORY_LABELS[document.category] ?? document.category}</p></div>
                              <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5">
                                {locked && <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold" style={{ color: "var(--status-warning)", backgroundColor: "var(--status-warning-surface)" }}><Lock size={10} />已锁定</span>}
                                <span className="rounded-full px-2.5 py-1 text-xs font-semibold" style={{ color: tone.color, backgroundColor: tone.soft }}>{STATUS_LABELS[document.status]}</span>
                              </div>
                            </div>
                            <p className="mt-2 text-xs font-semibold" style={{ color: "var(--primary)" }}>{document.targetLabel ?? "历史通用资料"}</p>
                            ...
                            {document.notes && <div className="mt-3 flex items-start gap-2 rounded-xl px-3 py-2.5 text-xs leading-5" style={{ color: "var(--support)", backgroundColor: "var(--support-surface)" }}><MessageSquareText className="mt-0.5 shrink-0" size={13} /><p className="whitespace-pre-wrap"><b>资料备注：</b>{document.notes}</p></div>}
                            {document.admin_note && <div className="mt-3 flex items-start gap-2 rounded-xl px-3 py-2.5 text-xs leading-5" style={{ color: "var(--status-warning)", backgroundColor: "var(--status-warning-surface)" }}><MessageSquarePlus className="mt-0.5 shrink-0" size={13} /><p className="whitespace-pre-wrap"><b>管理员备注：</b>{document.admin_note}</p></div>}
                            <div className="mt-3 flex items-center justify-between gap-2 border-t pt-3" style={{ borderColor: "var(--border-subtle)" }}>
