    ...
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto border bg-[var(--card)]">
        <table className="w-full min-w-[760px] border-collapse text-left text-xs">
          ...
          <thead className="bg-[var(--surface-soft)] text-[var(--foreground-muted)]"><tr><th scope="col" className="px-4 py-2">机构</th>{columns.map((column) => <th key={column} scope="col" className="px-4 py-2">{column}</th>)}</tr></thead>
          <tbody>
            {rows.map((row) => {
              ...
              return <tr key={row.id} className="border-t border-[var(--border-subtle)]"><td className="px-4 py-2 font-medium">{row.name}</td>{values.map((value, index) => <td key={columns[index]} className="px-4 py-2 tabular-nums">{value}</td>)}</tr>;
            })}
