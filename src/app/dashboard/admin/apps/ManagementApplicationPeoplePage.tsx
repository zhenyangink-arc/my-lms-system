        ...
        <section className="space-y-3">
          ...
          <div className="overflow-x-auto border bg-[var(--card)]">
            <table className="w-full min-w-[760px] border-collapse text-left text-xs">
              ...
              <thead className="bg-[var(--surface-soft)] text-[var(--foreground-muted)]"><tr><th className="px-4 py-2">员工</th><th className="px-4 py-2">机构角色</th><th className="px-4 py-2">应用角色</th><th className="px-4 py-2">状态</th><th className="px-4 py-2 text-right">保存</th></tr></thead>
              <tbody>
                {staff.map((member) => {
                  const current = staffAccessById.get(member.user_id);
                  return (
                    <tr key={member.user_id} className="border-t border-[var(--border-subtle)]">
                      <td className="px-4 py-2 font-medium">{displayName(profiles.get(member.user_id))}</td>
                      <td className="app-muted-text px-4 py-2">{roleLabels[member.role] ?? member.role}</td>
      ...
      <section className="space-y-3">
        ...
        <div className="overflow-x-auto border bg-[var(--card)]">
          <table className="w-full min-w-[940px] border-collapse text-left text-xs">
            ...
            <thead className="bg-[var(--surface-soft)] text-[var(--foreground-muted)]"><tr><th className="px-4 py-2">学生</th><th className="px-4 py-2">机构等级</th><th className="px-4 py-2">应用状态</th><th className="px-4 py-2">负责老师</th><th className="px-4 py-2">新增老师</th></tr></thead>
            <tbody>
              {students.map((student) => {
                ...
                return (
                  <tr key={student.user_id} className="border-t border-[var(--border-subtle)] align-top">
                    <td className="px-4 py-2 font-medium">{displayName(profiles.get(student.user_id))}</td>
                    <td className="app-muted-text px-4 py-2">{student.membership_tier}</td>
