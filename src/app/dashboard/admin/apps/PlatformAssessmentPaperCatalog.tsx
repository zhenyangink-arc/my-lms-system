...
export function PlatformAssessmentPaperCatalog({
  ...
  return (
    <section className="overflow-hidden border bg-[var(--card)]">
      ...
      <p className="app-muted-text px-4 pt-3 text-[11px]">
        共 {filteredPapers.length} 套{filteredPapers.length !== papers.length ? `（全部 ${papers.length} 套）` : ""}
      </p>
      <div className="max-h-[60vh] overflow-y-auto overflow-x-auto">
        <table className="w-full min-w-[1180px] table-fixed border-collapse text-left">
          ...
          <thead className="sticky top-0 z-10 bg-[var(--surface-soft)] app-muted-text">
            <tr>
              {[
                "试卷",
                "来源章节",
                ...
                <th
                  key={label}
                  className={`${index > 0 ? "border-l" : ""} px-4 py-2 text-[11px] font-bold ${index >= 2 && index <= 5 ? "text-center" : index === 6 ? "text-right" : ""}`}
                >
                  {label}
                </th>
              ))}
          ...
          <tbody>
            {filteredPapers.map((paper) => (
              ...
              >
                ...
                <td className="border-l px-4 py-2 text-center">
                  <span
                ...
                <td className="border-l px-4 py-2 text-center text-xs">
                  <p className="font-bold tabular-nums">
                    {paper.institutionCount} 家机构
                  </p>
                  <p className="app-muted-text mt-0.5 tabular-nums">
            ...
            {filteredPapers.length === 0 && (
              <tr>
                <td colSpan={7} className="app-muted-text px-5 py-12 text-center text-xs">
                  {papers.length === 0
                    ? "当前应用还没有标准试卷。"
                    : "没有符合当前筛选条件的试卷。"}
                </td>
