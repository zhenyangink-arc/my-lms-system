...
function FormRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <tr className="border-b border-black/[0.065] last:border-b-0">
      <th className="w-[145px] border-r border-black/[0.065] px-4 py-2 text-left text-[10px] font-medium text-zinc-500 align-top">
        {label}
      </th>
                  ...
                  <tbody>
                    {rows.map(([label, value]) => (
                      <tr key={label} className="border-b border-black/[0.07] last:border-b-0">
                        <th className="w-[150px] border-r border-black/[0.07] px-4 py-2 align-top text-[10px] font-medium uppercase tracking-[0.06em] text-zinc-500">{label}</th>
                        <td className="whitespace-pre-wrap px-4 py-2 font-medium leading-5 text-zinc-900">{value}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <div className="overflow-x-auto">
        <table className={`w-full border-collapse text-left ${canManageContent ? "min-w-[930px]" : "min-w-[760px]"}`}>
          <thead className="bg-white">
            <tr className="h-9 border-b border-black/[0.08] text-[10px] uppercase tracking-[0.07em] text-zinc-500">
                  ...
                  {isExpanded && regionUniversities.map((university, index) => {
                    ...
                    return (
                      ...
                      <tr onClick={() => setSelectedUniversityId(university.id)} className="group h-[46px] cursor-pointer border-b border-black/[0.06] text-xs transition hover:bg-zinc-50/80">
                        ...
                        <td className="px-3 py-2"><span className={`inline-flex items-center gap-2 text-[11px] font-medium ${university.is_published ? "text-emerald-700" : "text-amber-700"}`}><span className={`size-1.5 rounded-full ${university.is_published ? "bg-emerald-500" : "bg-amber-500"}`} />{university.is_published ? "已发布" : "已隐藏"}</span></td>
