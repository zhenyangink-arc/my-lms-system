...
const stageOptions = [
  { key: "admission", label: "入学许可", description: "学校录取与许可文件", icon: GraduationCap, color: "var(--status-success)", soft: "var(--status-success-surface)", suggestions: ["标准入学许可书", "学费缴纳证明", "录取通知书"] },
  { key: "identity", label: "身份材料", description: "护照、照片及身份关系", icon: UserRound, color: "var(--primary)", soft: "var(--accent)", suggestions: ["护照", "签证照片", "身份证", "户口本", "亲属关系证明"] },
  { key: "finance", label: "资金材料", description: "存款、收入与资金担保", icon: Landmark, color: "var(--support)", soft: "var(--support-surface)", suggestions: ["存款证明", "银行流水", "父母在职证明", "收入证明"] },
  { key: "application", label: "申请表格", description: "签证表格和补充说明", icon: FilePenLine, color: "var(--status-warning)", soft: "var(--status-warning-surface)", suggestions: ["签证申请表", "学习计划书", "学历证明", "成绩单"] },
...
function AddButton() {
  ...
  return <button type="submit" disabled={pending} className="inline-flex items-center justify-center gap-1.5 rounded-xl px-3 py-2.5 text-xs font-semibold text-white disabled:opacity-60" style={{ backgroundColor: "var(--status-success)" }}><Plus size={13} />{pending ? "添加中…" : "添加"}</button>;
}

function SaveButton() {
  const { pending } = useFormStatus();
  return <button type="submit" disabled={pending} className="inline-flex items-center justify-center gap-1.5 rounded-xl px-4 py-2.5 text-xs font-semibold text-white disabled:opacity-60" style={{ backgroundColor: "var(--status-success)" }}><Pencil size={13} />{pending ? "保存中…" : "保存修改"}</button>;
}
...
function SortButton({ direction, disabled, title }: { direction: "up" | "down"; disabled: boolean; title: string }) {
  ...
  return <button type="submit" disabled={disabled || pending} aria-label={title} title={title} className="inline-flex h-8 w-8 items-center justify-center rounded-lg transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-25" style={{ color: "var(--foreground-muted)" }}><Icon size={14} /></button>;
}
...
function RequirementEditDialog({ universityId, requirement }: { universityId: string; requirement: UniversityVisaRequirement }) {
  return (
    <Dialog>
      <DialogTrigger type="button" aria-label={`修改${requirement.title}`} className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition hover:bg-white" style={{ color: "var(--status-success)" }}><Pencil size={13} /></DialogTrigger>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="font-semibold">修改签证申请资料</DialogTitle>
        ...
        <form action={updateUniversityVisaRequirementAction.bind(null, universityId, requirement.id)} className="space-y-4">
          ...
          <div className="overflow-hidden rounded-xl border"><table className="w-full border-collapse text-left text-xs"><tbody>
            ...
            <tr className="border-b" style={{ borderColor: "var(--border-subtle)" }}><th className="border-r bg-[var(--surface-soft)] px-3 py-2 font-semibold" style={{ borderColor: "var(--border-subtle)" }}>办理阶段</th><td className="px-3 py-2"><select name="stage" defaultValue={requirement.stage} className="app-input w-full rounded-lg border px-3 py-2.5 outline-none">{stageOptions.map((stage) => <option key={stage.key} value={stage.key}>{stage.label}</option>)}</select></td></tr>
...
export function UniversityVisaRequirementsDialog({ canManage, universityId, universityName, requirements }: { canManage: boolean; universityId: string; universityName: string; requirements: UniversityVisaRequirement[] }) {
  ...
  return (
    <Dialog>
      <DialogTrigger type="button" className="inline-flex items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-semibold" style={{ color: "var(--status-success)", borderColor: "var(--status-success)" }}><FileCheck2 size={13} /> 签证申请</DialogTrigger>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-6xl">
        ...
        <section className="rounded-2xl border p-3" style={{ borderColor: "var(--border-subtle)", backgroundColor: "var(--surface-soft)" }}>
          <div className="grid gap-2 sm:grid-cols-4">
            {visaTypeOptions.map((visaType) => {
              ...
              return (
                <button key={visaType.key} type="button" onClick={() => setSelectedVisaType(visaType.key)} className="rounded-xl border px-3 py-3 text-left transition" style={active ? { color: "var(--status-success)", borderColor: "var(--status-success)", backgroundColor: "var(--status-success-surface)" } : { color: "var(--foreground-muted)", borderColor: "transparent", backgroundColor: "var(--card)" }}>
                  <span className="block text-xs font-semibold">{visaType.label}</span>
        ...
        <div key={selectedVisaType} className="overflow-x-auto rounded-xl border">
          <table className="w-full min-w-[900px] border-collapse text-left text-xs">
            <thead><tr className="border-b bg-[var(--surface-soft)] text-[10px] font-semibold app-muted-text" style={{ borderColor: "var(--border)" }}><th className="w-[135px] px-4 py-2">办理阶段</th><th className="w-[220px] px-3 py-2">资料名称</th><th className="px-3 py-2">学生可见备注</th><th className="w-[90px] px-3 py-2 text-center">顺序</th><th className="w-[100px] px-4 py-2 text-right">操作</th></tr></thead>
            <tbody>
