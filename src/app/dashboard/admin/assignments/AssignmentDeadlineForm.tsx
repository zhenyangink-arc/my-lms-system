...
export function AssignmentDeadlineForm({ assignmentId }: { assignmentId: string }) {
  ...
  return <form action={formAction} className="app-card rounded-2xl border p-4"><label className="text-xs font-black">调整截止时间（韩国时间）<input name="due_at" type="datetime-local" required className="app-input mt-2 w-full rounded-xl border px-3 py-2.5 text-sm" /></label>{state.message && <p className="mt-2 text-[11px] font-bold" style={{ color: state.status === "error" ? "#c94f45" : "var(--app-success)" }}>{state.message}</p>}<button disabled={pending} className="mt-3 inline-flex items-center gap-1.5 rounded-xl px-4 py-2.5 text-xs font-black text-white disabled:opacity-50" style={{ backgroundColor: "var(--app-secondary)" }}><CalendarClock size={14} />{pending ? "保存中…" : "更新截止时间"}</button></form>;
}
