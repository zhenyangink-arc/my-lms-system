...
export function SubmissionGradingForm({ submissionId, answers }: { submissionId: string; answers: AnswerForGrading[] }) {
  ...
  return (
    <form action={formAction} className="app-soft-card rounded-2xl border p-4">
      ...
      <div className="mt-3 space-y-3">{answers.map((answer) => <div key={answer.id} className="app-card grid gap-2 rounded-xl border p-3 sm:grid-cols-[120px_1fr]"><label className="text-[11px] font-black">第 {answer.index} 题得分<input name={`score_${answer.id}`} type="number" min={0} max={answer.maxPoints} step="0.5" required defaultValue={answer.awardedPoints ?? 0} className="app-input mt-1.5 w-full rounded-lg border px-2 py-2 text-sm" /></label><label className="text-[11px] font-black">单题评语<input name={`feedback_${answer.id}`} maxLength={2000} defaultValue={answer.feedback ?? ""} placeholder="可选" className="app-input mt-1.5 w-full rounded-lg border px-3 py-2 text-xs" /></label></div>)}</div>
      <label className="mt-3 block text-xs font-black">总体评语<textarea name="overall_feedback" maxLength={3000} rows={4} placeholder="总结完成情况；退回重做时必须写明原因。" className="app-input mt-2 w-full rounded-xl border px-3 py-2.5 text-xs leading-6" /></label>
