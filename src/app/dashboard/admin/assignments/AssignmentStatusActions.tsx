...
function StatusButton({ id, status, label }: { id: string; status: AssignmentStatus; label: string }) {
  ...
  return <form action={formAction}><button disabled={pending} className="app-soft-card inline-flex items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-black disabled:opacity-50" style={{ color: status === "published" ? "var(--app-accent)" : status === "closed" ? "var(--app-warm)" : "var(--app-muted)" }}><Icon size={13} />{pending ? "处理中…" : label}</button>{state.message && <p className="mt-1 text-xs font-bold" style={{ color: state.status === "error" ? "#c94f45" : "var(--app-success)" }}>{state.message}</p>}</form>;
}
