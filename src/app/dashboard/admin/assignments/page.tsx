  ...
  return (
    <div className="pb-12">
      ...
      <div className="mx-auto mt-6 w-full max-w-[1500px] space-y-6 px-4 sm:px-6 lg:px-8">
        <section className="app-card overflow-hidden rounded-[30px] border p-6 sm:p-8" style={{ background: "linear-gradient(125deg, var(--app-card-bg), var(--app-hero-start), var(--app-accent-soft))" }}>
          <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_560px] xl:items-end">
            <div><span className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black" style={{ color: "var(--app-accent)", backgroundColor: "var(--app-accent-soft)" }}><ClipboardCheck size={15} />教学任务工作台</span><h2 className="mt-5 text-3xl font-black tracking-tight sm:text-4xl">从布置到反馈形成完整闭环</h2><p className="app-muted-text mt-4 max-w-2xl text-sm leading-7">支持作业、测验和考试，既可发给全部学生，也可指定学生。每次提交都会保留独立版本，方便追踪进步。</p><p className="app-muted-text mt-3 text-xs">当前身份：{role === "teacher" ? "教师" : role === "admin" ? "管理员" : role === "ceo" ? "运营负责人" : "负责人"}</p></div>
        ...
        <section className="app-card rounded-[30px] border p-5 sm:p-7">
          ...
          <div className="mt-5 grid gap-4 xl:grid-cols-2">
            {assignments.map((assignment) => {
              ...
              return <article key={assignment.id} className="app-soft-card rounded-3xl border p-5"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full px-2.5 py-1 text-[10px] font-black" style={{ color: "var(--app-accent)", backgroundColor: "var(--app-accent-soft)" }}>{ASSIGNMENT_TYPE_LABELS[assignment.assignment_type]}</span><span className="rounded-full px-2.5 py-1 text-[10px] font-black" style={{ color: tone.color, backgroundColor: tone.soft }}>{ASSIGNMENT_STATUS_LABELS[assignment.status]}</span><span className="app-muted-text ml-auto text-[10px] font-bold">{assignment.total_points} 分</span></div><h3 className="mt-4 text-lg font-black">{assignment.title}</h3><p className="app-muted-text mt-2 line-clamp-2 text-xs leading-6">{assignment.description || "暂未填写任务说明"}</p><div className="mt-4 grid grid-cols-3 gap-2"><div className="app-card rounded-xl border p-2.5 text-center"><p className="text-lg font-black">{latestStudents}</p><p className="app-muted-text text-[9px] font-bold">提交学生</p></div><div className="app-card rounded-xl border p-2.5 text-center"><p className="text-lg font-black">{waiting}</p><p className="app-muted-text text-[9px] font-bold">待批改</p></div><div className="app-card rounded-xl border p-2.5 text-center"><p className="truncate text-xs font-black">{assignment.course_id ? courseNames.get(assignment.course_id) ?? "关联课程" : "综合任务"}</p><p className="app-muted-text mt-1 text-[9px] font-bold">课程</p></div></div><div className="app-muted-text mt-4 flex flex-wrap items-center gap-3 text-[10px]"><span>截止：{formatAssignmentDate(assignment.due_at)}</span><span>{assignment.target_scope === "all_students" ? "全部学生" : "指定学生"}</span>{assignment.duration_minutes && <span>建议 {assignment.duration_minutes} 分钟</span>}</div><div className="mt-4 flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between" style={{ borderColor: "var(--app-border-soft)" }}><AssignmentStatusActions id={assignment.id} status={assignment.status} /><Link href={`/dashboard/admin/assignments/${assignment.id}`} className="inline-flex items-center justify-center gap-1.5 rounded-xl px-4 py-2.5 text-xs font-black text-white" style={{ backgroundColor: "var(--app-secondary)" }}>查看与批改<ArrowRight size={13} /></Link></div></article>;
            })}
            {assignments.length === 0 && <div className="col-span-full rounded-3xl border border-dashed p-10 text-center"><ClipboardList className="mx-auto opacity-30" size={32} /><p className="mt-3 font-black">还没有教学任务</p><p className="app-muted-text mt-2 text-xs">使用下方表单布置第一份作业或考试。</p></div>}
          </div>
        </section>
        ...
        <section className="app-soft-card flex items-start gap-3 rounded-2xl border p-4 text-xs leading-6 app-muted-text"><UsersRound className="mt-0.5 shrink-0" size={16} /><p>指定学生模式只向选中的学生开放任务；全部学生模式会自动覆盖当前和后续新增的正常学生账号。</p></section>
      </div>
    </div>
  );
}
