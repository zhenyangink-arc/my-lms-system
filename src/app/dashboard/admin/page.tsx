...
export default async function AdminCenterPage() {
  const { role } = await requireAssignmentManager();
  ...
  return (
    <div className="mx-auto w-full max-w-[1380px] space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <section className="app-card overflow-hidden rounded-[32px] border p-6 sm:p-8" style={{ background: "linear-gradient(125deg, var(--app-hero-start), var(--app-card-bg), var(--app-accent-soft))" }}>
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_420px] xl:items-end">
          <div>
            ...
            <h1 className="mt-5 text-3xl font-black tracking-tight sm:text-4xl">把管理工作集中到一个清晰入口</h1>
            <p className="app-muted-text mt-4 max-w-2xl text-sm leading-7">课程、教学任务、院校服务与账号权限按业务分组。左侧管理导航可以随时收起，为内容操作留出更多空间。</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="app-card rounded-2xl border p-4"><Layers3 size={19} style={{ color: "var(--app-accent)" }} /><p className="mt-3 text-3xl font-black">{visibleItems.length}</p><p className="app-muted-text mt-1 text-xs font-bold">当前可用模块</p></div>
          ...
          <section key={group} className="space-y-4">
            ...
            <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">
              {items.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} className="app-card group rounded-[26px] border p-5 transition hover:-translate-y-0.5 hover:shadow-lg">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><h3 className="text-lg font-black">{item.label}</h3><ArrowRight className="shrink-0 transition group-hover:translate-x-1" size={17} /></div><p className="app-muted-text mt-2 text-xs leading-6">{item.description}</p></div>
                    </div>
      ...
      <section className="app-soft-card flex items-start gap-3 rounded-2xl border p-4 text-xs leading-6 app-muted-text"><LockKeyhole className="mt-0.5 shrink-0" size={16} /><p>页面入口和服务端权限使用同一角色规则。未授权模块不会显示，直接输入地址也会被原有服务端检查拦截。</p></section>
    </div>
  );
}
