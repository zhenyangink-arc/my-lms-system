  ...
  return (
    <div className="pb-12">
      ...
      <div className="mx-auto mt-5 w-full max-w-[1500px] space-y-5 px-4 sm:px-6 lg:px-8">
        ...
        <section className="management-detail-summary app-card rounded-3xl border p-5 sm:p-6"><div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-center"><div><span className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black" style={{ color: tenant.status === "active" ? "var(--app-success)" : "var(--app-warm)", backgroundColor: tenant.status === "active" ? "var(--app-success-soft)" : "var(--app-warm-soft)" }}><ShieldCheck size={14} />{tenant.status === "active" ? "运行中" : "已停用，可恢复"}</span><DashboardTitleWithHint className="mt-3" headingLevel={2} titleClassName="text-2xl font-black" title={<>租户管理工作台</>} description={<>停用会停止该租户使用，但不会删除管理员账号和成员关系；删除则不可恢复，必须先停用并通过对话框确认。</>} /></div><div className="dashboard-title-metrics"><div className="app-soft-card rounded-2xl border p-4"><Users size={18} style={{ color: "var(--app-secondary)" }} /><p className="mt-2 text-2xl font-black">{memberships.length}</p><p className="app-muted-text text-xs font-bold">租户成员</p></div><div className="app-soft-card rounded-2xl border p-4"><Building2 size={18} style={{ color: "var(--app-accent)" }} /><p className="mt-2 text-sm font-black">{tenant.plan_key}</p><p className="app-muted-text text-xs font-bold">套餐</p></div></div></div></section>

        <section className="app-card rounded-3xl border p-5"><DashboardTitleWithHint headingLevel={2} titleClassName="text-xl font-black" title={<>租户管理员与成员</>} description={<>密码不会显示或保存到页面中；负责人可在下方为机构负责人重置密码。</>} /><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[620px] text-left text-sm"><thead className="app-muted-text border-b text-xs" style={{ borderColor: "var(--app-border-soft)" }}><tr><th className="px-3 py-3 font-black">姓名</th><th className="px-3 py-3 font-black">登录账号</th><th className="px-3 py-3 font-black">租户角色</th><th className="px-3 py-3 font-black">状态</th></tr></thead><tbody>{memberships.map((member) => { const profile = profileById.get(member.user_id); return <tr key={member.user_id} className="border-b" style={{ borderColor: "var(--app-border-soft)" }}><td className="px-3 py-3 font-bold">{profile?.full_name ?? "未填写姓名"}</td><td className="px-3 py-3 font-mono text-xs">{profile?.login_id ?? "历史账号"}</td><td className="px-3 py-3">{roleLabels[member.role] ?? member.role}</td><td className="px-3 py-3"><span className="rounded-full px-2.5 py-1 text-xs font-black" style={{ color: member.status === "active" ? "var(--app-success)" : "var(--app-warm)", backgroundColor: member.status === "active" ? "var(--app-success-soft)" : "var(--app-warm-soft)" }}>{member.status === "active" ? "正常" : member.status}</span></td></tr>; })}</tbody></table></div>{memberships.length === 0 && <p className="app-muted-text mt-5 text-sm">当前没有成员记录。</p>}</section>
