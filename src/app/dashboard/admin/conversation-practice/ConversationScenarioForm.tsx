...
export function ConversationScenarioForm({ scenario }: { scenario?: ConversationScenarioFormValue }) {
  ...
  return (
    <section id={scenario ? "edit-scenario" : "create-scenario"} className="app-card rounded-[28px] border p-5 sm:p-7">
      <div className="flex items-start gap-3">
        ...
        <div><h2 className="text-lg font-black">{scenario ? "编辑会话场景" : "新建会话场景"}</h2><p className="app-muted-text mt-1 text-xs leading-6">先整理情景、示范对话和重点表达，再决定保存草稿或发布。</p></div>
      </div>
      ...
      <form action={formAction} className="mt-6 space-y-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="text-xs font-black md:col-span-2">场景简介
            <textarea name="description" maxLength={500} rows={3} defaultValue={scenario?.description} placeholder="学生在列表中看到的简短说明。" className="app-input mt-2 w-full resize-y rounded-xl border px-4 py-3 text-sm leading-7" />
          </label>
          <label className="text-xs font-black md:col-span-2">情景说明
            <textarea name="situation" maxLength={1500} rows={4} defaultValue={scenario?.situation} placeholder="说明人物、地点、目标和需要完成的交流任务。" className="app-input mt-2 w-full resize-y rounded-xl border px-4 py-3 text-sm leading-7" />
          </label>
        </div>

        <div className="grid gap-4 xl:grid-cols-2">
          <label className="text-xs font-black">学习目标（每行一条）
            <textarea name="learning_objectives" rows={6} defaultValue={scenario?.learning_objectives.join("\n")} placeholder={'能够礼貌地点单\n能够询问价格与数量'} className="app-input mt-2 w-full resize-y rounded-xl border px-4 py-3 text-sm leading-7" />
          </label>
          <label className="text-xs font-black">重点表达（每行：韩语｜中文）
            <textarea name="key_expressions" rows={6} defaultValue={scenario?.key_expressions.map((item) => `${item.korean}|${item.chinese}`).join("\n")} placeholder={'아메리카노 한 잔 주세요.|请给我一杯美式咖啡。\n얼마예요?|多少钱？'} className="app-input mt-2 w-full resize-y rounded-xl border px-4 py-3 text-sm leading-7" />
          </label>
        </div>

        <label className="block text-xs font-black">示范对话（每行：角色｜韩语｜中文）
          <textarea name="sample_dialogue" rows={10} defaultValue={scenario?.sample_dialogue.map((item) => `${item.speaker}|${item.korean}|${item.chinese}`).join("\n")} placeholder={'店员|어서 오세요. 무엇을 드릴까요?|欢迎光临，请问您要点什么？\n学生|아메리카노 한 잔 주세요.|请给我一杯美式咖啡。'} className="app-input mt-2 w-full resize-y rounded-xl border px-4 py-3 font-mono text-sm leading-7" />
        </label>

        <div className="grid gap-4 xl:grid-cols-2">
          <label className="text-xs font-black">开场任务
            <textarea name="starter_prompt" maxLength={1000} rows={4} defaultValue={scenario?.starter_prompt} placeholder="例如：你是顾客，请先向店员问好，然后点一杯饮料。" className="app-input mt-2 w-full resize-y rounded-xl border px-4 py-3 text-sm leading-7" />
          </label>
          <label className="text-xs font-black">练习提示
            <textarea name="practice_tips" maxLength={1500} rows={4} defaultValue={scenario?.practice_tips} placeholder="提示语气、发音、替换词或进阶挑战。" className="app-input mt-2 w-full resize-y rounded-xl border px-4 py-3 text-sm leading-7" />
          </label>
        </div>
