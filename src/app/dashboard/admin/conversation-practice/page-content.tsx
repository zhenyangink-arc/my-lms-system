        ...
        <table className="w-full min-w-[780px] border-collapse text-left">
          <thead>
            <tr className="app-muted-text border-b text-[10px]" style={{ borderColor: "var(--border)" }}>
              ...
              <th className="w-[36%] px-3 py-2 font-medium">练习复盘</th>
            </tr>
          </thead>
          <tbody>
            {progress.map((item) => (
              <tr key={item.user_id} className="border-b text-[11px] last:border-b-0" style={{ borderColor: "var(--border-subtle)" }}>
                <td className="px-3 py-2 font-medium">{studentNames.get(item.user_id) || "学生"}</td>
                ...
                <td className="app-muted-text px-3 py-2">{item.practice_count} 次</td>
                <td className="app-muted-text px-3 py-2">{item.confidence ? `${item.confidence} / 5` : "未评价"}</td>
                <td className="app-muted-text whitespace-nowrap px-3 py-2 text-[10px]"><LocalDateTime value={item.last_practiced_at} options={CONVERSATION_DATE_TIME_OPTIONS} /></td>
                <td className="app-muted-text px-3 py-2 leading-5">{item.reflection || "—"}</td>
              </tr>
            ))}
            {progress.length === 0 && (
