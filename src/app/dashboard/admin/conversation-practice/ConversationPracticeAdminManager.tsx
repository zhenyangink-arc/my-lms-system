...
function RevokeButton({ adminId }: { adminId: string }) {
  ...
  return <form action={formAction}><button type="submit" disabled={pending} title={state.message || undefined} className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-[10px] font-black disabled:opacity-50" style={{ color: "#c94f45", backgroundColor: "#fff0ed" }}><ShieldOff size={11} />{pending ? "处理中" : "撤销"}</button></form>;
}
...
export function ConversationPracticeAdminManager({ admins }: { admins: AdminOption[] }) {
  ...
  return (
    <section className="app-card rounded-[28px] border p-5 sm:p-7">
      <div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl" style={{ color: "var(--app-success)", backgroundColor: "var(--app-success-soft)" }}><ShieldCheck size={20} /></span><div><h2 className="text-lg font-black">指定后台管理员</h2><p className="app-muted-text mt-1 text-xs leading-6">负责人可单独授予或撤销普通管理员的编辑权限；CEO 无需单独授权。</p></div></div>
      ...
      <div className="mt-5 space-y-2">{admins.filter((admin) => admin.assigned).map((admin) => <div key={admin.id} className="app-soft-card flex items-center gap-3 rounded-xl border px-3 py-3"><span className="flex h-8 w-8 items-center justify-center rounded-lg" style={{ color: "var(--app-success)", backgroundColor: "var(--app-success-soft)" }}><ShieldCheck size={14} /></span><div className="min-w-0 flex-1"><p className="truncate text-xs font-black">{admin.name}</p><p className="app-muted-text truncate text-[10px]">{admin.email}</p></div><RevokeButton adminId={admin.id} /></div>)}{admins.every((admin) => !admin.assigned) && <p className="app-muted-text rounded-xl border border-dashed p-4 text-center text-xs">还没有单独指定普通管理员。</p>}</div>
    </section>
  );
}
