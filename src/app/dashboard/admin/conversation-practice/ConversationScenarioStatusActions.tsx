...
function StatusButton({ id, status, target, label }: { id: string; status: ConversationStatus; target: ConversationStatus; label: string }) {
  ...
  if (status === target && !state.message) return null;
  return (
    <form action={formAction}>
      {status !== target && <button type="submit" disabled={pending} className="app-soft-card inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] focus-visible:ring-offset-2 disabled:opacity-50">
