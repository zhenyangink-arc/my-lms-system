...
const statusColors = {
  draft: { color: "var(--foreground-muted)", background: "var(--surface-soft)" },
  published: { color: "var(--status-success)", background: "var(--status-success-surface)" },
  archived: { color: "var(--status-warning)", background: "var(--status-warning-surface)" },
};
    ...
    <>
      <section className="overflow-hidden border-y" style={{ borderColor: "var(--border)", backgroundColor: "var(--card)" }}>
        <div className="flex flex-wrap items-center gap-2 border-b px-3 py-3" style={{ borderColor: "var(--border)" }}>
          ...
          {canManage && <Link href={createHref} className="flex items-center gap-1.5 rounded-[6px] px-3 py-2 text-[11px] font-medium text-white" style={{ backgroundColor: "var(--primary)" }}><Plus size={13} />新建场景</Link>}
        </div>
          ...
          <table className="w-full min-w-[820px] border-collapse text-left">
            ...
            <tbody>
              {filteredRows.map((row) => {
                ...
                const name = (
                  <div className="flex min-w-0 items-start gap-2">
                    ...
                    <span className="min-w-0">
                      <span className="flex items-center gap-1.5 font-medium">{row.title}{row.isFeatured && <Star size={10} fill="currentColor" style={{ color: "var(--status-warning)" }} />}</span>
                ...
                return (
                  <tr key={row.id} className="border-b text-[11px] last:border-b-0" style={{ borderColor: "var(--border-subtle)", backgroundColor: row.active ? "var(--accent)" : undefined }}>
                    ...
                    <td className="px-4 py-3 text-right">{canManage ? <Link href={row.editHref} className="font-medium" style={{ color: "var(--primary-hover)" }}>编辑</Link> : <span className="app-muted-text">—</span>}</td>
                  </tr>
                );
              })}
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/25 p-3 backdrop-blur-[1px] sm:p-5" role="dialog" aria-modal="true" aria-label={activeRow ? `${activeRow.title}编辑工作窗` : "新建会话场景工作窗"}>
          <div className="course-editor-window app-card relative flex h-[min(920px,calc(100vh-24px))] w-full max-w-[1400px] flex-col overflow-hidden border shadow-2xl sm:h-[calc(100vh-40px)]" style={{ borderColor: "var(--border)" }}>
            <div className="flex h-12 shrink-0 items-center justify-between border-b px-4" style={{ borderColor: "var(--border)" }}>
              ...
              <button type="button" onClick={() => router.replace(closeHref, { scroll: false })} aria-label="关闭编辑工作窗" className="app-muted-text flex h-8 w-8 items-center justify-center rounded-[6px] border hover:bg-[var(--surface-soft)]" style={{ borderColor: "var(--border)" }}><X size={14} /></button>
            </div>
