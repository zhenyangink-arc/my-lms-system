...
export default async function ConversationScenarioManagementPage({ params }: { params: Promise<{ scenarioId: string }> }) {
  ...
  const { data: profileData, error: profileError } = studentIds.length ? await supabase.from("profiles").select("id,full_name,email").in("id", studentIds) : { data: [] as ProfileRow[], error: null };
  ...
  return (
    <>
      ...
      {(progressResult.error || profileError) && <section role="alert" className="rounded-2xl border p-4 text-sm font-semibold" style={{ color: "var(--status-danger)", backgroundColor: "var(--status-danger-surface)", borderColor: "var(--status-danger)" }}>学生练习数据暂时无法完整读取，请刷新页面重试。</section>}
