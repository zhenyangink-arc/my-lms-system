...
import { MEMBERSHIP_TIER_LABELS, type MembershipTier, type StudentFeature } from "@/lib/student-permissions";
...
function PermissionLink({item,collapsed=false,mobile=false,pathname,dashboardBasePath}:{item:Item;collapsed?:boolean;mobile?:boolean;pathname:string;dashboardBasePath:string}){const Icon=item.icon;const selected=active(pathname,item.href);return <Link href={scopeDashboardPath(item.href,dashboardBasePath)} title={collapsed?item.label:undefined} data-student-operation={item.requiresStudentSectionAccess?"true":undefined} data-permission={item.requiresStudentSectionAccess?(item.studentFeature??"dashboard_section"):undefined} className={mobile?"flex flex-col items-center gap-1 rounded-xl px-1 py-2 text-xs font-bold":`flex items-center gap-2 rounded-xl px-2.5 py-1.5 text-[11px] font-semibold transition ${collapsed?"justify-center":""}`} style={selected?{color:"var(--app-accent-strong)",backgroundColor:"var(--app-accent-soft)",boxShadow:"inset 0 0 0 1px var(--app-accent)"}:{color:"var(--app-muted)"}}><Icon size={mobile?17:15} className="shrink-0" />{!collapsed&&<span className="truncate">{item.label}</span>}</Link>}
