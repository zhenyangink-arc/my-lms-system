        ...
        <>
          ...
          <table className="w-full table-fixed text-left text-sm">
            <thead className="bg-[var(--surface-soft)] text-xs text-[var(--foreground-secondary)]">
              <tr>
                <th scope="col" className="w-2/5 px-4 py-2 font-medium">班级</th>
                <th scope="col" className="px-4 py-2 text-right font-medium">学生</th>
                <th scope="col" className="px-4 py-2 text-right font-medium">今日活跃率</th>
                <th scope="col" className="px-4 py-2 text-right font-medium">必做完成率</th>
              </tr>
            </thead>
            <tbody>
              {institution.classes.map((row) => (
                <tr key={row.classKey} className="border-t first:border-t-0">
                  <th scope="row" className="break-words px-4 py-2 font-medium">{row.className}</th>
                  <td className="px-4 py-2 text-right tabular-nums">{row.studentCount}</td>
      ...
      {institutions.length === 0 ? (
        ...
        <>
          ...
          <table className="w-full table-fixed text-left text-xs">
            <thead className="bg-[var(--surface-soft)] text-xs text-[var(--foreground-secondary)]">
              <tr>
                <th scope="col" className="w-1/6 px-3 py-2 font-medium">机构</th>
                <th scope="col" className="px-2 py-2 text-right font-medium">有效学生</th>
                {metricDefinitions.map((metric) => (
                  <th key={metric.key} scope="col" className="break-words px-2 py-2 text-right font-medium">{metric.title}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {institutions.map((institution) => (
                <tr key={institution.tenantId} className="border-t first:border-t-0">
                  <th scope="row" className="break-words px-3 py-2 font-medium">{institution.tenantName}</th>
                  <td className="px-2 py-2 text-right tabular-nums">{institution.studentCount}</td>
                  {metricDefinitions.map((metric) => (
                    <td key={metric.key} className="px-2 py-2 text-right tabular-nums">
                      {institution[metric.key].rate.toFixed(1)}%
                    </td>
                  ))}
                </tr>
