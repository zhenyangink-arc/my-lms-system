...
function CountCell({ value }: { value: number }) {
  return (
    <td className="border-r border-[var(--border-subtle)] px-4 py-2 font-mono text-xs font-semibold tabular-nums last:border-r-0">
      ...
      <DialogContent className="max-h-[min(900px,calc(100vh-32px))] max-w-[980px] gap-0 overflow-y-auto p-0">
        ...
        <div className="overflow-x-auto border-b border-[var(--border)]">
          <table className="w-full min-w-[760px] border-collapse text-left text-xs">
            <thead className="bg-[var(--surface-soft)] text-[var(--foreground-muted)]">
              <tr>
                <th className="border-r border-[var(--border-subtle)] px-4 py-2 font-medium">统计项目</th>
                <th className="border-r border-[var(--border-subtle)] px-4 py-2 font-medium">完成课时</th>
                <th className="border-r border-[var(--border-subtle)] px-4 py-2 font-medium">学习中课时</th>
                <th className="border-r border-[var(--border-subtle)] px-4 py-2 font-medium">任务提交</th>
                <th className="border-r border-[var(--border-subtle)] px-4 py-2 font-medium">会话练习</th>
                <th className="border-r border-[var(--border-subtle)] px-4 py-2 font-medium">成绩记录</th>
                <th className="px-4 py-2 font-medium">人工辅导备注</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <th className="border-r border-[var(--border-subtle)] px-4 py-2 font-medium">当前数量</th>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] px-5 py-3">
            ...
            {canManageNotes && (
              <button
                ...
                className="inline-flex h-8 items-center gap-1.5 bg-[var(--support)] px-3 text-xs font-semibold text-white"
              >
                <Plus size={13} />
                新增人工辅导备注
              </button>
          ...
          <div className="divide-y divide-[var(--border)]">
            {student.notes.map((note) => (
              <article key={note.id} className="px-5 py-4">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="bg-[var(--surface-soft)] px-2 py-1 text-[10px] font-semibold">
                  ...
                  <time className="ml-auto text-[10px] text-[var(--foreground-muted)]">
                    <LocalDateTime value={note.occurred_at} options={DATE_TIME_OPTIONS} />
                  </time>
                </div>
                <h4 className="mt-2 text-xs font-semibold text-[var(--foreground)]">{note.title}</h4>
                <p className="mt-1 whitespace-pre-wrap text-[11px] leading-5 text-[var(--foreground-secondary)]">{note.content}</p>
                {note.next_action && (
                  <p className="mt-2 border-l-2 border-[var(--primary)] pl-3 text-[11px] leading-5">
                    <span className="text-[var(--foreground-muted)]">下一步建议：</span>
                    {note.next_action}
                  </p>
                )}
            ...
            {student.notes.length === 0 && (
              <div className="px-5 py-10 text-center">
                ...
                <p className="mt-1 text-[10px] text-[var(--foreground-muted)]">系统自动学习档案仍会根据真实学习行为持续更新。</p>
              </div>
            )}
          </div>
        </section>
        ...
        <div className="flex justify-end border-t border-[var(--border)] px-5 py-3">
          <Link
            ...
            className="inline-flex h-8 items-center border border-[var(--border)] px-3 text-xs font-semibold transition-colors hover:bg-[var(--surface-soft)]"
          >
            查看账号档案
          </Link>
        </div>
