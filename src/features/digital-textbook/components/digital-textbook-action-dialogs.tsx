        ...
        <section key={node.id} className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              ...
              className="inline-flex h-8 items-center gap-1.5 border border-[var(--border)] px-3 text-xs font-semibold text-[var(--foreground-secondary)] hover:bg-[var(--surface-soft)]"
            >
            ...
            <table className="w-full min-w-[900px] border-collapse text-left text-xs">
              ...
              <tbody>
                {node.vocabulary.map((word, index) => {
                  ...
                  return (
                    <tr key={key} className="border-t border-[var(--border)]">
                      <td className="px-3 py-2 tabular-nums text-[var(--foreground-muted)]">{index + 1}</td>
                      <td className="px-3 py-2 font-semibold">{word.ko}</td>
                      <td className="px-3 py-2">{word.zh}</td>
                      <td className="px-3 py-2">{word.pos}</td>
                      <td className="px-3 py-2">{word.collocation}</td>
                      <td className="px-3 py-2">{word.transcription}</td>
                      <td className="px-3 py-2 text-right">
                        <button
                          ...
                          className="p-1.5 text-[var(--foreground-muted)] hover:text-[var(--primary)]"
                          aria-label="编辑词汇"
                        >
                        ...
                        <button
                          ...
                          disabled={pending}
                          className="p-1.5 text-[var(--foreground-muted)] hover:text-rose-700 disabled:opacity-50"
                          aria-label="删除词汇"
                        >
                ...
                {node.vocabulary.length === 0 && addingNodeId !== node.id && (
                  <tr>
                    <td colSpan={7} className="px-3 py-8 text-center text-[var(--foreground-muted)]">
                      这个节点还没有词汇
                    </td>
                  </tr>
                )}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={onCancel} className="h-9 border border-[var(--border)] px-3 text-xs font-semibold">
          取消
        </button>
      </div>
    </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          className="inline-flex h-8 items-center gap-1.5 border border-[var(--border)] px-3 text-xs font-semibold hover:bg-[var(--surface-soft)]"
        >
      ...
      {items.length === 0 && !adding && (
        <p className="border border-[var(--border)] px-4 py-8 text-center text-sm text-[var(--foreground-muted)]">
      ...
      <div className="space-y-3">
        {items.map((item, index) =>
          editingIndex === index ? (
            ...
            <article key={index} className="border border-[var(--border)] p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-sm font-semibold text-[var(--foreground)]">{item.title}</h3>
                  <p className="mt-1 text-xs text-[var(--foreground-muted)]">{item.meaning || "暂无中文含义"}</p>
                </div>
                <div className="flex gap-1">
                  <button type="button" onClick={() => setEditingIndex(index)} className="p-2 text-[var(--foreground-muted)] hover:text-[var(--primary)]" aria-label="编辑语法">
                    <Pencil className="size-3.5" />
                  </button>
                  <button type="button" onClick={() => remove(index)} disabled={pending} className="p-2 text-[var(--foreground-muted)] hover:text-rose-700 disabled:opacity-50" aria-label="删除语法">
                    <Trash2 className="size-3.5" />
                  </button>
                </div>
              </div>
...
function GrammarForm({
  ...
  return (
    <div className="space-y-4 border border-[var(--border)] bg-[var(--surface-soft)] p-4">
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={onCancel} className="h-9 border border-[var(--border)] bg-[var(--card)] px-3 text-xs font-semibold">
          取消
        </button>
      </div>
    </div>
...
function EditableRows<T>({
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs font-semibold text-[var(--foreground-secondary)]">{title}</p>
      {rows.map((item, index) => (
        <div key={index} className="flex items-start gap-2">
          ...
          <button type="button" onClick={() => onRowsChange(rows.filter((_, rowIndex) => rowIndex !== index))} className="p-2 text-[var(--foreground-muted)] hover:text-rose-700" aria-label={`删除${title}行`}>
            <Trash2 className="size-3.5" />
          </button>
        </div>
      ))}
      <button type="button" onClick={() => onRowsChange([...rows, createRow()])} className="inline-flex h-8 items-center gap-1.5 border border-[var(--border)] bg-[var(--card)] px-3 text-xs font-semibold">
        <Plus className="size-3.5" />
        {emptyLabel}
      </button>
    </div>
...
function GrammarAudioField({
  ...
  return (
    <div className="space-y-1">
      <div className="flex min-h-9 items-center gap-2 border border-[var(--border)] bg-[var(--card)] px-2">
        <label className="inline-flex cursor-pointer items-center gap-1 text-[11px] font-semibold text-[var(--foreground-secondary)]">
        ...
        {value && (
          <button type="button" onClick={play} disabled={pending} className="inline-flex items-center gap-1 text-[11px] font-semibold text-[var(--primary)] disabled:opacity-50">
            <Volume2 className="size-3.5" />
            试听
          </button>
        )}
        <span className="min-w-0 flex-1 truncate font-mono text-[9px] text-[var(--foreground-muted)]">{value || "尚未上传"}</span>
      </div>
...
function LabeledInput({
  ...
  return (
    <label className="space-y-1">
      <span className="block text-xs font-semibold text-[var(--foreground-secondary)]">{label}</span>
...
function SummaryField({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-[var(--surface-soft)] px-3 py-2">
      <p className="text-[10px] text-[var(--foreground-muted)]">{label}</p>
      <p className="mt-0.5 font-semibold text-[var(--foreground-secondary)]">{value}</p>
    </div>
  );
}
