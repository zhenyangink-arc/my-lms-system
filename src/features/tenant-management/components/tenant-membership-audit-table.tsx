...
const columns: ColumnDef<TenantMembershipAuditItem>[] = [
  { id: "tenant", accessorKey: "tenantName", header: sortableHeader("机构"), cell: ({ row }) => <div className="min-w-36"><p className="font-medium">{row.original.tenantName}</p><p className="mt-0.5 font-mono text-[10px] text-[var(--foreground-muted)]">{row.original.tenantSlug}</p></div> },
  ...
  { id: "change", accessorFn: changeSummary, header: "变更内容", cell: ({ row }) => <span className="min-w-64 text-[var(--foreground-secondary)]">{changeSummary(row.original)}</span> },
...
export function TenantMembershipAuditTable({ data }: { data: TenantMembershipAuditItem[] }) {
  ...
  return <DataTable toolbar={<div className="flex flex-wrap items-center gap-2"><label className="app-input flex h-9 min-w-64 flex-1 items-center gap-2 border px-2.5 sm:max-w-sm"><Search size={14} className="text-[var(--foreground-muted)]" /><span className="sr-only">搜索成员审计记录</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索机构、操作人或目标账号" className="min-w-0 flex-1 bg-transparent text-xs outline-none" /></label><select value={operation} onChange={(event) => setOperation(event.target.value)} className="app-input h-9 border px-2.5 text-xs" aria-label="成员操作"><option value="all">全部操作</option><option value="insert">新增成员</option><option value="update">修改成员</option><option value="delete">移除成员</option></select></div>} isEmpty={filteredData.length === 0} emptyContent="没有符合条件的成员变更记录" footer={<p className="text-xs text-[var(--foreground-muted)]">机构成员变更审计 · 当前显示 {filteredData.length} / {data.length} 条</p>}><Table className="min-w-[1120px]"><TableHeader className="bg-[var(--surface-soft)]">{table.getHeaderGroups().map((group) => <TableRow key={group.id}>{group.headers.map((header) => <TableHead key={header.id} sortDirection={header.column.getCanSort() ? header.column.getIsSorted() : undefined} className="px-4 text-xs">{header.isPlaceholder ? null : flexRender(header.column.columnDef.header, header.getContext())}</TableHead>)}</TableRow>)}</TableHeader><TableBody>{table.getRowModel().rows.map((row) => <TableRow key={row.id}>{row.getVisibleCells().map((cell) => <TableCell key={cell.id} className="px-4 py-3 text-xs">{flexRender(cell.column.columnDef.cell, cell.getContext())}</TableCell>)}</TableRow>)}</TableBody></Table></DataTable>;
}
