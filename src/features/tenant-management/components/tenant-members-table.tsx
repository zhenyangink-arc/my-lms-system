...
const columns: ColumnDef<TenantMemberDetail>[] = [
  { id: "member", accessorKey: "name", header: sortableHeader("成员"), cell: ({ row }) => <div className="min-w-40"><p className="font-semibold">{row.original.name}</p><p className="mt-0.5 font-mono text-[10px] text-[var(--app-muted)]">{row.original.loginId}</p></div> },
...
export function TenantMembersTable({ data, institutionName }: { data: TenantMemberDetail[]; institutionName: string }) {
  ...
  return (
    <DataTable toolbar={<div className="flex flex-wrap items-center gap-2"><label className="app-input flex h-9 min-w-64 flex-1 items-center gap-2 border px-2.5 sm:max-w-sm"><Search size={14} className="text-[var(--app-muted)]" /><span className="sr-only">搜索成员</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索姓名或登录账号" className="min-w-0 flex-1 bg-transparent text-xs outline-none" /></label><select value={role} onChange={(event) => setRole(event.target.value)} className="app-input h-9 border px-2.5 text-xs" aria-label="机构角色"><option value="all">全部角色</option><option value="tenant_super_admin">机构负责人</option><option value="ceo">运营负责人</option><option value="admin">管理员</option><option value="teacher">教师</option><option value="student">学生</option></select><select value={status} onChange={(event) => setStatus(event.target.value)} className="app-input h-9 border px-2.5 text-xs" aria-label="成员状态"><option value="all">全部状态</option><option value="active">正常</option><option value="suspended">已停用</option><option value="invited">待接受</option><option value="left">已离开</option></select></div>} isEmpty={filteredData.length === 0} emptyContent="没有符合条件的机构成员" footer={<p className="text-xs text-[var(--app-muted)]">{institutionName} · 当前显示 {filteredData.length} / {data.length} 条成员关系，本步骤不提供成员修改操作</p>}>
      <Table className="min-w-[840px]"><TableHeader className="bg-[var(--app-soft-bg)]">{table.getHeaderGroups().map((group) => <TableRow key={group.id}>{group.headers.map((header) => <TableHead key={header.id} className="px-4 text-xs">{header.isPlaceholder ? null : flexRender(header.column.columnDef.header, header.getContext())}</TableHead>)}</TableRow>)}</TableHeader><TableBody>{table.getRowModel().rows.map((row) => <TableRow key={row.id}>{row.getVisibleCells().map((cell) => <TableCell key={cell.id} className="px-4 py-3 text-xs">{flexRender(cell.column.columnDef.cell, cell.getContext())}</TableCell>)}</TableRow>)}</TableBody></Table>
    </DataTable>
  );
}
