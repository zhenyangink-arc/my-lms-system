...
const controlClass = "app-input min-h-10 w-full border px-2.5 text-xs outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)] disabled:cursor-not-allowed disabled:opacity-60";
const smallButtonClass = "inline-flex min-h-10 items-center justify-center gap-1.5 border border-[var(--border)] bg-[var(--card)] px-3 text-xs font-semibold text-[var(--foreground-secondary)] transition hover:border-[var(--primary)] hover:text-[var(--primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)] disabled:cursor-not-allowed disabled:opacity-50";
...
export function TeachingBlackboardEditor({
  ...
  const serialized = useMemo(() => JSON.stringify({ mode: "slides", slides }), [slides]);
  ...
  function addElement(type: TeachingBlackboardElementType) {
    if (!selectedSlide) return;
    if (selectedSlide.elements.length >= 12) return;
      ...
      <div className="grid min-w-0 gap-3 xl:grid-cols-[10.5rem_minmax(28rem,1fr)_14rem]">
        <aside className="min-w-0 border border-[var(--border)] bg-[var(--muted)]/25 p-2" aria-label="黑板画面列表">
          <div className="mb-2 flex items-center justify-between gap-2 px-1">
            <span className="text-xs font-bold text-[var(--foreground)]">黑板画面</span>
            <button type="button" onClick={addSlide} disabled={disabled} className="inline-flex h-8 w-8 items-center justify-center text-[var(--primary)] hover:bg-[var(--accent)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)]" aria-label="增加黑板画面"><Plus size={15} aria-hidden="true" /></button>
          </div>
          <div className="grid max-h-[36rem] gap-2 overflow-y-auto pr-1">
            {slides.map((slide, index) => (
              ...
              >
                ...
                <span className="mt-1.5 block truncate text-[10px] font-semibold text-[var(--foreground-secondary)]">{index + 1}. {slide.name}</span>
                <span className="block truncate text-[9px] text-[var(--foreground-muted)]">{lineLabel(scriptLines[slide.segmentIndex] ?? "", slide.segmentIndex)}</span>
              </button>
            ))}
          </div>
        </aside>
        ...
        <div className="min-w-0 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            <button type="button" onClick={() => addElement("text")} disabled={disabled || (selectedSlide?.elements.length ?? 0) >= 12} className={smallButtonClass}><Type size={14} aria-hidden="true" />文字</button>
            <button type="button" onClick={() => addElement("bullets")} disabled={disabled || (selectedSlide?.elements.length ?? 0) >= 12} className={smallButtonClass}><List size={14} aria-hidden="true" />要点</button>
            <button type="button" onClick={() => addElement("expression")} disabled={disabled || (selectedSlide?.elements.length ?? 0) >= 12} className={smallButtonClass}><Languages size={14} aria-hidden="true" />韩语例句</button>
            <span className="ml-auto text-[10px] text-[var(--foreground-muted)]">拖动画面中的内容即可调整位置</span>
          </div>
          {selectedSlide ? (
          ...
          {selectedSlide ? (
            <>
              <div className="space-y-2">
                ...
                <label className="block space-y-1"><span className="text-[10px] text-[var(--foreground-muted)]">画面名称</span><input value={selectedSlide.name} onChange={(event) => updateSlide({ name: event.target.value.slice(0, 40) })} disabled={disabled} className={controlClass} /></label>
                <label className="block space-y-1"><span className="text-[10px] text-[var(--foreground-muted)]">何时显示</span><select value={selectedSlide.segmentIndex} onChange={(event) => updateSlide({ segmentIndex: Number(event.target.value) })} disabled={disabled} className={controlClass}>{scriptLines.map((line, index) => <option key={index} value={index}>{lineLabel(line, index)}</option>)}</select></label>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={duplicateSlide} disabled={disabled} className={smallButtonClass}><Copy size={13} aria-hidden="true" />复制</button>
              ...
              <div className="border-t border-[var(--border)] pt-3">
                ...
                {selectedElement ? (
                  <div className="space-y-2">
                    ...
                    {selectedElement.type === "expression" ? <label className="block space-y-1"><span className="text-[10px] text-[var(--foreground-muted)]">中文释义</span><textarea value={selectedElement.translation ?? ""} onChange={(event) => updateElement({ translation: event.target.value.slice(0, 300) })} disabled={disabled} maxLength={300} rows={2} className={`${controlClass} resize-y py-2 leading-5`} /></label> : null}
                    <div className="grid grid-cols-2 gap-2">
                      <label className="block space-y-1"><span className="text-[10px] text-[var(--foreground-muted)]">字号</span><input type="number" min={12} max={56} value={selectedElement.fontSize} onChange={(event) => updateElement({ fontSize: Math.max(12, Math.min(56, Number(event.target.value) || 12)) })} disabled={disabled} className={controlClass} /></label>
                    ...
                    <div className="grid grid-cols-3 gap-1" aria-label="文字对齐方式">
                      {([ ["left", AlignLeft, "左对齐"], ["center", AlignCenter, "居中"], ["right", AlignRight, "右对齐"] ] as const).map(([align, Icon, label]) => <button key={align} type="button" onClick={() => updateElement({ align })} disabled={disabled} className={`${smallButtonClass} px-2 ${selectedElement.align === align ? "border-[var(--primary)] text-[var(--primary)]" : ""}`} aria-label={label}><Icon size={14} aria-hidden="true" /></button>)}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <label className="block space-y-1"><span className="text-[10px] text-[var(--foreground-muted)]">宽度 %</span><input type="number" min={8} max={100} value={Math.round(selectedElement.width)} onChange={(event) => updateElement({ width: Math.max(8, Math.min(100, Number(event.target.value) || 8)) })} disabled={disabled} className={controlClass} /></label>
                      <label className="block space-y-1"><span className="text-[10px] text-[var(--foreground-muted)]">高度 %</span><input type="number" min={6} max={100} value={Math.round(selectedElement.height)} onChange={(event) => updateElement({ height: Math.max(6, Math.min(100, Number(event.target.value) || 6)) })} disabled={disabled} className={controlClass} /></label>
                    </div>
                ...
                ) : <p className="text-[10px] leading-5 text-[var(--foreground-muted)]">点击画面中的内容后，可以在这里修改文字、大小、颜色和对齐方式。</p>}
              </div>
            </>
          ) : null}
        </aside>
