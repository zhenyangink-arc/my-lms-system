  ...
  return (
    <DataTable isEmpty={data.length === 0} emptyContent="本机构还没有老师账号" footer={<p className="text-xs text-[var(--foreground-muted)]">共 {teachers.length} 位老师，{assignments.length} 条负责关系</p>}>
      <Table className="min-w-[780px]">
        ...
        <TableBody>{table.getRowModel().rows.map((row) => <TableRow key={row.original.id}>{row.getVisibleCells().map((cell) => <TableCell key={cell.id} className="px-4 py-2 text-xs">{flexRender(cell.column.columnDef.cell, cell.getContext())}</TableCell>)}</TableRow>)}</TableBody>
      </Table>
    </DataTable>
  );
}
