      ...
      >
        <table className="w-full min-w-[760px] text-left text-sm">
          <thead className="bg-[var(--surface-soft)] text-xs text-[var(--foreground-secondary)]">
            <tr>
              <th scope="col" className="px-3 py-2 font-medium">月份</th>
              <th scope="col" className="px-3 py-2 font-medium">政策版本</th>
              <th scope="col" className="px-3 py-2 text-right font-medium">机构</th>
              <th scope="col" className="px-3 py-2 text-right font-medium">资格记录</th>
              <th scope="col" className="px-3 py-2 text-right font-medium">符合资格率</th>
              <th scope="col" className="px-3 py-2 text-right font-medium">颁发率</th>
            </tr>
          </thead>
          <tbody>
            {trend.map((point) => (
              <tr key={`${point.periodStart}:${point.policyId}:${point.policyVersion}`} className="border-t">
                <td className="px-3 py-2 tabular-nums">{formatMonth(point.periodStart)}</td>
                <th scope="row" className="px-3 py-2 font-medium">{point.policyCode} 第 {point.policyVersion} 版</th>
                <td className="px-3 py-2 text-right tabular-nums">{point.institutionCount}</td>
                <td className="px-3 py-2 text-right tabular-nums">{point.totalEvaluations}</td>
                <td className="px-3 py-2 text-right tabular-nums">{formatRate(point.eligibleRate)} <span className="app-muted-text text-xs">{point.eligibleCount}/{point.totalEvaluations}</span></td>
                <td className="px-3 py-2 text-right tabular-nums">{formatRate(point.issuanceRate)} <span className="app-muted-text text-xs">{point.issuedCount}/{point.eligibleCount}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
