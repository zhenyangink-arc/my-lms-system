        ...
        <div className="overflow-x-auto border-b border-[var(--border)]">
          <table className="w-full min-w-[760px] border-collapse text-left text-xs">
            <thead className="bg-[var(--surface-soft)] text-[var(--foreground-muted)]">
              <tr>
                <th className="px-4 py-2 font-medium">审核状态</th>
                <th className="px-4 py-2 font-medium">申请阶段</th>
                <th className="px-4 py-2 font-medium">资料进度</th>
                <th className="px-4 py-2 font-medium">学生端状态</th>
                <th className="px-4 py-2 font-medium">审核意见</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                ...
                <td className="max-w-72 whitespace-pre-wrap px-4 py-2 text-[var(--foreground-secondary)]">
            ...
            <table className="w-full min-w-[850px] border-collapse text-left text-xs">
              <thead className="bg-[var(--surface-soft)] text-[var(--foreground-muted)]">
                <tr>
                  ...
                  <th className="px-4 py-2 font-medium">资料名称</th>
                  <th className="px-4 py-2 font-medium">准备状态</th>
                  <th className="px-4 py-2 font-medium">锁定状态</th>
                  <th className="px-4 py-2 font-medium">操作</th>
                  <th className="px-4 py-2 font-medium">截止日期</th>
                  <th className="px-4 py-2 font-medium">管理员备注</th>
                </tr>
              </thead>
              <tbody>
                {application.documents.map((document) => (
                  <tr key={document.id} className="border-t border-[var(--border-subtle)]">
                    <td className="px-4 py-2">{CATEGORY_LABELS[document.category]}</td>
                    <td className="px-4 py-2 font-semibold">{document.title}</td>
                    <td className="px-4 py-2">{ITEM_STATUS_LABELS[document.status]}</td>
                    <td className="px-4 py-2">{document.lockedAt ? "已锁定" : "未锁定"}</td>
                    <td className="px-4 py-2">
                      <DocumentReviewItemLockAction
                        studentId={application.studentId}
                        item={document}
                    ...
                    <td className="px-4 py-2 font-mono">{document.dueDate || "—"}</td>
                    <td className="max-w-72 whitespace-pre-wrap px-4 py-2 text-[var(--foreground-secondary)]">
                      {document.adminNote || "—"}
                    </td>
                  </tr>
