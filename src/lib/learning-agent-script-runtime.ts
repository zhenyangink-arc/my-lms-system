...
export type ScriptLearningLayout = "split" | "learning" | "teaching";

export function scriptSegmentLearningLayout(
  configuration: Record<string, unknown> | null,
  segmentIndex: number,
): ScriptLearningLayout {
  const performances = Array.isArray(configuration?.scriptPerformances)
    ? configuration.scriptPerformances
    : [];
  const value = performances[segmentIndex];
  const performance = value && typeof value === "object" && !Array.isArray(value)
    ? value as Record<string, unknown>
    : {};
  return performance.learningLayout === "learning" || performance.learningLayout === "teaching"
