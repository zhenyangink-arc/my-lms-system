...
export type GradeCenterAccess = { canManage: boolean; canAssignAdmins: boolean; role: UserRole; supabase: Awaited<ReturnType<typeof requireActiveUser>>["supabase"]; user: Awaited<ReturnType<typeof requireActiveUser>>["user"] };
...
export async function requireGradeCenterManager() { const access = await getGradeCenterAccess(); if (!access.canManage) redirect("/dashboard"); return access; }
