...
export function defaultLearningTargetForPage(
  targets: SmartTextbookLearningTarget[],
  pageKey: string,
) {
  const pageTargets = targets.filter((item) => item.pageKey === pageKey);
  return pageTargets.find((item) => item.scope === "page")
    ?? pageTargets.find((item) => item.scope === "region")
    ?? pageTargets[0];
}

export function defaultLearningTargetForRegion(
  targets: SmartTextbookLearningTarget[],
  pageKey: string,
  regionKey: string,
) {
  const regionTargets = targets.filter((item) =>
    item.pageKey === pageKey && item.regionKey === regionKey,
  );
  return regionTargets.find((item) => item.scope === "page" || item.scope === "region")
    ?? regionTargets[0];
}
