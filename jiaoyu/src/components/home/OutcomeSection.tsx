...
const skillItems = [
  ...
  { label: "口语表达", width: "64%", color: "bg-[#6fa0f2]" },
...
export function OutcomeSection() {
  return (
    <section id="outcomes" className="scroll-mt-24 bg-[#f4faff] py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-12">
          ...
          <article className="relative overflow-hidden rounded-[2rem] border border-[#cad4f6] bg-[#f0f8ff] p-6 sm:p-8 lg:col-span-5">
            <div className="absolute -right-12 -top-12 h-40 w-40 rounded-full bg-[#d2dfff]" />
            <div className="relative flex items-center justify-between">
              <div>
                <p className="text-sm font-black text-[#5f7aae]">留学准备进度</p>
                <h3 className="mt-1 text-2xl font-black text-[#35517f]">申请任务同步积累</h3>
              </div>
              <ClipboardList className="text-[#5d92e8]" size={26} />
            </div>
            ...
            <div className="relative mt-8 flex flex-col items-center gap-7 sm:flex-row">
              <div
                ...
                style={{
                  background:
                    "conic-gradient(#679bf0 0deg 282deg, #cfd8f7 282deg 360deg)",
                }}
                aria-label="留学准备进度持续更新"
              >
                <div className="grid h-full w-full place-items-center rounded-full bg-white text-center shadow-inner">
                  <div>
                    <p className="text-2xl font-black text-[#477ac7]">同步</p>
                    <p className="mt-1 text-xs font-bold text-[#6d7da1]">规划与行动</p>
                  </div>
                </div>
              </div>
                ...
                {["院校梯度方案", "材料准备清单", "入学衔接任务"].map((item, index) => (
                  <div key={item} className="flex items-center gap-3 rounded-xl bg-white/75 px-4 py-3">
                    {index < 2 ? (
                      ...
                      <span className={`${styles.routePulse} h-4 w-4 shrink-0 rounded-full bg-[#6398f0]`} />
                    )}
                    <span className="text-sm font-black text-[#475678]">{item}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <article className="rounded-[2rem] border border-[#d8eadf] bg-[#f2f6fb] p-6 sm:p-8 lg:col-span-7">
