...
export function GrowthPathSection() {
  ...
  return (
    <section id="growth-path" className="scroll-mt-24 bg-[#f8fbff] py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="mx-auto max-w-3xl text-center">
          <span className="inline-flex items-center gap-2 rounded-full bg-[#e8f3ff] px-4 py-2 text-sm font-black text-[#2879a9]">
            <Sparkles size={16} />
            双线成长体系
          </span>
            ...
            return (
              <button
                ...
                className={`flex flex-1 items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-black transition sm:text-base ${
                  active
                    ? key === "study"
                      ? "bg-[#e6f3ff] text-[#226f9d] shadow-sm"
                      : "bg-[#e9f4ff] text-[#487cc9] shadow-sm"
        ...
        >
          <div
            className={`relative overflow-hidden p-7 sm:p-10 ${
              activeTrack === "study" ? "bg-[#eaf4ff]" : "bg-[#ebf5ff]"
            }`}
          >
            <div
              className={`absolute -right-14 -top-14 h-40 w-40 rounded-full ${
                activeTrack === "study" ? "bg-[#c7eaff]" : "bg-[#ccddff]"
              }`}
            />
            <div className="relative">
              <span
                className={`flex h-14 w-14 items-center justify-center rounded-2xl text-white shadow-lg ${
                  activeTrack === "study" ? "bg-[#54a9d9]" : "bg-[#6099f4]"
                }`}
              >
                <TrackIcon size={28} />
              </span>
