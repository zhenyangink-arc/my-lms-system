...
-- enforce_application_document_checklist_update only ever checked the
-- checklist item's own admin_locked_at. submitApplicationDocumentsAction
-- locks the whole application by setting
-- student_university_targets.documents_locked_at, but nothing in this
-- trigger looked at that column, so a student could keep calling
-- saveApplicationDocumentDraftAction to flip individual item statuses after
-- submission even though the UI treats the checklist as locked.
...
begin
  ...
  if old.target_id is not null then
    select (documents_locked_at is not null) into target_is_locked
    from public.student_university_targets
    where id = old.target_id;

    if target_is_locked then
      raise exception '申请材料已提交锁定，如需修改请联系管理员解锁';
    end if;
  end if;
