...
-- Platform identities intentionally have no tenant membership. This aggregate
-- keeps the existing platform-owner scope, remains SECURITY INVOKER so table
-- RLS still applies, and correlates every business table to an active tenant.
create or replace function public.get_platform_management_app_overview(
  p_app_id uuid
)
returns table (
  tenant_id uuid,
...
begin
  ...
  time_totals as (
    select
      time_log.tenant_id,
      round(
        sum(greatest(0, coalesce(time_log.seconds, 0)))::numeric / 3600,
        1
      ) as learning_hours
    from public.learning_time_log as time_log
    join active_tenants as tenant
      on tenant.tenant_id = time_log.tenant_id
    where time_log.tenant_id = tenant.tenant_id
      and time_log.student_app_id = p_app_id
    group by time_log.tenant_id
  ),
  scenario_totals as (
    select
      scenario.tenant_id,
  ...
  overview as (
    select
      ...
      coalesce(time_total.learning_hours, 0)::numeric as learning_hours,
    ...
    left join time_totals as time_total
      on time_total.tenant_id = tenant.tenant_id
...
comment on function public.get_platform_management_app_overview(uuid) is
  '平台负责人按应用读取活跃机构级汇总；SECURITY INVOKER，所有业务表逐表绑定 tenant_id，不返回学生级数据。';
