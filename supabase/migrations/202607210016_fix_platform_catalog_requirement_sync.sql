-- 平台负责人修改院校申请资料模板时，会由同步触发器更新已有学生清单。
-- 清单保护触发器仍使用旧版 profiles 角色判断，导致平台负责人被误判为学生。
-- 统一复用平台目录权限函数，确保模板写权限与下游同步权限保持一致。
...
begin
  ...
  select
    ...
    or (
      ...
      and public.current_profile_role() in ('admin', 'ceo', 'super_admin')
    )
  into actor_is_admin;
