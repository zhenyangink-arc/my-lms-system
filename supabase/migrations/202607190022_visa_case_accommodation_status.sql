alter table public.student_visa_cases
  ...
  add constraint student_visa_cases_accommodation_status_check
    check (accommodation_status is null or accommodation_status in ('on_campus_dormitory', 'off_campus_dormitory', 'rental'));

comment on column public.student_visa_cases.accommodation_status is
  '学生填写的住宿安排状态：校内宿舍、校外宿舍或租房。';
...
begin
  ...
  if (to_jsonb(new)
        ...
        - 'planned_entry_date' - 'accommodation_status'
        - 'departure_province' - 'departure_airport'
    ...
    raise exception '学生只能更新自己的递签领区、行程与住宿信息';
  end if;
  return new;
end;
$$;
