...
-- audit_profile_management_change() 之前完全没有跟踪 global_role 这个字段：账号
-- 从普通机构成员切换成平台角色（platform_owner/platform_deputy/platform_admin）
-- 时，虽然 role 字段一般也会跟着变、审计记录还是会生成，但 before_data/after_data
-- 快照里压根没有 global_role 这一项，看不出这次变更具体是不是一次"提权到平台层"，
-- 也看不出提权前后的 global_role 具体是什么。这里补上 global_role 的变化检测和快照。
...
begin
  if tg_op = 'INSERT' then
    ...
    if old.global_role is distinct from new.global_role then
      fields := array_append(fields, 'global_role');
    end if;
    ...
    if 'role' = any(fields) or 'global_role' = any(fields) then
